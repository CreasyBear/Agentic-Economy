...
| Well 7 — simple user expectations | Persona-ordered checklist (buyer, provider, operator, newcomer) built from the 73-item grouped papercut corpus and the Whop/Locus/Nevermined comparisons: outbound events and notifications, API reference from zod and SDK, fleet spend view, settings pages that act (keys rotate, connections, payouts), refund/dispute intake, funding-wall guidance, one app shell (no market↔settings stutter). Copy familiar patterns; no handrolling. | Research outputs in this session; plan to follow in `~/.claude/plans/`. |
