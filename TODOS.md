...
| Well 6 input — business suppression path | No product path writes `suppressedAt`; only the seed does. Eligibility, routing and the projection all honour it, so the switch exists without a lever. Owner: security/removal-dispute flows. | [Well 4 closeout](docs/workflow/well-4-catalogue-truth-closeout.md). |
| Well 6 input — one descriptor shape | The public Tool descriptor is declared in five places (types, zod strict mirror, wire serialize/deserialize, wire type, Convex return validator); adding `listingTier` touched all five. Derive the mirrors from one source. | [Well 4 closeout](docs/workflow/well-4-catalogue-truth-closeout.md). |
