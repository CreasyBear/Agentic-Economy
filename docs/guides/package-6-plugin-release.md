...
Native Claude Code and Cursor connections use the MCP commands generated by
`src/lib/cli-distribution.ts`. The former root `.claude-plugin/` and
`.cursor-plugin/` directories contained only orphan metadata and were retired;
they are not alternate installable bundles. The maintained plugin remains at
`plugins/agentic-economy/`.
