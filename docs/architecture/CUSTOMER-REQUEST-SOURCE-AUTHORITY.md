...
| V2 action preparation and authority boundaries | `src/modules/customer-request/action-preparation.ts`, `src/modules/customer-request/prepared-action-v2.ts`, `src/modules/customer-request/approval-grant-v2.ts`, `src/modules/customer-request/action-attempt-v2.ts` |
...
| Durable V2 preparation, approval, execution, and reconciliation state | `convex/customerRequestV2Preparation.ts`, `convex/customerRequestV2PreparedAction.ts`, `convex/customerRequestV2ApprovalGrant.ts`, `convex/customerRequestV2ActionAttempt.ts`, `convex/customerRequestV2ProviderExecution.ts`, `convex/customerRequestV2ProviderReconciliation.ts` |
| Machine submit, message refinement, facts, resume, comparison, authorization, approval, and action-attempt boundaries | `src/lib/server/customer-request-api.ts`, `src/lib/server/customer-request-messages-api.ts`, `src/lib/server/customer-request-facts-api.ts`, `src/lib/server/customer-request-inspect-api.ts`, `src/lib/server/customer-options-api.ts`, `src/lib/server/customer-request-authorization-api.ts`, `src/lib/server/customer-request-approval-api.ts`, `src/lib/server/customer-request-action-attempt-api.ts` |
...
- `POST /api/requests/:requestRef/approval` approves one prepared action.
- `POST /api/requests/:requestRef/attempts` admits an approved action attempt.
