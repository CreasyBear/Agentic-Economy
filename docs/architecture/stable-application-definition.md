...
| 4 | `/owner/operations` is the single Operations landing; connections, payouts and identity are nested child routes, not parallel destinations. | PACKAGE-2 §2B.1-2B.4 | Resolved by deletion of `owner.settings.{connections,payouts,developers}.tsx` (see Transitions 2); connections and earnings render on the Operations landing, renamed from `/owner/offerings` to `/owner/operations` (Well 8 Lane A). Detail resolves by slug at `/owner/operations/<slug>` for published Tools (Well 8 Lane C); pre-publish Tools still resolve by `offering:` ref until they have a slug (see Smells). | Lane 1: `/owner/operations/connections` (or the §2B path) renders a working section. | Met (2026-09-11) |
...
| C | One read model (single source for catalogue/registry reads, replaces `offering:` refs with real slugs) | Done, latest commit on well-7/app-shell | two real publications appear on local `/market` with `/tools/<provider>/<slug>` hrefs and the Tool page renders; 5,122 tests |
...
| E | OpenAPI (public HTTP surface gets a generated spec) | Done, commit 2beda21c0 | curl 200 and byte size; api-catalog `service-desc` count 6; Playwright 0 console errors |
...
- **Lane C detail:** provider-owned reviewed-tier publications written as `source: provider` rows in `marketDirectorySearchEntries` through the same helpers as Coinbase rows (`upsertProviderDirectoryRows` in `convex/x402DirectoryIndexStore.ts`), on publish/withdraw/republish and the hourly reconcile; tie-break by `sourceRouteRef` (Coinbase row absorbed and suppressed while a Provider row claims the route; reappears on the next scan after withdrawal); sweep exempts provider rows via a sentinel `lastSeenRunAt`; browse/search/facets/bySlug/canonical include them with no special path; owner detail resolves by slug at `/owner/operations/<slug>` (pre-publish Tools still use `offering:` refs, two lifecycle phases, not an alias); interim rail and `readProviderListedToolsProjection` deleted; `ToolCardViewModel` kept for the admitted-registry consumers (`/for-providers` list, command panel).
...
- **Lane E detail:** `/openapi.json` generated from `listCallRouteDescriptors`, `TOOL_MARKET_ACTION_ENTRIES` and `FUNDING_PREFLIGHT_ROUTE_CONTRACTS`, plus data-only descriptors for utility routes (23 operations); one shared RFC 9457 Problem component; 174 KB with `$ref` against 373 KB inlined, validated by `@scalar/openapi-parser`. `/.well-known/api-catalog` carries a `service-desc` entry; `/developers/api` renders Scalar's CDN embed, with the CDN origin allowed in `src/lib/http/security-headers.ts`.
...
- pre-publish Tools carry `offering:` refs in owner URLs until they have a slug.
...
- `ToolCardViewModel` and `IndexedEntry` coexist by design: registry consumers vs market index.
