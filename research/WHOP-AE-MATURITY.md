...
**Date:** 2026-08-24; AE implementation reassessed 2026-09-03
...
The latest implementation delta was re-audited on local `main` at `9d95b4030`. The Packages 1A–1C verification passed **116 focused tests**, typecheck, lint, and the production build. Those checks establish source behaviour, not customer demand or live production proof; the earlier range rescans retain their exact audited revisions and test counts.
...
|---|---|---|
| WHOP-005, 006, 008 | Capability, authority, credential, and resource readiness are separate facts. | **Strong.** AE separates Operation availability, authentication, provider authority, buyer mandate, and delegated budget. See operation projection types (historical reference; unavailable: `src/modules/capability-supply/internal/operation-projection-types.ts`), [agent access policy](../convex/agentAccessPolicy.ts), and credential budget (historical reference; unavailable: `src/modules/money/internal/credential-budget.ts`). |
| WHOP-009, 010, 011, 089, 103 | Consequential work needs stable identity, explicit lifecycle state, safe retry, unknown-outcome handling, and continuation. | **Strong core; incomplete operating proof.** AE has idempotent reservation, worker leases, status, cancellation, reconciliation, and fail-closed unknown states. Operator-side failures now retain safe content, avoid false state claims, preserve command identity after unconfirmed outcomes, and suppress blind duplicates where authoritative readback is required. Generic signed outbound continuation and live proof remain open. See invocation admission (historical reference; unavailable: `convex/lib/operationInvocations/admission.ts`), invocation projection (historical reference; unavailable: `convex/capabilityOperationInvocationProjection.ts`), receipt projection (historical reference; unavailable: `src/modules/capability-execution/invocation-receipt-view.ts`), and side-surface recovery tests (historical reference; unavailable: `tests/unit/routes/invocation-status-route.test.tsx`). |
...
| WHOP-014 | Discovery facts must be refreshed at commitment because price, authority, and provider state change. | **Strong.** Readiness has observed/valid-until boundaries and plan inspection refuses non-routeable supply. See inspect plan (historical reference; unavailable: `src/modules/capability-supply/internal/operation-inspect-plan.ts`) and readiness probes under `src/modules/capability-supply/internal/readiness-probe-*`. |
| WHOP-016, 020, 030, 031, 038 | Money truth includes authorization, settlement, refund, dispute, reserve, and unknown states—not just a charge. | **Strong source foundation; market economics unproven.** AE has canonical Accounts, exact-amount Ledger transactions, Stripe-hosted owner and agent funding, explicit unpaid/processing/ready/failed/expired status, signed-webhook fulfilment, Charges, Provider accrual, Payout, Refund, dispute loss, and reconciliation. Browser return and provider payment status alone cannot create credit. Embedded Elements and its browser key/dependencies have been removed, leaving one hosted funding presentation. See `convex/money*.ts`, [funding actions](../src/modules/money/funding-handoff.actions.ts), and [Stripe evidence](../src/lib/server/stripe-checkout-evidence.ts). There is no live evidence here that the economics work at the expected Operation price and frequency. |
...
| WHOP-100 | Recovery needs inspectable operational history and support tooling, not only correct backend transitions. | **Mixed, locally improved.** Buyers have a receipt timeline and owners have Calls, Connections, audit, and catalogue-health readbacks. Source failures now stay in-shell, show owned retry, and preserve safe partial content. There is still no unified fleet view for stuck invocations, provider-readiness failures, settlement discrepancies, and intervention history. |
| WHOP-104 | Irreducible human action should be a resumable state, not lost context. | **Mixed, with two mature bounded handoffs.** OAuth/device connection and `awaiting_authority` are durable. Agent funding now returns an exact, expiring Stripe URL plus durable Session ID and private polling; the payer-safe public view grants no AE identity or authority. Reconciliation is still technical, and no generic handoff envelope is justified. See [funding actions](../src/modules/money/funding-handoff.actions.ts), [payer return](../src/routes/fund.$fundingSessionId.tsx), invocation status route (historical reference; unavailable: `src/routes/operations.invocations.$invocationRef.tsx`), and [CLI recovery](../tools/ae/commands/recover.ts). |
...
The market thesis depends on AE owning the moment of allocation. Current search ranks lexical overlap: exact token matches score 4, prefixes 2, substrings 1. Filters cover readiness, effects, data classification, currency, price, and a shallow location match against supplier name/slug. Ratings, completion rate, latency, price, buyer context, and expected usefulness do not affect rank. See Operation search (historical reference; unavailable: `src/modules/capability-supply/internal/operation-search.ts`).
...
The durable economic position requires the second comparable need to return through AE. Current evidence kinds are invocation, completed invocation, settlement, qualified use, and reconciliation required. The tracked product analytics still contain substantial retired-surface events and no explicit repeat-use, supplier-switching, or bypass signal. See [.posthog-events.json](architecture/legacy-posthog-events.json) and [market evidence](../convex/marketEvidence.ts).
...
The supplier lifecycle is correct and admirably truthful: describe, admit, check readiness, test, publish, withdraw, republish. But onboarding commonly asks a supplier to provide raw OpenAPI/tool JSON plus contract and commercial metadata, then pass four distinct stages. That protects catalogue authority while creating a high activation burden. See supplier funnel (historical reference; unavailable: `src/components/ae/supply/AeSupplyFunnel.tsx`) and endpoint configuration (historical reference; unavailable: `src/components/ae/supply/AeSupplyEndpointConfigStep.tsx`).
