...
| WHOP-104 | Irreducible human action should be a resumable state, not lost context. | **Mixed, with two mature bounded handoffs.** OAuth/device connection and `awaiting_authority` are durable. Agent funding now returns an exact, expiring Stripe URL plus durable Session ID and private polling; the payer-safe public view grants no AE identity or authority. Reconciliation is still technical, and no generic handoff envelope is justified. See [funding actions](../src/modules/money/funding-handoff.actions.ts), [payer return](../src/routes/fund.$fundingSessionId.tsx), [invocation status route](../src/routes/operations.invocations.$invocationRef.tsx), and [CLI recovery](../tools/ae/commands/recover.ts). |
