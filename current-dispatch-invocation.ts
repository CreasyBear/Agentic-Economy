...
import type { DispatchInvocation } from './types'

export async function currentDispatchInvocation(
  input: Readonly<{
    dispatchRef: string
    now: number
  }>,
  ports: DispatchLifecycleOpenPorts,
): Promise<DispatchInvocation | null> {
  const dispatch = await ports.loadDispatchByRef(input.dispatchRef)
  if (dispatch === null
    || (dispatch.state !== 'pending' && dispatch.state !== 'delivered')
