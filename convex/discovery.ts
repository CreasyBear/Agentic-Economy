...
function buildLlmsTxtFromCatalogs(
  ...
  const body = [
    ...
    `- compare=${canonicalBaseUrl}/api/v1/requests/:requestRef/options`,
    `- confirm=${canonicalBaseUrl}/api/v1/requests/:requestRef/confirmation`,
    `- start-or-resume=${canonicalBaseUrl}/api/v1/requests/:requestRef/run`,
    `- inspect-evidence=${canonicalBaseUrl}/api/v1/requests/:requestRef/evidence`,
    `- report-problem=${canonicalBaseUrl}/api/v1/requests/:requestRef/problems`,
    `- stop=${canonicalBaseUrl}/api/v1/requests/:requestRef/cancellation`,
    ...
    '5. If routes_ready, show the result, businesses, maximum cost, data sharing, effects, evidence, expiry, cancellation, and fallback for each displayed option. Do not infer a choice.',
    '6. Only after explicit customer approval, POST the exact displayed routeRef and revision to /confirmation. Confirmation does not start the work.',
    '7. POST /run with a stable idempotencyKey to start or safely resume the confirmed choice. Never construct businesses, steps, limits, or authority fields.',
    '8. GET /api/v1/requests/:requestRef to resume after interruption or waiting. Follow nextAction and activity.safeNextAction.',
    '9. GET /evidence for observed receipts, POST /problems to report a problem, and POST /cancellation to stop before another step begins when AE says cancellation remains available.',
    '',
    'Catalog entries:',
