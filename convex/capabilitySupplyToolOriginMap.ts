...
/**
 * W1 origin seam: one exact admitted capability-Tool entry per catalog
...
export const offeringToolMapReturns = v.array(v.object({
...
export async function offeringToolMapHandler(
  ctx: QueryCtx,
  args: { businessIds: string[] },
) {
  const entries = await buildOfferingToolMap(ctx, args.businessIds, Date.now())
...
async function buildOfferingToolMap(
  ctx: QueryCtx,
  businessIds: readonly string[],
  now: number,
): Promise<CatalogOfferingToolMapEntry[]> {
  if (businessIds.length === 0) return []
  const entries: CatalogOfferingToolMapEntry[] = []
  for (const businessId of businessIds) {
    ...
    for (const publication of publications) {
      ...
      const parameters = projectCapabilityToolParameters(record)
      const catalogPrice = projectCapabilityToolCatalogPrice(record)
