...
function canInvokeOperation(args: InvokeArgs, principal: AgentAccessPrincipal): boolean {
  return isPublicOperationRef(args.operationRef)
    && principal.scopes.includes(MARKET_OPERATIONS_INVOKE_SCOPE)
}

function shouldPersistRefusal(state: InvokePersistenceState): state is InvokePersistenceState & { reservedInvocationRef: string } {
  return [
    state.reservationWasCreated || state.reservationWasReplayed,
    state.reservationCleanup === 'none',
    state.reservedInvocationRef !== undefined,
  ].every(Boolean)
}
...
async function persistProjectedRefusal(
  ctx: ActionCtx,
  principal: AgentAccessPrincipal,
  result: Extract<OperationResult, { kind: 'refused' }>,
  ...
  await ctx.runMutation(internal.capabilityOperationInvocations.record, {
    invocationRef: state.reservedInvocationRef,
    principalId: principal.principalId,
    state: 'refused',
    result,
...
function projectedInvocationState(
  result: Exclude<OperationResult, { kind: 'refused' }>,
...
async function persistProjectedInvocation(
  ...
  if (!('invocationRef' in result)) return
  if (reservationWasReplayed && result.kind === 'pending') return
  await ctx.runMutation(internal.capabilityOperationInvocations.record, {
    invocationRef: result.invocationRef,
    principalId: principal.principalId,
    state: projectedInvocationState(result),
    result,
...
export async function invokeHandler(
  ...
  await persistProjectedInvokeResult(ctx, principal, projectedResult, {
    ...(reservedInvocationRef === undefined ? {} : { reservedInvocationRef }),
...
function recordPatch(args: RecordArgs): Partial<OperationInvocationRow> {
  return {
    state: args.state,
...
function recoveryProjectionIdentityMatches(row: OperationInvocationRow | null, args: ProjectRecoveryArgs): row is OperationInvocationRow {
...
function isCompletedProjection(row: OperationInvocationRow): boolean {
  return row.state === 'completed' && row.result?.kind === 'completed'
}

function resetsPendingRecovery(row: OperationInvocationRow, args: ProjectRecoveryArgs): boolean {
  return [
...
function recoveryProjectionTransitionAllowed(row: OperationInvocationRow, args: ProjectRecoveryArgs): boolean {
