...
import { callReconciliationValue } from '@/modules/capability-execution/convex'
import {
...
import { projectRecoveryHandler, recordHandler } from './callActions'
import { reconcilePersistedCallAuthority } from './authorityHandlers'
...
type CallReconciliation = Infer<typeof callReconciliationValue>
type ReconciliationReason = CallReconciliation['reason']
...
function isValidCallReconciliation(value: CallReconciliation | undefined): value is CallReconciliation {
...
function hasClaimableReconciliation(
  row: CallRow | null,
  now: number,
): row is CallRow & { reconciliation: CallReconciliation } {
  const reconciliation = row?.reconciliation
  if (row === null || !isValidCallReconciliation(reconciliation)) return false
...
function hasCurrentAutomaticLease(
  row: CallRow,
  reconciliation: CallReconciliation | undefined,
  leaseOwner: string,
  now: number,
): reconciliation is CallReconciliation {
  if (!isValidCallReconciliation(reconciliation)) return false
  return [
    row.state === 'reconciliation_required',
    reconciliation.disposition === 'automatic',
...
type FinishDecision =
  ...
  | Readonly<{ kind: 'manual_review'; reconciliation: CallReconciliation }>
  | Readonly<{ kind: 'retried'; reconciliation: CallReconciliation }>
...
function automaticReconciliationAt(now: number): CallReconciliation {
  return {
    attemptCount: 0,
    nextAttemptAt: now,
...
export async function recordWithReconciliationInitialization(
  ...
  await initializeReconciliationIfAbsent(ctx, args.callRef, args.now)
  return result
}
...
export async function claimDispatchWithReconciliationInitialization(
  ...
  await initializeReconciliationIfAbsent(ctx, args.dispatch.callRef, Date.now())
  return result
}
...
export async function cancelBeforeClaimWithReconciliationInitialization(
  ...
  await initializeReconciliationIfAbsent(ctx, args.callRef, Date.now())
  return result
}
...
export async function completeWorkWithReconciliationInitialization(
  ...
  const authority = await reconcilePersistedCallAuthority(ctx, args.context.callRef, Date.now())
  const result = await completeWorkHandler(ctx, authority === null
    ...
    : args)
  await initializeReconciliationIfAbsent(ctx, args.context.callRef, Date.now())
  return result
}
...
export async function listDueAutomaticReconciliationCandidatesHandler(
  ...
  return rows.flatMap((row) => {
    const reconciliation = row.reconciliation
    if (
      !isValidCallReconciliation(reconciliation)
      || reconciliation.disposition !== 'automatic'
...
export async function claimAutomaticReconciliationCandidateHandler(
  ctx: MutationCtx,
  args: { callRef: string; leaseOwner: string; now: number },
  ...
  const row = await ctx.db.query('capabilityCalls')
    .withIndex('by_callRef', (q) => q.eq('callRef', args.callRef))
    .unique()
