...
async function projectIdentity(
  ...
  identity: Doc<'capabilityProviderToolProjections'>,
  now: number,
  ...
  const [calls, qualifiedUses] = includeEvidence
    ? await Promise.all([
        ctx.db.query('capabilityCallProjections').withIndex('by_toolRef_and_createdAt', (query) => query.eq('toolRef', identity.toolRef).gte('createdAt', windowStartAt)).take(EVIDENCE_READ_LIMIT + 1),
        ctx.db.query('qualifiedUseReceipts').withIndex('by_toolRef_and_qualifiedAt', (query) => query.eq('toolRef', identity.toolRef).gte('qualifiedAt', windowStartAt)).take(EVIDENCE_READ_LIMIT + 1),
      ])
    : [[], []]
...
export const readOwner = query({
  ...
  handler: async (ctx, args) => {
    ...
    const identity = await ctx.db.query('capabilityProviderToolProjections')
...
export const listOwner = query({
  ...
  handler: async (ctx, args) => {
    ...
    const rows = await ctx.db.query('capabilityProviderToolProjections')
...
export const readAgent = mutation({
  args: { ...agentReadArgs, toolRef: v.string() },
  returns: readResultValue,
  handler: async (ctx, args) => {
    if ((await requireSourceWrite(ctx, args, 'catalog_publish')).kind === 'rejected') return { kind: 'not_found' as const }
    if ((await authorize(ctx, args)) === null) return { kind: 'not_found' as const }
    const identity = await ctx.db.query('capabilityProviderToolProjections')
      .withIndex('by_businessId_and_toolRef', (query) => query.eq('businessId', args.businessId).eq('toolRef', args.toolRef))
      .unique()
    if (identity === null) return { kind: 'not_found' as const }
