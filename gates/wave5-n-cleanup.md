...
- [x] G1: Six obsolete packages are absent and required retained packages remain.
  ...
  EVIDENCE: `43ace4bf` removed the six direct roots, reducing root packages from 79 to 73. The required Agent 0.7.1, AI SDK, OpenRouter, TanStack AI, Convex helpers, and YAML roots remain. Solana/x402 strings are transitive Coinbase/CDP metadata only.
...
- [x] G3: Lockfile, manifest, package audit, and build remain green.
  ...
  EVIDENCE: Node 22/npm 11 regenerated the lockfile; clean install, 62 focused tests, 18 gateway-smoke unit tests, development manifest validation, lint, typecheck, and production build pass. `npm audit --omit=dev` reports zero vulnerabilities.
