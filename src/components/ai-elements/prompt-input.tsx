...
export const PromptInput = ({
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      // Validate against the latest attachment count, then call onError
      // outside the updater: React may replay a state updater, so a side
      // effect inside it could fire more than once.
      ...
      if (validated.files.length === 0) return;
      const incoming = validated.files.map((file) => ({
        filename: file.name,
        id: nanoid(),
        mediaType: file.type,
        type: "file" as const,
