  ...
  return (
    <AePublicShell immersive>
      <div className="flex h-full min-h-0 w-full flex-col bg-background">
        <header className="sticky top-0 z-20 flex min-h-12 items-center justify-between gap-3 border-b border-border bg-background px-4 md:px-6">
          <Link to="/" className="inline-flex min-h-11 items-center rounded-sm font-mono text-xs font-semibold uppercase tracking-wider text-foreground no-underline hover:text-primary">
            Agentic Economy / Market
          </Link>
          <Button asChild variant="secondary" size="sm" className="min-h-11">
            <a href="/t/new">Start a search</a>
          </Button>
        </header>
        {projection === null ? (
          <div className="mx-auto my-12 w-full max-w-[36rem] px-4 md:px-6">
            <Empty className="border border-border bg-card p-5">
              <EmptyHeader>
                <h1 className="text-lg font-medium tracking-tight">Shared search unavailable</h1>
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="mx-auto w-full max-w-[56rem] px-4 pt-4 md:px-6">
              <p className="font-mono text-xs uppercase tracking-wider text-muted-foreground" role="note">Shared read-only search</p>
              <h1 dir="auto" style={{ unicodeBidi: 'isolate' }} className="mt-1 truncate text-lg font-semibold text-foreground">{displayTitle}</h1>
            </div>
