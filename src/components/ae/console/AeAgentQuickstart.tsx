  ...
  return (
    <section className="grid gap-3">
      ...
      <dl className="divide-y rounded-lg border">
        {items.map((item) => (
          <div key={item.command} className="grid gap-1 px-4 py-3 md:grid-cols-[minmax(16rem,0.8fr)_minmax(0,1fr)] md:gap-5">
            <dt><AeCopyCommand compact label={`${title} command`} code={item.command} /></dt>
            <dd className="grid gap-1 text-sm text-muted-foreground"><code>{item.route}</code><span>{item.description}</span></dd>
          </div>
        ))}
      </dl>
    </section>
