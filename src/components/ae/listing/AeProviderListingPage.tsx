      ...
      <aside className="ae-listing-sticky-rail" aria-label="Actions for this business">
        <div className="ae-listing-sticky-cta">
          {inquiryAffordance.kind === 'available' ? (
            ...
            <p className="ae-listing-page__contact-note">{inquiryAffordance.reason}</p>
          )}
        </div>
        <AeProtectedByAe />
...
function ListingPhotosSection({ catalog }: { catalog: PublicRouteCatalogContract }) {
  ...
  return (
    <section className="ae-listing-page__section ae-listing-page__section--photos" aria-labelledby="listing-photos">
      <h2 id="listing-photos">Photos</h2>
      {photos.length > 0 ? (
        <ul className="ae-listing-page__photos">
          {photos.map((photo) => (
            <li key={photo.url}>
              <figure className="ae-listing-page__photo">
        ...
        <figure className="ae-listing-page__photo ae-listing-page__photo--illustration">
          <img src={illustration} alt="" />
          <figcaption className="ae-listing-page__photo-caption">Hand-drawn category mark</figcaption>
        </figure>
      )}
    </section>
  )
