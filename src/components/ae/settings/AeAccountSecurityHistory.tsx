      ...
      {result.kind === 'unavailable' ? (
        ...
        <div className="grid gap-related">
          <AeRecordTable
            ...
            maxHeight="32rem"
          />
          {result.page.isDone ? null : (
            <Button
              type="button"
              variant="secondary"
              className="justify-self-start"
              disabled={loading}
              onClick={() => void load(result.page.continueCursor)}
            >
...
function historyColumns(): ColumnDef<AccountSecurityHistoryItem, unknown>[] {
  return [
    {
      id: 'time',
      header: 'Time',
    ...
    },
    {
      id: 'actor',
      header: 'Actor',
      accessorFn: (row) => `${row.actorKind} ${row.actorRef}`,
      cell: ({ row }) => (
    ...
    },
    {
      id: 'target',
      header: 'Target',
      accessorFn: (row) => `${row.targetType} ${row.targetRef}`,
      cell: ({ row }) => (
        <span className="grid gap-1">
