...
function OptionsCard({ projection, turns, edit, restart }: { projection: CustomerRequestView; turns: readonly ConversationTurn[]; edit: () => void; restart: () => void }) {
  ...
  return <section className="grid gap-6" aria-live="polite"><Conversation turns={turns} /><WorkingUnderstanding projection={projection} correct={edit} /><div className="grid gap-2"><Text className="text-sm font-semibold text-accent">{isSingle ? 'One option found' : 'Options found'}</Text><Heading level={2} className="text-3xl">{isSingle ? 'One registered option matched.' : `${projection.options.length} registered options found.`}</Heading><Text color="secondary">{isSingle ? 'This is not a comparison or recommendation.' : optionSet?.ordering.kind === 'unranked' ? 'These options are not ranked. AE has not recommended one.' : 'Compare the provider-reported details that matter to you.'} Nothing has been selected, booked, or purchased.</Text>{coverage ? <Text type="supporting" color="secondary">AE evaluated {coverage.evaluated} connected {coverage.evaluated === 1 ? 'business' : 'businesses'}: {coverage.optionsReceived} returned an option, {coverage.unavailable} unavailable, {coverage.pending} pending, {coverage.uncertain} uncertain.</Text> : null}</div><div className="grid gap-4 md:grid-cols-2">{projection.options.map((candidate) => <Card key={candidate.optionRef} padding={5}><article className="grid gap-3"><div><Text type="supporting" color="secondary">Provider-reported option</Text><Heading level={3}>{candidate.business.name}</Heading></div><Text type="large" weight="semibold">{formatMoney(candidate.expectedCost.currency, candidate.expectedCost.amountMinor)}</Text>{candidate.maximumCost.amountMinor !== candidate.expectedCost.amountMinor ? <Text type="supporting" color="secondary">Provider maximum {formatMoney(candidate.maximumCost.currency, candidate.maximumCost.amountMinor)}</Text> : null}{candidate.comparableOutputs.map((output) => <Text key={output.label} color="secondary"><strong>{output.label}:</strong> {String(output.value)}</Text>)}{candidate.materialTerms.map((term) => <Text key={term} color="secondary">Provider term: {term}</Text>)}<Text type="supporting" color="secondary">Provider cancellation: {candidate.cancellation.summary}</Text><Text type="supporting" color="secondary">Valid until {formatOptionTime(candidate.provenance?.validUntil ?? candidate.expiresAt)}</Text></article></Card>)}</div><RecoveryActions edit={edit} restart={restart} /></section>
}
