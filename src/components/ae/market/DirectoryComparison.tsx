...
export function DirectoryComparison({ entries, open, onOpenChange, onCloseAutoFocus, onSelect }: Readonly<{
    ...
    <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-5xl" onCloseAutoFocus={onCloseAutoFocus}>
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow><TableCell />{entries.map(entry => <TableCell key={entry.resource}><Button variant="outline" onClick={() => { onOpenChange(false); onSelect(entry) }}>Inspect Tool</Button></TableCell>)}</TableRow>
        </TableBody>
      </Table>
    </DialogContent>
  </Dialog>
