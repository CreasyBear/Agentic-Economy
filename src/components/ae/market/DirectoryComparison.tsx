...
export function DirectoryComparison({ entries, open, onOpenChange, onCloseAutoFocus, returnTo }: Readonly<{
    ...
    <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-5xl" onCloseAutoFocus={onCloseAutoFocus}>
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow><TableCell />{entries.map(entry => <TableCell key={entry.resource}><Button asChild variant="outline"><Link to="/tools/$toolRef" params={{ toolRef: x402PendingToolRef(entry.resource) }} {...(returnTo === undefined ? {} : { search: { from: returnTo } })}>Inspect Tool</Link></Button></TableCell>)}</TableRow>
        </TableBody>
      </Table>
    </DialogContent>
  </Dialog>
