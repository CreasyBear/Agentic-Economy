...
import { ArrowUpRight, BracesIcon } from 'lucide-react'
...
export function DirectoryIndexDiscovery({ overview, returnTo, onSave, isSaved, onCompare, isComparing, compareDisabled }: Props) {
  ...
  return <div className="grid min-w-0 gap-12 pb-10 sm:gap-16">
    <p className="-mb-6 text-xs text-muted-foreground">Find your next favourite. See what other agents use.</p>
      ...
      <Carousel opts={{ align: 'start', loop: false }} aria-labelledby={`${id}-${shelf.key}`}>
        <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button asChild variant="ghost" size="sm" className="mr-1 rounded-full"><Link to="/market" search={{ view: 'tools', sort: shelf.sort }} aria-label={`View all ${shelf.title.toLowerCase()}`}>View all<ArrowUpRight className="size-4" /></Link></Button>
    ...
    <section aria-labelledby={`${id}-providers`}>
      <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
        ...
        <Button asChild variant="ghost" size="sm" className="rounded-full"><Link to="/market" search={{ view: 'providers' }}>All Providers<ArrowUpRight className="size-4" /></Link></Button>
      </div>
      {overview.providers.length === 0 ? <p className="text-sm text-muted-foreground">No Provider hostnames are available for this selection.</p> : <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {overview.providers.map(provider => <li key={provider.key} className="min-w-0">
          <Card className="h-full gap-0 border-border/70 py-0 shadow-none"><CardContent className="p-0">
            <Link to="/market" search={{ view: 'tools', provider: provider.key }} aria-label={`View Tools from ${provider.label}`} className="flex min-w-0 items-center gap-3 rounded-card p-4 transition-colors hover:bg-muted/40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">
