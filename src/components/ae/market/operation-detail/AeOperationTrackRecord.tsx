...
import { AeOperationLatencyChart } from './AeOperationLatencyChart'

export function AeOperationTrackRecord({
  evidence,
}: Readonly<{
    ...
    >
      ...
      <dl className="grid min-w-0 grid-cols-2 border-y border-border [&>div:nth-child(-n+2)]:border-b sm:grid-cols-4 sm:[&>div:nth-child(-n+2)]:border-b-0">
        <TrackMetric
          ...
          value={evidence.popularity.completedInvocations.toLocaleString()}
      ...
      <div className="min-w-0 px-gutter py-5">
        <AeOperationLatencyChart latency={evidence.latency} />
      </div>
    </section>
  )
}
