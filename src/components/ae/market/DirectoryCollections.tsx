...
export function DirectoryCollections({ window }: Readonly<{ window: MarketWindow }>) {
  return <nav aria-label="Explore by job" className="grid auto-cols-[72%] grid-flow-col gap-3 overflow-x-auto pb-1 md:auto-cols-auto md:grid-flow-row md:grid-cols-4">
    {collections.map(collection => <Link key={collection.title} to="/market" search={{ window, query: collection.query }} className="group relative isolate min-h-36 overflow-hidden rounded-card bg-foreground text-background focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">
