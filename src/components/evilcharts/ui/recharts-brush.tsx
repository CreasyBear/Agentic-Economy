...
export interface BrushProps {
  height?: (number) | undefined; // brush preview strip height in px
  formatLabel?: ((value: unknown, index: number) => string) | undefined; // formats the range-handle labels
  onChange?: ((range: EvilBrushRange) => void) | undefined; // fires as the range moves
}
...
interface EvilBrushProps {
  ...
  variant?: (EvilBrushVariant) | undefined;
  ...
  strokeVariant?: ("solid" | "dashed" | "animated-dashed") | undefined;
  ...
  defaultEndIndex?: (number) | undefined;

  /** Fired whenever the visible range changes */
  onChange?: ((range: EvilBrushRange) => void) | undefined;
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 4, right: 0, bottom: 0, left: 0 }}>
          ...
          {keys.map((dk) => (
            <Line
              key={dk}
              {...(curveType === undefined ? {} : { type: curveType })}
              dataKey={dk}
