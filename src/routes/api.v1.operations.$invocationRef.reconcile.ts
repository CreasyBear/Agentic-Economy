...
import { handleOperationInvokeReconcilePost } from '@/lib/server/operation-invoke-api'
import { methodNotAllowed } from '@/lib/server/method-guard'
import { OPERATION_INVOKE_ROUTE_CONTRACT } from '@/modules/capability-execution/operation-invoke-entry'

const routeContract = OPERATION_INVOKE_ROUTE_CONTRACT.reconcile

export const Route = createFileRoute('/api/v1/operations/$invocationRef/reconcile')({
  server: {
    handlers: {
      [routeContract.method]: ({ request, params }) => handleOperationInvokeReconcilePost(request, params.invocationRef),
