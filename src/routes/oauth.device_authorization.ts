...
import { handleDeviceAuthorizationPost } from '@/lib/server/agent-access-oauth-api'
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        return await handleDeviceAuthorizationPost(request, { store: createConvexAgentAccessOAuthStore(request, body), rateLimit: admitOAuth })
      },
