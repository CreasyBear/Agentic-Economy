...
function FundingReturnPage() {
  ...
  if (funding.state === 'ready') return <AePageState tone="positive" title="Account credit added" description={`${amount} is ready for ${funding.agentName}. You can return to the agent.`} />
  if (funding.state === 'processing') return <AePageState tone="neutral" title="Payment received" description={`Account credit is being confirmed for ${funding.agentName}. Do not pay again.`} />
  if (funding.state === 'awaiting_payment') return <AePageState tone="neutral" title="Payment not completed" description={`Return to Stripe to finish adding ${amount} for ${funding.agentName}.`} />
  return <AePageState tone="neutral" title="Funding link ended" description={`Ask ${funding.agentName} to create a new funding link.`} />
}
