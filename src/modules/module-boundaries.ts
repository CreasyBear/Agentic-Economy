...
export const MODULE_BOUNDARY_MANIFEST: ModuleBoundaryManifest = {
  modules: [
    ...
    { name: 'agent-access', entrySurfaces: ['public.ts', 'contract.ts', 'account.actions.ts', 'issued-agent-binding.ts', 'agent-access.ts', 'agent-access.functions.ts', 'policy.ts', 'policy.functions.ts', 'production-policy.ts', 'sandbox-policy.ts', 'service-auth-envelope.ts', 'agent-access-console.ts', 'agent-operator-view-model.ts', 'oauth-state.ts'], allowedDependencies: ['common', 'capability-contract', 'security', 'money', 'principal-account'] },
