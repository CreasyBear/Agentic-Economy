...
const defaultAllowedSignatureAgents = ['https://chatgpt.com'] as const
...
type DirectoryJwk = JsonWebKey & { kid?: string }
...
export async function verifyAgentIdentity(
  ...
  if (options.bodyText !== undefined) {
    ...
    const expectedContentDigest = sourceWriteContentDigestHeader(options.bodyText)
  ...
  if (new URL(request.url).host !== expectedAuthority) {
    return identityError('authority_mismatch', 401, 'Signature authority does not match this agent door.')
  }
  ...
  const allowedSignatureAgents = options.allowedSignatureAgents ?? defaultAllowedSignatureAgents
  ...
  const pretrustedDirectoryOrigins = options.pretrustedDirectoryOrigins ?? defaultAllowedSignatureAgents
