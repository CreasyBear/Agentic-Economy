...
function canonicalTicket(
  ...
  if (candidate === undefined
    ...
    || !OPAQUE_REF.test(candidate.callRef)
    || !OPAQUE_REF.test(candidate.toolRef)
    ...
    || candidate.callRef !== callRef
    || candidate.toolRef !== toolRef
...
export function providerConsequenceInvocationDigest(
  ...
  const authority = invocation.authority
  if (authority.callRef === undefined
    || authority.toolRef === undefined
  ...
  return canonicalDigest({
    ...
    authority: {
      ...
      // The v1 preimage keys are historical evidence. Current authority
      // values are mapped here without changing the encoded bytes.
