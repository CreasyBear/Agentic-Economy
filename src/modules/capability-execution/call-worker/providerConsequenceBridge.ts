...
export async function invokeProviderConsequenceViaVercel(
  ...
  const commandId = `provider-effect:${invocation.authority.callRef}:${invocation.authority.attemptRef}:${invocation.authority.effectGeneration}`
  ...
  try {
    issue = await ctx.runMutation(
      ...
      callRef: invocation.authority.callRef,
      toolRef: invocation.authority.toolRef,
