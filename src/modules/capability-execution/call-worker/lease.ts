...
> {
  ...
  const lease = await ctx.runMutation(internal.capabilityProviderConnections.issueLease, {
    commandId: `operation-lease:${input.dispatch.callRef}:${input.durableAttemptRef}`,
    leaseRef: `operation-lease:${input.dispatch.callRef}:${input.durableAttemptRef}:${input.durableEffectGeneration}`,
...
export async function settleProviderLease(
  ...
  const commandPrefix = `operation-lease:${dispatch.callRef}:${attemptRef}:${effectGeneration}`
...
export function providerCredentialReader(
  ...
  return async (lookup) => {
    if (
      ...
      || lookup.callRef !== dispatch.callRef
      || lookup.toolRef !== dispatch.toolRef
    ...
    return await ctx.runQuery(internal.capabilityProviderConnections.resolveLeaseCredentialRef, {
      ...
      connectionRef: lookup.connectionRef,
      callRef: lookup.callRef,
      toolRef: lookup.toolRef,
