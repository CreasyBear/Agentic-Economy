...
export async function reconcilePreSubmissionRecovery(
  ...
  if (proof === undefined) return recoveryNotFound(args.callRef)
...
function preparePreSubmissionProof(work: RecoveryWorkContext): PreSubmissionProof | undefined {
  ...
  const evidenceMaterial = {
    ...
    version: 1 as const,
    evidenceRef,
    source: `published-tool:${operation.operationId}`,
    invocationRef: recovered.callRef,
    attemptRef: x402Attempt.attemptRef,
    effectGeneration: x402Attempt.effectGeneration,
    operationRef: recovered.toolRef,
...
function paymentIdentityMatchesInvocation(
  ...
  return attempt.dispatchRef === recovered.callRef
    && attempt.toolRef === recovered.toolRef
