...
import { pricingConfigDecisionAmount, type ExactAmount, type MoneyAcceptedCallCharge } from '@/modules/money/public'
import type { StableHashValue } from '@/modules/common/stable-hash'
import { uniqueSorted } from '@/modules/common/unique-sorted'

export type CallMaterialInput = Readonly<{
  operationKey: string
...
export type RecoveryControlResult = ActionResult & Readonly<{
  ...
  failureCode?: string
  usage?: Pick<MoneyAcceptedCallCharge, 'usageRef' | 'observedAt' | 'chargeState' | 'amount' | 'priceDigest' | 'transactionRef'>
}>
export function buildCallMaterial(input: Readonly<{
  operation: PublishedTool
  descriptor: RuntimePublishedToolDescriptor
  value: StableHashValue
}>): CallMaterialInput {
  return buildCallMaterialForAmount({
    ...input,
    decisionAmount: executableFixedPrice(input.operation),
  })
}

export function buildCommittedCallMaterial(input: Readonly<{
  operation: PublishedTool
  descriptor: RuntimePublishedToolDescriptor
  value: StableHashValue
  decisionAmount: ExactAmount
}>): CallMaterialInput {
  return buildCallMaterialForAmount(input)
}

function buildCallMaterialForAmount(input: Readonly<{
  operation: PublishedTool
  descriptor: RuntimePublishedToolDescriptor
  value: StableHashValue
  decisionAmount: ExactAmount
  ...
  const target: StableHashValue = Object.assign(
    ...
    stablePublishedToolIdentity(input.operation.identity),
    {
      readiness: {
  ...
  const sourceSnapshotDigest = callMaterialSourceDigest(input.operation, input.descriptor)
...
export function callMaterialSourceDigest(
  operation: PublishedTool,
  descriptor: RuntimePublishedToolDescriptor,
): string {
...
export function assertExactDescriptor(
  ...
  if (
    ...
    || publishedToolIdentityDigest(descriptor.target)
      !== publishedToolIdentityDigest(operation.identity)
...
export function createRecoveryControlAction(input: Readonly<{
...
}>): Action<CallMaterialInput, RecoveryControlResult> {
  ...
  return {
    ...
    schema: z.unknown() as z.ZodType<CallMaterialInput>,
    parameters: [],
    readOnly: false,
    effect,
    surfaces: [],
    ...
    invocationContract: {
      ...
      reconciliationEvidenceSource: `published-tool:${operation.operationId}`,
    },
    projectInvocationPreparation: {
      project: (_value: CallMaterialInput) => ({
        dataUse: {
    ...
    preReleaseCheck: {
      check: async ({ data, context }: { data: CallMaterialInput; context: ActionContext }) =>
...
function stablePublishedToolIdentity(
  identity: PublishedTool['identity'],
  ...
  return {
    ...
    ...(connectionAuthority === undefined ? {} : {
      connectionAuthority: {
        ...
        toolRef: connectionAuthority.toolRef,
...
function stablePayment(payment: PublishedTool['identity']['payment']): StableHashValue {
...
function stablePricingConfig(config: PublishedTool['pricingConfig']): StableHashValue {
