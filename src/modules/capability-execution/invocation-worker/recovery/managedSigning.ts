...
export async function reconcileManagedSigningRecovery(
  ...
  if (attempt === undefined) {
    observeStage(args.invocationRef, 'attempt_not_eligible')
    return recoveryNotFound(args.invocationRef)
  }

  const expiresAt = expiredAuthorizationTime(attempt)
  if (expiresAt === undefined) {
    observeStage(args.invocationRef, 'authorization_not_expired')
  ...
  const replay = await replayManagedX402SigningForRecovery(
    ...
    work.operation,
    work.recovered.operationRef,
    work.recovered.environment,
  )
  observeStage(args.invocationRef, `cdp_replay_${replay.kind}`, unresolvedReplayDiagnostic(replay))
  if (replay.kind !== 'definitive_rejection') return projectPersistedRecovery(work.recovered)
  return await finalizeManagedSigningRecovery(ctx, args.invocationRef, work, attempt, expiresAt, replay, alreadyRetryable)
}
...
async function finalizeManagedSigningRecovery(
  ...
  if (prepared === undefined) {
    observeStage(invocationRef, 'observation_identity_incomplete')
    return projectPersistedRecovery(work.recovered)
  }
  const reconciled = await reconcileManagedSigningMoney(ctx, work, attempt, observedAt, replay, prepared)
  if (!reconciled) {
    observeStage(invocationRef, 'money_reconciliation_refused')
    return projectPersistedRecovery(work.recovered)
  }
  return await advanceManagedSigningControl(ctx, invocationRef, work, prepared.evidence, alreadyRetryable)
}
...
function prepareManagedSigningEvidence(
  ...
  const evidenceMaterial = {
    ...
    version: 1 as const,
    evidenceRef,
    source: `published-operation:${work.operation.operationId}`,
    invocationRef: work.recovered.invocationRef,
    attemptRef: attempt.attemptRef,
    effectGeneration: attempt.effectGeneration,
    operationRef: work.recovered.operationRef,
...
async function advanceManagedSigningControl(
  ...
  if (!reconciliationReachedRetryable(reconciliation)) {
    observeStage(invocationRef, 'control_reconciliation_refused')
    return projectPersistedRecovery(work.recovered)
  }
  observeStage(invocationRef, 'retryable')
...
>> {
  ...
  if (loaded.kind === 'persisted') {
    observeStage(args.invocationRef, 'recovery_control_not_ready')
