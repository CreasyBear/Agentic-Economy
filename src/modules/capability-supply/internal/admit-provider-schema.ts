...
export type AdmitCredentialSpec =
  ...
  | Readonly<{ kind: 'api_key'; location: 'query' | 'header'; name: string; schemeName: string }>
 ...
 * schema against a resolution root (the OpenAPI document or MCP tool). Injected so the
 * convex-reachable admission chain never statically imports a Node-API library: the Convex seed
 * mutation runs in the isolate runtime and can only process already-conformant (ref-free)
 * schemas, so callers without a dereferencer get a named refusal on expansion instead. Node-side
 * callers (owner funnel, CLI, tests) pass `dereferenceOpenApiSchema` from './schema-deref'.
 */
export type SchemaDereferencer = (schema: SchemaRecord, root: unknown) => Promise<SchemaRecord>
...
export async function admitProviderSchema(
  ...
  const inlineInput = await inlineSchemaReferences(input.inputSchema, input.resolutionRoot, derefSchema)
  if (inlineInput.kind === 'refused') return inlineInput
  const inlineOutput = await inlineSchemaReferences(input.outputSchema, input.resolutionRoot, derefSchema)
  ...
  const contract = deriveAdmissionMetadata(boundedInput, input.contract, outputPlan.evidencePointer)
...
async function inlineSchemaReferences(
  ...
  if (derefSchema === undefined) return { kind: 'refused', reason: 'admit_schema_deref_unavailable' }
  let dereferenced: SchemaRecord
  try {
    dereferenced = await derefSchema(schema, root)
  } catch {
    return { kind: 'refused', reason: 'admit_schema_reference_unresolvable' }
  }
  const residual = residualReferenceKind(dereferenced as JsonValue)
  if (residual === 'unresolvable') return { kind: 'refused', reason: 'admit_schema_reference_unresolvable' }
  if (residual === 'circular') return { kind: 'refused', reason: 'admit_schema_circular_reference' }
  return { kind: 'ok', schema: dereferenced }
}

function schemaNeedsExpansion(schema: Readonly<Record<string, JsonValue>>): boolean {
  ...
  while (pending.length > 0) {
    ...
    if (Array.isArray(current)) {
      ...
      if ('$ref' in current || 'allOf' in current || 'oneOf' in current || 'anyOf' in current) return true
...
type ResidualReferenceKind = 'circular' | 'unresolvable' | null

function residualReferenceKind(schema: JsonValue): ResidualReferenceKind {
  const pending: JsonValue[] = [schema]
  let hasLocal = false
  let hasRemote = false
  while (pending.length > 0) {
...
type OutputPlan =
  | Readonly<{ kind: 'ok'; outputSchema: SchemaRecord; evidencePointer: string | undefined }>
  | Readonly<{ kind: 'refused'; reason: AdmitProviderSchemaRefusal }>
...
function firstGuaranteedOutputPointer(schema: SchemaRecord): string | undefined {
...
function deriveAdmissionMetadata(
  ...
  for (const annotation of contract.customerAnnotations) {
    if (annotation.document === 'input') annotatedInputPointers.add(annotation.pointer)
    annotationIds.add(annotation.annotationId)
  }
  const coveredByDataUse = new Set<string>()
  for (const declaration of contract.dataUse) coveredByDataUse.add(declaration.inputPointer)

  let dataReleaseEffect = effects.find((effect) => effect.class === 'data_release')

  const properties = inputSchema.properties
  const requiredInputs = new Set(
    (Array.isArray(inputSchema.required) ? inputSchema.required : []).filter((value): value is string => typeof value === 'string'),
  )
  if (isRecord(properties)) {
    for (const name of Object.keys(properties)) {
      const pointer = `/${escapePointerSegment(name)}`
      const covered = [...coveredByDataUse].some((declaration) => pointerCovers(declaration, pointer))
      if (!covered) {
        if (dataReleaseEffect === undefined) {
          dataReleaseEffect = {
            effectId: uniqueIdentifier('provider_data_use', new Set(effects.map((effect) => effect.effectId))),
            class: 'data_release',
            authority: 'explicit',
      ...
      if (requiredInputs.has(name)) {
        const annotated = [...annotatedInputPointers].some((candidate) => pointerCovers(candidate, pointer))
        if (!annotated) {
          const annotationId = uniqueIdentifier(annotationIdFor(name), annotationIds)
  ...
  if (evidencePointer !== undefined && !evidence.some((requirement) => requirement.purpose === 'completion')) {
    ...
    evidence.push({
      evidenceId: uniqueIdentifier('completion', new Set(evidence.map((requirement) => requirement.evidenceId))),
...
function annotationIdFor(name: string): string {
  return slugToken(name).toLowerCase().replace(/^_+|_+$/g, '') || 'input'
}
...
function purposeForProperty(name: string, schema: JsonValue | undefined): string {
