...
export type PublicCommercialTerms = Readonly<{
  ...
  relationship: Readonly<{ kind: 'none' | 'direct' | 'affiliate' | 'ownership'; summary: string }>
}>
export type PublicDataUsePolicy = readonly Readonly<{
  effectId: string; inputPointer: string; classification: 'public' | 'personal' | 'sensitive' | 'credential'; phase: 'preparation' | 'execution'; recipient: 'candidate_binding' | 'selected_binding' | 'named_recipient'; purposes: readonly string[]
}>[]
export type PublicEffectPolicy = readonly Readonly<{
  effectId: string; class: 'data_release' | 'financial_exposure' | 'external_state_change'; authority: 'none' | 'explicit' | 'mandate_or_explicit'; reversibility: 'not_applicable' | 'reversible' | 'conditional' | 'irreversible'
}>[]
export type PublicEvidencePolicy = readonly Readonly<{ evidenceId: string; outputPointer: string; purpose: 'comparison' | 'completion' | 'recovery' }>[]
...
export type PublicOperationNavigationRelation = Readonly<{
  ...
  method: 'GET' | 'POST'; actionId: string; authentication: 'none' | 'required'; inputSchema?: Readonly<Record<string, JsonValue>>; precondition?: string
}>
export type PublicOperationDescriptor = Readonly<{
  operationRef: PublicOperationRef; operationId: string
  contract: Readonly<{
    capabilityId: string; version: number; inputJsonSchema: Readonly<Record<string, JsonValue>>; outputJsonSchema: Readonly<Record<string, JsonValue>>
...
export type CapabilityOperationSourcePort = Readonly<{
  listCurrent: (input: Readonly<{ networkId?: string; limit: number; now: number }>) => Promise<Readonly<{ operations: readonly CapabilityOperationSourceRecord[]; snapshotKey: string }>>
...
export type OperationSearchResult =
  | Readonly<{ kind: 'ok'; schemaVersion: PublicOperationRegistrySchemaVersion; query: string; items: readonly PublicOperationDescriptor[]; pagination: Readonly<{ limit: number; nextCursor?: string; hasMore: boolean }>; navigation: readonly PublicOperationNavigationRelation[] }>
  | Readonly<{ kind: 'no_candidates'; schemaVersion: PublicOperationRegistrySchemaVersion; query: string; appliedFilters: OperationSearchFilters; navigation: readonly PublicOperationNavigationRelation[] }>
  | Readonly<{ kind: 'unavailable'; schemaVersion: PublicOperationRegistrySchemaVersion; reason: 'query_invalid' | 'source_unavailable' | 'source_capacity_exceeded'; navigation: readonly PublicOperationNavigationRelation[] }>
export type OperationDetailInput = Readonly<{ operationRef: string }>
export type OperationDetailResult =
  | Readonly<{ kind: 'found'; schemaVersion: PublicOperationRegistrySchemaVersion; operation: PublicOperationDescriptor }>
  | Readonly<{ kind: 'unavailable'; schemaVersion: PublicOperationRegistrySchemaVersion; operationRef: string; reason: PublicCapabilityUnavailableReason; navigation: readonly PublicOperationNavigationRelation[] }>
  | Readonly<{ kind: 'not_found'; schemaVersion: PublicOperationRegistrySchemaVersion; operationRef: string; navigation: readonly PublicOperationNavigationRelation[] }>
...
export type OperationComparisonFact = Readonly<{
  ...
  values: readonly Readonly<{ operationRef: PublicOperationRef; value: OperationComparisonValue; source: 'publication' | 'readiness' | 'contract' | 'catalog'; observedAt?: number; validUntil?: number }>[]
}>
export type OperationCompareInput = Readonly<{ operationRefs: readonly string[] }>
export type OperationCompareResult =
  | Readonly<{ kind: 'ok'; schemaVersion: PublicOperationRegistrySchemaVersion; operations: readonly PublicOperationDescriptor[]; facts: readonly OperationComparisonFact[]; navigation: readonly PublicOperationNavigationRelation[] }>
...
export type OperationSurfaceWireDescriptor = {
  ...
  contract: {
    ...
    customerAnnotations: DeepWritable<PublicOperationDescriptor['contract']['customerAnnotations'][number]>[]
  }
  ...
  commercial: {
    ...
    materialTerms: DeepWritable<PublicCommercialTerms['materialTerms'][number]>[]
...
type DeepWritable<Value> = Value extends string | number | boolean | bigint | null | undefined
  ...
  : Value extends readonly (infer Item)[]
    ...
    : Value extends object
      ...
      : Value
type OperationSurfaceWireNavigation = DeepWritable<Omit<PublicOperationNavigationRelation, 'inputSchema'>>
...
export type OperationDetailWireResult =
  | { kind: 'found'; schemaVersion: PublicOperationRegistrySchemaVersion; operation: OperationSurfaceWireDescriptor }
  | { kind: 'unavailable'; schemaVersion: PublicOperationRegistrySchemaVersion; operationRef: string; reason: PublicCapabilityUnavailableReason; navigation: OperationSurfaceWireNavigation[] }
  | { kind: 'not_found'; schemaVersion: PublicOperationRegistrySchemaVersion; operationRef: string; navigation: OperationSurfaceWireNavigation[] }
export type OperationCompareWireResult =
  | { kind: 'ok'; schemaVersion: PublicOperationRegistrySchemaVersion; operations: OperationSurfaceWireDescriptor[]; facts: OperationComparisonWireFact[]; navigation: OperationSurfaceWireNavigation[] }
  | { kind: 'unavailable'; schemaVersion: PublicOperationRegistrySchemaVersion; reason: 'query_invalid' | 'operation_not_found' | 'operation_unavailable'; navigation: OperationSurfaceWireNavigation[] }
export type InspectPlanWireResult =
  ...
  | { kind: 'unavailable'; schemaVersion: PublicOperationRegistrySchemaVersion; reason: 'query_invalid' | 'operation_not_found' | 'operation_unavailable' | 'mapping_unavailable' | 'mapping_incompatible' | 'mapping_cycle'; navigation: OperationSurfaceWireNavigation[] }
export type OperationSurfaceWireResult = OperationSearchWireResult | OperationDetailWireResult | OperationCompareWireResult | InspectPlanWireResult
...
const INSPECT_PLAN_NAVIGATION: PublicOperationNavigationRelation = { relation: 'inspect_plan', method: 'POST', actionId: 'registry.operations.inspectPlan', authentication: 'none' }
const AUTHENTICATE_NAVIGATION: PublicOperationNavigationRelation = { relation: 'authenticate', method: 'POST', actionId: 'customer_request.authenticate', authentication: 'none' }
const CREATE_REQUEST_NAVIGATION: PublicOperationNavigationRelation = { relation: 'create_customer_request', method: 'POST', actionId: 'customer_request.create', authentication: 'required', precondition: 'authenticated_principal' }
const NAVIGATION: readonly PublicOperationNavigationRelation[] = Object.freeze([
  SEARCH_NAVIGATION,
  DETAIL_NAVIGATION,
  COMPARE_NAVIGATION,
  INSPECT_PLAN_NAVIGATION,
  AUTHENTICATE_NAVIGATION,
  CREATE_REQUEST_NAVIGATION,
])
const NAV_NONE: readonly PublicOperationNavigationRelation[] = Object.freeze([
  SEARCH_NAVIGATION,
  DETAIL_NAVIGATION,
  AUTHENTICATE_NAVIGATION,
])

export async function searchCapabilityOperations(port: CapabilityOperationSourcePort, input: OperationSearchInput, now = Date.now()): Promise<OperationSearchResult> {
  const normalized = normalizeSearch(input)
  if (normalized === undefined) return searchUnavailable('query_invalid')
  const source = await port.listCurrent({ ...(normalized.filters.networkId === undefined ? {} : { networkId: normalized.filters.networkId }), limit: MAX_SOURCE + 1, now })
  ...
  if (lastItem === undefined) return { kind: 'no_candidates', schemaVersion: PublicOperationRegistrySchemaVersion, query: normalized.query, appliedFilters: normalized.filters, navigation: NAV_NONE }
  const hasMore = start + items.length < matches.length
  return { kind: 'ok', schemaVersion: PublicOperationRegistrySchemaVersion, query: normalized.query, items, pagination: { limit: normalized.limit, hasMore, ...(hasMore ? { nextCursor: encodeCursor(normalized.query, normalized.filters, source.snapshotKey, lastItem.operationRef) } : {}) }, navigation: NAVIGATION }
}

export async function detailCapabilityOperation(port: CapabilityOperationSourcePort, input: OperationDetailInput, now = Date.now()): Promise<OperationDetailResult> {
  if (!isPublicOperationRef(input.operationRef)) return { kind: 'not_found', schemaVersion: PublicOperationRegistrySchemaVersion, operationRef: input.operationRef, navigation: NAV_NONE }
  const record = await port.loadCurrent(input.operationRef)
  if (record === null) return { kind: 'not_found', schemaVersion: PublicOperationRegistrySchemaVersion, operationRef: input.operationRef, navigation: NAV_NONE }
  const operation = projectCapabilityOperation(record, now)
  return operation.availability.posture === 'unavailable' ? { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, operationRef: operation.operationRef, reason: operation.availability.reason ?? 'not_supported_by_ae', navigation: NAV_NONE } : { kind: 'found', schemaVersion: PublicOperationRegistrySchemaVersion, operation }
}

export async function compareCapabilityOperations(port: CapabilityOperationSourcePort, input: OperationCompareInput, now = Date.now()): Promise<OperationCompareResult> {
  const refs = normalizeRefs(input.operationRefs, MAX_COMPARISON)
  if (refs === undefined) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'query_invalid', navigation: NAV_NONE }
  ...
  return { kind: 'ok', schemaVersion: PublicOperationRegistrySchemaVersion, operations, facts: comparisonFacts(operations), navigation: NAVIGATION }
}

export async function inspectCapabilityOperationPlan(port: CapabilityOperationSourcePort, input: InspectPlanInput, now = Date.now()): Promise<InspectPlanResult> {
  const refs = normalizeRefs(input.operationRefs, MAX_PLAN)
  const mappingRefs = input.mappingRefs === undefined ? [] : input.mappingRefs.length <= MAX_MAPPING_REFS && input.mappingRefs.every(mappingRefIsValid) ? [...new Set(input.mappingRefs)] as RegisteredOperationMappingRef[] : undefined
  const expiresInMs = input.expiresInMs ?? 300_000
  if (refs === undefined || mappingRefs === undefined || !Number.isSafeInteger(expiresInMs) || expiresInMs < 1_000 || expiresInMs > 86_400_000) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'query_invalid', navigation: NAV_NONE }
  const records = await Promise.all(refs.map((ref) => port.loadCurrent(ref)))
  const presentRecords = records.filter((record): record is CapabilityOperationSourceRecord => record !== null)
  if (presentRecords.length !== records.length) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'operation_not_found', navigation: NAV_NONE }
  const operations = presentRecords.map((record) => projectCapabilityOperation(record, now))
  ...
  for (const mappingRef of mappingRefs) {
    const mapping = port.resolveMapping === undefined ? null : await port.resolveMapping(mappingRef, networkId)
    if (mapping === null) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'mapping_unavailable', navigation: NAV_NONE }
    mappings.push(mapping)
  }
  if (!mappingsCompatible(mappings, presentRecords)) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'mapping_incompatible', navigation: NAV_NONE }
  if (mappingCycle(mappings)) return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason: 'mapping_cycle', navigation: NAV_NONE }
  const expiry = Math.min(now + expiresInMs, ...operations.map((operation) => operation.availability.validUntil ?? now + expiresInMs))
  return { kind: 'ok', schemaVersion: PublicOperationRegistrySchemaVersion, inspectPlanRef: `inspect-plan:v1:${canonicalDigest({ operationRefs: refs, mappingRefs, expiresAt: expiry }).slice(7)}`, operationRefs: refs, mappingRefs, summary: { maximumCost: aggregatePrice(operations.map((operation) => operation.commercial.price)), dataUse: mergeDataUse(operations), effects: mergeEffects(operations), expiry }, navigation: [AUTHENTICATE_NAVIGATION, CREATE_REQUEST_NAVIGATION] }
}
...
export function serializeOperationDescriptor(operation: PublicOperationDescriptor): OperationSurfaceWireDescriptor {
  return {
    ...
    contract: {
      ...
      customerAnnotations: operation.contract.customerAnnotations.map((annotation) => ({
    ...
    commercial: {
      ...
      materialTerms: operation.commercial.materialTerms.map((term) => ({ label: term.label, value: term.value })),
      relationship: {
...
export function serializeOperationSearchResult(result: OperationSearchResult): OperationSearchWireResult {
  if (result.kind === 'ok') {
    return {
      kind: 'ok',
...
export function deserializeOperationSearchResult(result: OperationSearchWireResult): OperationSearchResult {
  if (result.kind === 'ok') {
    return {
      kind: 'ok',
      schemaVersion: result.schemaVersion,
...
export function serializeOperationDetailResult(result: OperationDetailResult): OperationDetailWireResult {
...
export function deserializeOperationDetailResult(result: OperationDetailWireResult): OperationDetailResult {
  if (result.kind === 'found') {
    return {
      kind: 'found',
      schemaVersion: result.schemaVersion,
...
export function serializeOperationCompareResult(result: OperationCompareResult): OperationCompareWireResult {
  if (result.kind === 'ok') {
    return {
      kind: 'ok',
      schemaVersion: result.schemaVersion,
...
export function deserializeOperationCompareResult(result: OperationCompareWireResult): OperationCompareResult {
  if (result.kind === 'ok') {
    return {
      kind: 'ok',
      schemaVersion: result.schemaVersion,
...
export function serializeInspectPlanResult(result: InspectPlanResult): InspectPlanWireResult {
  if (result.kind === 'unavailable') {
    return {
      kind: 'unavailable',
      schemaVersion: result.schemaVersion,
...
export function deserializeInspectPlanResult(result: InspectPlanWireResult): InspectPlanResult {
  if (result.kind === 'unavailable') {
    return {
      kind: 'unavailable',
      schemaVersion: result.schemaVersion,
...
function serializeDataUse(dataUse: PublicDataUsePolicy[number]): DeepWritable<PublicDataUsePolicy[number]> {
  return {
...
function serializeEffect(effect: PublicEffectPolicy[number]): DeepWritable<PublicEffectPolicy[number]> {
  return {
...
function serializeAvailability(availability: PublicOperationAvailability): DeepWritable<PublicOperationAvailability> {
  return {
    posture: availability.posture,
    ...(availability.observedAt === undefined ? {} : { observedAt: availability.observedAt }),
...
function serializeNavigation(navigation: readonly PublicOperationNavigationRelation[]): OperationSurfaceWireNavigation[] {
...
function serializeSearchFilters(filters: OperationSearchFilters): OperationSearchWireFilters {
  return {
...
function serializeComparisonValue(
  ...
  if (field === 'summary') {
    if (typeof value !== 'string') throw new Error('operation_comparison_value_invalid')
    return value
  }
  if (field === 'price') {
    if (!isPublicOperationPrice(value)) throw new Error('operation_comparison_value_invalid')
    return serializePrice(value)
  }
  if (field === 'effects') {
    if (!isPublicEffectPolicy(value)) throw new Error('operation_comparison_value_invalid')
    return value.map(serializeEffect)
  }
  if (field === 'dataUse') {
    if (!isPublicDataUsePolicy(value)) throw new Error('operation_comparison_value_invalid')
    return value.map(serializeDataUse)
  }
  if (field === 'availability') {
    if (!isPublicAvailability(value)) throw new Error('operation_comparison_value_invalid')
    return serializeAvailability(value)
  }
  if (field === 'provenance') {
    if (!isPublicProvenance(value)) throw new Error('operation_comparison_value_invalid')
    return { publisher: value.publisher, sourceKind: value.sourceKind }
  }
  if (!isPublicRecoveryPolicy(value)) throw new Error('operation_comparison_value_invalid')
...
function isPublicEffectPolicy(value: unknown): value is PublicEffectPolicy {
  return Array.isArray(value) && value.every((item: unknown) => (
    ...
    && (item.class === 'data_release' || item.class === 'financial_exposure' || item.class === 'external_state_change')
    && (item.authority === 'none' || item.authority === 'explicit' || item.authority === 'mandate_or_explicit')
    && (item.reversibility === 'not_applicable' || item.reversibility === 'reversible' || item.reversibility === 'conditional' || item.reversibility === 'irreversible')
  ))
}
...
function isPublicDataUsePolicy(value: unknown): value is PublicDataUsePolicy {
  return Array.isArray(value) && value.every((item: unknown) => {
    if (!isRecord(item)
      ...
      || (item.classification !== 'public' && item.classification !== 'personal' && item.classification !== 'sensitive' && item.classification !== 'credential')
    ...
    return item.recipient.kind === 'candidate_binding'
      || item.recipient.kind === 'selected_binding'
      || item.recipient.kind === 'named_recipient' && typeof item.recipient.recipientId === 'string'
  })
}

function isPublicAvailability(value: unknown): value is PublicOperationAvailability {
  if (!isRecord(value)
    || (value.posture !== 'integrated' && value.posture !== 'routeable' && value.posture !== 'unavailable')) return false
  if (value.observedAt !== undefined && typeof value.observedAt !== 'number') return false
  if (value.validUntil !== undefined && typeof value.validUntil !== 'number') return false
...
function decodePublicSchema(serialized: string): Readonly<Record<string, JsonValue>> {
  ...
  return projectPublicSchema(checked.data as Readonly<Record<string, JsonValue>>)
}


function searchUnavailable(reason: 'query_invalid' | 'source_unavailable' | 'source_capacity_exceeded'): OperationSearchResult { return { kind: 'unavailable', schemaVersion: PublicOperationRegistrySchemaVersion, reason, navigation: NAV_NONE } }
function projectAvailability(record: CapabilityOperationSourceRecord, now: number): PublicOperationAvailability {
  const { observedAt, validUntil } = record.readiness
  if (record.routeable && validUntil !== undefined && validUntil > now) return { posture: 'routeable', ...(observedAt === undefined ? {} : { observedAt }), validUntil }
  const reason = validUntil !== undefined && validUntil <= now ? 'readiness_expired' as const : record.unavailableReason ?? 'setup_required'
  return { posture: record.integrated ? 'integrated' : 'unavailable', ...(observedAt === undefined ? {} : { observedAt }), ...(validUntil === undefined ? {} : { validUntil }), reason }
}
function normalizeSearch(input: OperationSearchInput): Readonly<{ query: string; limit: number; cursor?: string; filters: OperationSearchFilters }> | undefined {
  if (typeof input.query !== 'string' || input.query.trim().length > MAX_QUERY) return undefined
...
function comparisonFacts(operations: readonly PublicOperationDescriptor[]): OperationComparisonFact[] {
  const fields: OperationComparisonFact['field'][] = ['summary', 'price', 'effects', 'dataUse', 'availability', 'provenance', 'recovery']
  return fields.map((field) => ({ field, values: operations.map((operation) => ({ operationRef: operation.operationRef, value: comparisonValue(operation, field), source: field === 'availability' ? 'readiness' as const : field === 'price' ? 'catalog' as const : field === 'provenance' ? 'publication' as const : 'contract' as const, ...(operation.availability.observedAt === undefined ? {} : { observedAt: operation.availability.observedAt }), ...(operation.availability.validUntil === undefined ? {} : { validUntil: operation.availability.validUntil }) })) }))
}
function comparisonValue(operation: PublicOperationDescriptor, field: OperationComparisonFact['field']): OperationComparisonValue {
...
function dedupe<T>(values: readonly T[]): T[] { const seen = new Set<string>(); return values.filter((value) => { const digest = canonicalDigest(value as JsonValue); if (seen.has(digest)) return false; seen.add(digest); return true }) }
function mappingsCompatible(
  mappings: readonly RegisteredOperationMapping[],
  records: readonly CapabilityOperationSourceRecord[],
): boolean {
  const contracts = new Map(records.map((record) => [
    `${record.contract.ref.capabilityId}:${record.contract.ref.version}`,
    record.contract,
  ]))
  return mappings.every((mapping) => {
    const source = contracts.get(`${mapping.sourceContractRef.capabilityId}:${mapping.sourceContractRef.version}`)
    const target = contracts.get(`${mapping.targetContractRef.capabilityId}:${mapping.targetContractRef.version}`)
    ...
    const sourcePointer = mapping.kind === 'array_project'
      ? mapping.sourceArrayPointer
      : mapping.sourceOutputPointer
    const targetPointer = mapping.kind === 'array_project'
      ? mapping.targetArrayPointer
      : mapping.targetInputPointer
    const sourceSchema = resolvePointedSchema(source.outputSchema, sourcePointer)
    const targetSchema = resolvePointedSchema(target.inputSchema, targetPointer)
    if (sourceSchema === undefined || targetSchema === undefined
      || canonicalDigest(sourceSchema as StableHashValue) !== mapping.sourceSchemaIdentity
      || canonicalDigest(targetSchema as StableHashValue) !== mapping.targetSchemaIdentity) {
      return false
    }
    return mapping.kind !== 'array_project'
...
function mappingCycle(mappings: readonly RegisteredOperationMapping[]): boolean { const graph = new Map<string, string[]>(); for (const mapping of mappings) { const source = mapping.sourceContractRef.capabilityId + ':' + mapping.sourceContractRef.version; const target = mapping.targetContractRef.capabilityId + ':' + mapping.targetContractRef.version; graph.set(source, [...(graph.get(source) ?? []), target]) } const visiting = new Set<string>(); const visited = new Set<string>(); const visit = (node: string): boolean => { if (visiting.has(node)) return true; if (visited.has(node)) return false; visiting.add(node); if ((graph.get(node) ?? []).some(visit)) return true; visiting.delete(node); visited.add(node); return false }; return [...graph.keys()].some(visit) }
function encodeCursor(query: string, filters: OperationSearchFilters, snapshotKey: string, lastOperationRef: PublicOperationRef): string { return `cursor:v1:${canonicalDigest({ query, filters, snapshotKey, lastOperationRef }).slice(7)}:${encodeURIComponent(snapshotKey)}:${encodeURIComponent(lastOperationRef)}` }
...
function decodeCursor(
  ...
  if (digest === undefined || encodedSnapshot === undefined || encodedRef === undefined) return undefined
  ...
  if (cursorSnapshot !== snapshotKey || !isPublicOperationRef(lastRef)) return undefined
...
function projectPublicSchema(schema: Readonly<Record<string, JsonValue>>): Readonly<Record<string, JsonValue>> { const state = { depth: 0, properties: 0, refs: 0 }; const projected = projectSchemaValue(schema, state); if (new TextEncoder().encode(JSON.stringify(projected)).byteLength > MAX_SCHEMA_BYTES) throw new Error('operation_public_schema_too_large'); return projected as Readonly<Record<string, JsonValue>> }
function projectSchemaValue(value: JsonValue, state: { depth: number; properties: number; refs: number }): JsonValue {
  if (state.depth > MAX_SCHEMA_DEPTH) throw new Error('operation_public_schema_too_deep')
  ...
  for (const [key, child] of Object.entries(object)) {
    if (!SCHEMA_KEYS.has(key)) throw new Error('operation_public_schema_keyword_unsupported')
    if (key === '$ref') {
      state.refs += 1
      if (state.refs > MAX_SCHEMA_REFS || typeof child !== 'string' || !child.startsWith('#/')) {
        throw new Error('operation_public_schema_ref_invalid')
      }
    }
    if (key === 'properties' || key === '$defs' || key === 'patternProperties') {
      if (!isRecord(child)) throw new Error('operation_public_schema_properties_invalid')
      const childObject = child as Readonly<Record<string, JsonValue>>
      state.properties += Object.keys(childObject).length
      if (state.properties > MAX_SCHEMA_PROPERTIES) throw new Error('operation_public_schema_properties_exceeded')
      result[key] = Object.fromEntries(Object.entries(childObject).map(([childKey, childValue]) => (
        [childKey, projectSchemaValue(childValue, state)]
      )))
    } else {
