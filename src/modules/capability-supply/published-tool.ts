...
export type PublishedToolUsageObservation = Readonly<{
...
export type PublishedTool = Readonly<{
  ...
  usageObservation?: PublishedToolUsageObservation
}>

export type RuntimePublishedToolDescriptor = Readonly<{
  id: string
  version: string
  name: string
  summary: string
  ...
  price: CapabilityOfferingRegistration['presentation']['price']
  target: PublishedTool['identity']
  validateInput(value: unknown): boolean
  validateOutput(value: unknown): boolean
}>

function isPublishedToolSnapshot(value: unknown): value is PublishedTool {
...
export function parsePublishedToolSnapshot(toolJson: string): PublishedTool | undefined {
  try {
...
export function publishedToolMaterialMatches(
  reserved: PublishedTool | undefined,
  current: PublishedTool | undefined,
): boolean {
...
export function publishedToolIdentityDigest(
  identity: PublishedTool['identity'],
): string {
  const connectionAuthority = identity.connectionAuthority
  if (connectionAuthority === undefined) return canonicalDigest(identity)
  if (Object.hasOwn(connectionAuthority, 'operationRef')) {
    throw new Error('published_tool_identity_invalid')
  }
  const { toolRef, ...connectionAuthorityMaterial } = connectionAuthority
  return canonicalDigest({
    ...identity,
    connectionAuthority: {
      ...connectionAuthorityMaterial,
      operationRef: toolRef,
    },
  })
}

export function materializePublishedTool(input: Readonly<{
  ...
  const operationId = capabilityToolId(contract.ref.capabilityId)
  ...
  const materialDigest = publishedToolIdentityDigest(identity)
  return {
    kind: 'published_operation',
    environment: 'SOURCE-OWNED DEVELOPMENT EVIDENCE',
    runtimeEnvironment: publication.runtimeEnvironment,
...
export function materializeRuntimePublishedTool(
...
function transportIdentity(
  endpointUrl: string,
  config: JsonValue,
): PublishedTool['identity']['endpoint'] | undefined {
...
function paymentIdentity(
  adapterId: string,
  config: JsonValue,
): PublishedTool['identity']['payment'] {
...
function validateUsage(observation: PublishedToolUsageObservation | undefined): void {
...
function connectionAuthorityIsExact(input: Readonly<{
  ...
  return connectionAuthoritySnapshotIsValid(snapshot)
    ...
    && snapshot.toolRef === input.expectedToolRef
}
