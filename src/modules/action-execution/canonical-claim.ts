...
export type CanonicalClaimAcceptedAuthority = NonNullable<ActionExecutionView['acceptedAuthority']>
...
export function buildCanonicalClaimCommand(input: CanonicalClaimInput): CanonicalClaimCommand {
  ...
  const commandId = `action-invocation-claim:v1:${input.executionRef}:${input.attempt.attemptRef}`
  const canonicalCommandMaterial = {
    format: 'action-invocation-claim:v1',
    invocationRef: input.executionRef,
    sourceRef: input.sourceRef,
    invocationVersion: input.executionVersion,
    expectedInvocationVersion: input.expectedExecutionVersion,
...
export async function claimCanonicalExecution(
  input: CanonicalClaimInput,
  port: Pick<DurableActionExecutionPort, 'transact' | 'readControl' | 'readAttempt'>,
  ...
  const snapshot = await readClaimSnapshot(port, input.executionRef, input.attempt.attemptRef)
...
export function buildCanonicalReleaseFenceCommand(
  ...
  const nextExecutionVersion = control.executionVersion + 1
  const nextAttempt = {
    ...attempt,
  ...
  const commandId = `action-invocation-release-fence:v1:${attempt.executionRef}:${attempt.attemptRef}:${attempt.effectGeneration}`
  return makeTransitionCommand({
    commandId,
    kind: 'release_fence_before_network',
    expectedExecutionVersion: control.executionVersion,
    ...
    material: {
      ...
      invocationRef: attempt.executionRef,
...
export async function persistCanonicalReleaseFence(
  input: CanonicalReleaseFenceInput,
  port: Pick<DurableActionExecutionPort, 'transact'>,
...
export function buildCanonicalTerminalOutcomeCommand(
  ...
  const commandId = `action-invocation-terminal:v1:${attempt.executionRef}:${attempt.attemptRef}:${attempt.effectGeneration}`
  return makeTransitionCommand({
    commandId,
...
function makeTransitionCommand(input: Readonly<{
  ...
  const history: Omit<DurableHistoryRow, 'executionVersion' | 'recordedAt' | 'current'> = {
    executionRef: input.row.executionRef,
    ...
    attemptTransition: {
      ...
      priorDigest: canonicalDigest(canonicalAttemptMaterial(input.priorAttempt)),
      nextDigest: canonicalDigest(canonicalAttemptMaterial(input.currentAttemptWrite)),
...
// Attempt transition digests are an existing durable evidence boundary. Keep
// their pre-rename canonical key while stored rows use executionRef.
...
async function readClaimSnapshot(
  port: Pick<DurableActionExecutionPort, 'readControl' | 'readAttempt'>,
  executionRef: string,
  attemptRef: string,
): Promise<CanonicalClaimSnapshot | undefined> {
  const control = await port.readControl(executionRef)
  if (control === undefined || control.currentAttemptRef !== attemptRef) return undefined
  const attempt = await port.readAttempt(executionRef, attemptRef)
...
function assertActiveClaim(snapshot: CanonicalClaimSnapshot, recordedAt: string): void {
  const { control, attempt } = snapshot
  if (
    control.executionRef !== attempt.executionRef
    ...
    throw new Error('action_execution_claim_not_active')
  }
}
...
function assertClaimForTerminalOutcome(
  ...
  if (
    ...
    throw new Error('action_execution_claim_not_current')
  }
  if (outcome.kind === 'returned' && (outcome.resultRef.trim().length === 0 || outcome.resultDigest.trim().length === 0)) {
    throw new Error('action_execution_terminal_result_invalid')
  }
  if (outcome.kind === 'uncertain' && outcome.reconciliationRequiredAt <= recordedAt) {
    throw new Error('action_execution_terminal_reconciliation_time_invalid')
  }
}
...
function validateClaimInput(input: CanonicalClaimInput): void {
  ...
  if (references.some((value) => value.trim().length === 0)) {
    throw new Error('action_execution_claim_identity_invalid')
  }
  if (!Number.isSafeInteger(input.executionVersion) || input.executionVersion < 1) {
    throw new Error('action_execution_claim_version_invalid')
  }
  if (
    input.expectedExecutionVersion !== null
    && (!Number.isSafeInteger(input.expectedExecutionVersion) || input.expectedExecutionVersion < 0)
  ) {
    throw new Error('action_execution_claim_expected_version_invalid')
  }
  if (
    input.expectedEffectGeneration !== undefined
    && (!Number.isSafeInteger(input.expectedEffectGeneration) || input.expectedEffectGeneration < 1)
  ) {
    throw new Error('action_execution_claim_expected_generation_invalid')
  }
  if (
    ...
    throw new Error('action_execution_claim_attempt_invalid')
  }
  ...
  if (input.authority.acceptedBasis.kind === 'public_capability_use') {
    ...
    if (
      ...
      throw new Error('action_execution_claim_public_capability_authority_invalid')
    }
  }
}
