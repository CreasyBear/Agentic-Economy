...
export const actionExecutionOriginValue = v.union(
...
export const executionActorValue = v.object({ callerRef: v.string(), principalRef: v.string() })
export const executionFreshnessValue = v.union(
  v.object({ state: v.literal('not_observed') }),
  v.object({ state: v.literal('current'), observedAt: v.string() }),
)
export const executionControlValue = v.union(
...
export const authorityBindingValue = v.object({
  reference: v.string(), executionRef: v.string(), actor: executionActorValue, origin: actionExecutionOriginValue,
  executionVersion: v.number(), actionId: v.string(), contractVersion: v.string(),
  digest: v.string(), targetDigest: v.string(), consequence: v.string(),
  limits: executionLimitsValue, expiresAt: v.string(),
  acceptedBasis: v.optional(acceptedAuthorityValue),
})
export const durableControlProjectionValue = v.object({
  executionRef: v.string(), executionVersion: v.number(),
  origin: actionExecutionOriginValue, owner: executionActorValue,
  ...
  freshness: executionFreshnessValue, control: executionControlValue,
})

export const actionExecutionTables = {
  actionExecutionControls: defineTable({
    ...
    dataLimitSummary: v.optional(executionLimitsValue),
    ...
    .index('by_executionRef', ['executionRef'])
    .index('by_control_owner_principalRef_and_executionRef', ['control.owner.principalRef', 'executionRef'])
    .index('by_sourceRef_and_executionRef', ['sourceRef', 'executionRef']),

  actionExecutionAttempts: defineTable({
    executionRef: v.string(),
    attemptRef: v.string(),
    attemptNumber: v.number(),
    effectGeneration: v.number(),
    actor: executionActorValue,
    ...
    .index('by_executionRef_and_attemptNumber', ['executionRef', 'attemptNumber'])
    .index('by_executionRef_and_attemptRef', ['executionRef', 'attemptRef'])
    .index('by_idempotency_effectIdentity_and_attemptRef', ['idempotency.effectIdentity', 'attemptRef']),

  actionExecutionHistory: defineTable({
    executionRef: v.string(),
    commandId: v.string(),
    ...
    .index('by_executionRef_and_commandId', ['executionRef', 'commandId'])
    .index('by_executionRef_and_executionVersion', ['executionRef', 'executionVersion'])
    .index('by_executionRef_and_effectGeneration', ['executionRef', 'effectGeneration']),
} as const
