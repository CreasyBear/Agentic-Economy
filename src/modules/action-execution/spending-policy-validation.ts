...
const spendingPolicyScope = z.strictObject({
  objective: nonEmpty,
...
const spendingPolicyInput = z.strictObject({
  mode: z.enum(['spending_policy', 'unrestricted_test_only']).optional(),
  spendingPolicyRef: nonEmpty,
  ...
  const mode = input.mode ?? 'spending_policy'
  ...
  if (mode === 'unrestricted_test_only') {
...
const spendingPolicy = z.strictObject({
  format: z.literal('ae.action-invocation-standing-mandate:v1'),
  mode: z.enum(['spending_policy', 'unrestricted_test_only']),
  spendingPolicyRef: nonEmpty,
  version: positiveSafeInteger,
  generation: positiveSafeInteger,
  grantorRef: nonEmpty,
  ...
  const result = spendingPolicyInput.safeParse({
    mode: input.mode,
    spendingPolicyRef: input.spendingPolicyRef,
...
const authorityUseMaterial = z.strictObject({
  ...
  spendingPolicyRef: nonEmpty,
  spendingPolicyVersion: positiveSafeInteger,
  spendingPolicyGeneration: positiveSafeInteger,
  callerRef: nonEmpty,
  principalRef: nonEmpty,
  delegateRef: nonEmpty,
  executionRef: nonEmpty,
  action: actionIdentity,
...
const spendingPolicySnapshot = z.strictObject({
  format: z.literal('ae.action-invocation-standing-mandate-store:v1'),
  mandates: z.array(spendingPolicy),
...
export function parseSpendingPolicyInput(input: unknown) {
  return spendingPolicyInput.safeParse(input)
}

export function spendingPolicyMaterialValid(input: unknown): boolean {
  return spendingPolicy.safeParse(input).success
}
...
export function parseSpendingPolicySnapshot(input: unknown) {
  return spendingPolicySnapshot.safeParse(input)
}
