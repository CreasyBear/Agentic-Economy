...
export function leaseIsExpired(
  control: Extract<ActionExecutionView['control'], { state: 'leased' }>,
  now: string,
): boolean {
...
export function expireLease<Result extends { kind: string }>(
  view: ActionExecutionView<Result>,
  observedAt: string,
): ActionExecutionView<Result> | undefined {
  const control = view.control
  ...
  return {
    ...view,
    executionVersion: view.executionVersion + 1,
...
export function acquireLease<Result extends { kind: string }>(input: Readonly<{
  ...
  return {
    ...input.view,
    executionVersion: input.view.executionVersion + 1,
