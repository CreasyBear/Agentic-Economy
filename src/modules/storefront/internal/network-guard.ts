...
export type StorefrontResolvedAddress = {
  address: string
  family?: number
}

export type StorefrontDnsResolver = {
  lookup(hostname: string): Promise<readonly StorefrontResolvedAddress[]>
}

export const defaultStorefrontDnsResolver: StorefrontDnsResolver = {
  async lookup(hostname: string) {
    return nodeDnsLookup(hostname, { all: true, verbatim: true })
  },
}

export async function isPublicHttpTarget(url: URL, resolver: StorefrontDnsResolver): Promise<boolean> {
  ...
  let addresses: readonly StorefrontResolvedAddress[]
  try {
...
export function createStorefrontGuardedLookup(resolver: StorefrontDnsResolver): LookupFunction {
