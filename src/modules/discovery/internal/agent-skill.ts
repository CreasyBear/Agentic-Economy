export function buildPublicAgentSkillMarkdown(options: { canonicalBaseUrl: string; routingBaseUrl?: string }): string {
  ...
  return [
    ...
    '4. Follow the returned `nextAction`: provide only requested facts, prepare options, wait, retry, or revise.',
    '5. Resume at any time with `GET /api/v1/requests/:requestRef` using the same API key.',
    '6. Inspect `options_ready` as an unranked comparison. Do not infer selection or commitment.',
    '',
