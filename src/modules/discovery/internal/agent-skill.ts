...
export function buildPublicAgentSkillMarkdown(options: {
  ...
  return [
    ...
    '```',
    '',
    '## 2. Connect only when operation.inspect is challenged',
    '',
    'Use the native entry for the client already in use:',
    '',
    'Codex:',
    '```sh',
    ...
    '',
    `Public search works immediately. Complete standard OAuth approval using the client step above; it returns to the same task. Then call \`${AE_MCP_WHOAMI_TOOL_NAME}\` and report the connected Agent Principal and Account. Do not request or paste an AE API key.`,
    '',
    'Machine discovery remains public:',
    '```sh',
    `curl -fsSL ${base}/.well-known/ucp`,
    '```',
    '',
    `Reference: \`${base}/llms.txt\` and \`${base}/for-agents\`.`,
    '',
    '## Supplier path',
    '',
    ...
    '',
    '## 5. Describe one exact Operation — anonymous',
    '',
    '```sh',
    `curl -sS '${base}${OPERATION_MARKET_DESCRIBE_PATH}' -H 'content-type: application/json' --data '{"operationRef":"operation:v1:…"}'`,
    '```',
    '',
    'CLI:',
    '```sh',
    ...
    '```',
    '',
    `HTTP describe: \`POST ${base}${OPERATION_MARKET_DESCRIBE_PATH}\`. Catalog price and health are indicative; only \`operation.inspect\` confirms caller-specific terms and returns a Commitment.`,
    `Optional anonymous reads: \`${cli} list\` and \`${cli} compare\` mirror \`POST ${base}${OPERATION_MARKET_LIST_PATH}\` and \`POST ${base}${OPERATION_MARKET_COMPARE_PATH}\`.`,
    ...
    OperationMarketAnonymousBoundaryLine,
    '',
    '## 6. Inspect, then invoke',
    '',
    '```sh',
    `${cli} call "$AE_OPERATION_REF" --input "$AE_INPUT_JSON" --json`,
    '```',
    '',
    'Eligible free keyless reads run anonymously through official MCP and return literal output plus an `evidenceHash`.',
    '',
    `For MCP clients, complete the client-specific authentication step in section 1, then repeat the protected call. A connection is established only after token exchange and an authenticated account read through \`${AE_MCP_WHOAMI_TOOL_NAME}\`.`,
    '',
    'Direct CLI callers use the equivalent owner-approved connection:',
    '',
    '```sh',
    `${cli} connect --json`,
    '```',
    ...
    '## 7. Authenticated gateway details',
    '',
    ...
    '',
    '## 8. Wait for the recorded result',
    '',
    '```sh',
