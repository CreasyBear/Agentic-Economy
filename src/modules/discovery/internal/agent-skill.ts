...
export function buildPublicAgentSkillMarkdown(options: {
  ...
  return [
    ...
    '```',
    '',
    `Read \`${base}/llms.txt\` (the public Operation index), then open \`${base}/for-agents\` to connect once. Install the compiled CLI only when local execution is needed:`,
    '```sh',
    ...
    '',
    `\`${registeredActionId('registry.search')}\` and \`${registeredActionId('registry.detail')}\` read published businesses and offering portfolios; they neither authorize execution nor create an Agent Service. One admitted Market Operation maps to one Agent Service via \`registry.operations.*\`.`,
    '',
    '## Stop rules',
    '',
