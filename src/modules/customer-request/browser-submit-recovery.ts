...
export async function fetchBrowserRequestWithInterpreterRecovery(
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (!await isRetryableInterpreterFailure(response) || attempt === 3) return response
    await (options.sleep ?? defaultSleep)(1_000)
  }
  throw new Error('browser_submit_interpreter_recovery_exhausted')
}

async function isRetryableInterpreterFailure(response: Response): Promise<boolean> {
  if (response.status !== 503) return false
  try {
    const value: unknown = await response.clone().json()
    return value !== null && typeof value === 'object' && 'reason' in value
      && value.reason === 'interpreter_unavailable'
  } catch {
    return false
  }
}
