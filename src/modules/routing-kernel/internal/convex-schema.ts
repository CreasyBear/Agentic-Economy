...
const offerOutput = v.union(
  v.object({ field: v.string(), valueType: v.literal('string'), value: v.string() }),
  v.object({ field: v.string(), valueType: v.literal('integer'), value: v.number() }),
  v.object({ field: v.string(), valueType: v.literal('boolean'), value: v.boolean() }),
  v.object({ field: v.string(), valueType: v.literal('url'), value: v.string() }),
  v.object({ field: v.string(), valueType: v.literal('money'), value: exactAmount }),
)
...
const routingSnapshotV1 = v.object({
  ...
  constraints: v.object({ maximumSpend: exactAmount, optimizeFor: literalUnion(['cost', 'latency'] as const) }),
...
const organicDecisionV1 = v.object({
  ...
  factors: v.array(v.object({
    bindingId: v.string(), feasible: v.boolean(), expectedCost: v.optional(exactAmount), maximumCost: v.optional(exactAmount),
...
export const routingKernelTables = {
  ...
  routingKernelIncidentRecoveryGrants: defineTable({
    ...
    canaryPlan: v.optional(v.object({
      ...
      bindingId: v.string(), capabilityContractId: v.string(), maximumSpend: exactAmount,
  ...
  routingKernelBudgetAuthorities: defineTable({
    ...
    maximumGross: exactAmount,
    reservedGross: exactAmount,
    committedGross: exactAmount,
    expiresAt: v.number(),
    status: literalUnion(['active', 'revoked'] as const),
    revision: v.number(),
    createdAt: v.number(),
  ...
  routingKernelAuthorizations: defineTable({
    ...
    budgetMaximumGross: v.optional(exactAmount),
  ...
  routingKernelStepReleases: defineTable(v.union(
    ...
    v.object({
      ...
      nodeId: v.string(), capabilityContractId: v.string(), maximumCost: exactAmount,
  ...
  routingKernelRootRuns: defineTable({
    ...
    effectState,
    authorizedAmount: exactAmount,
    quotedMaximumAmount: exactAmount,
    reservedAmount: v.optional(exactAmount),
    providerReportedAmount: v.optional(exactAmount),
    settledAmount: v.optional(exactAmount),
    legacyCommittedEstimateDiscardedAt: v.optional(v.number()),
    committedAmount: v.optional(exactAmount),
    heldAmount: v.optional(exactAmount),
    updatedAt: v.number(),
    completedAt: v.optional(v.number()),
  })
    .index('by_rootRunId', ['rootRunId'])
  ...
  routingKernelProtocolRecords: defineTable({
    ...
    reportedCost: v.optional(exactAmount),
    financialObservation: v.optional(v.literal('provider_reported')),
    budgetAuthorityRef: v.optional(v.string()),
    budgetMaximumGross: v.optional(exactAmount),
    spendReservation: v.optional(exactAmount),
    dataAuthorizationBudgetRef: v.optional(v.string()),
    ...
    maximumCost: v.optional(exactAmount),
  ...
  routingKernelProviderOffers: defineTable({
    ...
    expectedCost: exactAmount, maximumCost: exactAmount,
    expectedLatencyMs: v.number(), termsDigest: v.string(), cancellationTermsDigest: v.string(),
    offerOutputs: v.array(offerOutput),
