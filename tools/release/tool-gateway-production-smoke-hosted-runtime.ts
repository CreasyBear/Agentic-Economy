...
import { GatewaySmokeError } from "./tool-gateway-production-smoke-receipt";
import { requestJson } from "./tool-gateway-production-smoke-call";
import {
...
} from "./tool-gateway-production-smoke-hosted-owner";
import {
  createHostedMoneyRuntime,
  type HostedMoneyRuntime,
} from "./tool-gateway-production-smoke-hosted-money";
...
export type { HostedMoneyRuntime } from "./tool-gateway-production-smoke-hosted-money";
...
type RunOwnedClerkKeyProof = Readonly<{
  ...
  runId: string;
  lifecycle: "active";
  scopes: readonly [typeof MARKET_TOOLS_CALL_SCOPE];
}>;
  ...
  const credentialProof = async (): Promise<RunOwnedClerkKeyProof> => {
    credentialProofPromise ??= (async () => {
      ...
      if (
        ...
        key.expired ||
        key.scopes.length !== 1 ||
        key.scopes[0] !== MARKET_TOOLS_CALL_SCOPE
      )
        throw new GatewaySmokeError("gateway_smoke_api_key_identity_invalid");
      return {
        credentialId: key.id,
  ...
  > => {
    revokePromise ??= (async () => {
      ...
      if (toolRef !== undefined) {
        const idempotencyKey = `ae-release-smoke:revoked:${canonicalDigest({ credentialId: proof.credentialId, toolRef })}`;
        const response = await requestJson(
          options.fetch,
          `${options.baseUrl}${CALL_HTTP_PATH}`,
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({ toolRef, input, idempotencyKey }),
          },
          options.apiKey,
        );
