...
async function createRequest(args: readonly string[], options: CliOptions): Promise<void> {
  ...
  if (text.length === 0) throw new CliFailure('Usage: ae request create "<text>"')
  ...
  report('POST /api/v1/requests', outcome.status, outcome.durationMs, outcome.body ?? outcome.bodyText, options)
...
async function readRequest(args: readonly string[], options: CliOptions, suffix: string): Promise<void> {
  ...
  if (ref === undefined || ref.length === 0) throw new CliFailure(`Usage: ae request ${suffix === '' ? 'get' : 'options'} <ref>`)

  const path = `/api/v1/requests/${encodeURIComponent(ref)}${suffix}`
  const outcome = await callJson(options.baseUrl, path)
  report(`GET ${path}`, outcome.status, outcome.durationMs, outcome.body ?? outcome.bodyText, options)
}

async function confirmRequest(args: readonly string[], options: CliOptions): Promise<void> {
  const [ref, optionRef] = args
  if (ref === undefined || optionRef === undefined) throw new CliFailure('Usage: ae request confirm <ref> <optionRef>')
  ...
  report(`POST ${path}`, outcome.status, outcome.durationMs, outcome.body ?? outcome.bodyText, options)
}
