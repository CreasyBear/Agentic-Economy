...
import { OPERATION_MARKET_LIST_PATH } from '@/modules/registry/operation-entry'
...
export async function runListCommand(args: readonly string[], options: CliOptions): Promise<void> {
  ...
  if (!parsed.success) throw new CliFailure('The market returned an invalid Operation list.', { kind: 'UNAVAILABLE', code: 'operation-list-result-invalid' })
  if (parsed.data.kind === 'unavailable') throwOperationReadFailure({ reason: parsed.data.reason })
  ...
  heading(`Current Operations (${outcome.durationMs}ms)`)
  line(`  ${parsed.data.count} shown`)
  for (const [index, operation] of parsed.data.items.entries()) {
    line(`  ${index + 1}. ${operation.provider.name} — ${operation.title}`)
    line(`     ${operation.healthStatus} · ${operation.priceLabel}`)
    line(`     ${operation.operationRef}`)
  }
...
export const listCommandDescriptor = {
  command: 'list', actionId: 'registry.operations.list', path: OPERATION_MARKET_LIST_PATH,
  inputSchema: operationListInputSchema, outputSchema: operationChoiceListOutputSchema, run: runListCommand,
} as const
