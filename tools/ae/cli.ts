...
const JSON_HELP_FLAGS = {
  ...
  '--input': { type: 'string', description: 'Schema-valid JSON object for call, quote, or provider lifecycle write; call and quote alone accept - to read it from standard input.' },
...
const COMMAND_OPTIONS: Readonly<Record<string, readonly string[]>> = {
  ...
  'supply disconnect': ['input', 'idempotency-key'],
  fund: [],
  quote: ['input'],
...
async function main(argv: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  const commands: Record<string, CommandRunner> = {
    ...
    [callCommands.quoteCommandDescriptor.command]: callCommands.quoteCommandDescriptor.run,
