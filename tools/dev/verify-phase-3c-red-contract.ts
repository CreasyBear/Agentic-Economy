...
export function runPhase3CRedVerifier(args = process.argv.slice(2)): number {
  ...
  const spawnError = command.error as NodeJS.ErrnoException | undefined
  const classification = spawnError?.code === 'ETIMEDOUT'
    ? rejected('timeout_failure', 'Vitest exceeded 60 seconds.')
    : spawnError !== undefined
      ? rejected('infrastructure_failure', spawnError.message)
