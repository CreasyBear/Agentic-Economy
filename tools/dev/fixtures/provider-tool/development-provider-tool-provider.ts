...
import type { DevelopmentProviderToolSigningCustody } from './development-provider-tool-signing-custody'
import type {
...
export type DevelopmentAvailabilityObservation = DevelopmentProviderToolInput['slot']
...
export type DevelopmentProviderToolProviderSnapshot = Readonly<{
  options: Readonly<{
    providerRef?: string
    slotRef?: string
  ...
  effectRecords: readonly Readonly<{ operationKey: string; digest: string; input: DevelopmentProviderToolInput; result: DevelopmentProviderToolResult }>[]
...
export function createDevelopmentProviderToolProvider(options: Readonly<{
  providerRef?: string
  slotRef?: string
  refusal?: 'terms_changed' | 'provider_refused'
  exposureAmount?: ExactAmount
  signingCustody?: DevelopmentProviderToolSigningCustody
  snapshot?: DevelopmentProviderToolProviderSnapshot
}> = {}) {
  ...
  return {
    ...
    check: async (input: DevelopmentProviderToolInput, now: number) => {
    ...
    execute: async (input: DevelopmentProviderToolInput): Promise<DevelopmentProviderToolResult> => {
      ...
      if (options.refusal !== undefined) {
        const result: DevelopmentProviderToolResult = {
          kind: 'effect_refused',
          environment: 'MOCK/DEVELOPMENT ONLY',
    ...
    checkCancellation: async (input: DevelopmentProviderToolCancellationInput) => {
    ...
    cancel: async (
      input: DevelopmentProviderToolCancellationInput,
    ): Promise<DevelopmentProviderToolCancellationResult> => {
      const digest = canonicalDigest(input)
      ...
      const result: DevelopmentProviderToolCancellationResult = {
    ...
    exportSnapshot: (): DevelopmentProviderToolProviderSnapshot => ({
      options: {
