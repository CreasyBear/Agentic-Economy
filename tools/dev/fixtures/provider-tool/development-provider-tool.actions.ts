...
import { developmentProviderToolDependencies } from './development-provider-tool-context'
import {
  developmentLabel,
  developmentProviderToolCancellationInputSchema,
  developmentProviderToolInputSchema,
...
export type { DevelopmentProviderToolCancellationInput, DevelopmentProviderToolInput }

export const developmentProviderToolOutputSchema = z.discriminatedUnion('kind', [
  z.object({
...
export type DevelopmentProviderToolResult = z.infer<typeof developmentProviderToolOutputSchema>

export const executeDevelopmentProviderToolAction = defineAction({
  ...
  schema: developmentProviderToolInputSchema,
  parameters: [
  ...
  surfaces: [],
  outputSchema: developmentProviderToolOutputSchema,
  invocationContract: {
    version: 'v1',
  ...
  preReleaseCheck: async ({ data, context }) => {
    const input = developmentProviderToolInputSchema.parse(data)
    const dependencies = developmentProviderToolDependencies(context)
  ...
  run: async ({ data, context }) => {
    const input = developmentProviderToolInputSchema.parse(data)
    const adapter = developmentProviderToolDependencies(context).execute
    if (adapter === undefined) throw new Error('development_provider_operation_adapter_unavailable')
    return developmentProviderToolOutputSchema.parse(await adapter(input))
  },
})

export const developmentProviderToolCancellationOutputSchema = z.discriminatedUnion('kind', [
  z.object({
...
export type DevelopmentProviderToolCancellationResult = z.infer<typeof developmentProviderToolCancellationOutputSchema>

export const cancelDevelopmentProviderToolAction = defineAction({
  ...
  schema: developmentProviderToolCancellationInputSchema,
  parameters: [
  ...
  surfaces: [],
  outputSchema: developmentProviderToolCancellationOutputSchema,
  invocationContract: {
    version: 'v1',
    consequenceClass: 'external_effect',
  ...
  classifyInvocationResult: (result) => result.kind === 'effect_cancellation_confirmed'
    ...
    : { outcome: 'refused', referenceable: false },
  preReleaseCheck: async ({ data, context }) => {
    const input = developmentProviderToolCancellationInputSchema.parse(data)
    const dependencies = developmentProviderToolDependencies(context)
    if (
  ...
  run: async ({ data, context }) => {
    const input = developmentProviderToolCancellationInputSchema.parse(data)
    const adapter = developmentProviderToolDependencies(context).cancel
    if (adapter === undefined) throw new Error('development_provider_operation_cancellation_adapter_unavailable')
    return developmentProviderToolCancellationOutputSchema.parse(await adapter(input))
  },
})
