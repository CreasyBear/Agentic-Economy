...
} from './development-published-tool-evidence'
...
export function buildDevelopmentAlternatePublishedToolEvidence() {
  ...
  const tool = materializePublishedTool(sourceMaterial)
...
export function verifyDevelopmentAlternatePublishedToolEvidence(
  packet: ReturnType<typeof buildDevelopmentAlternatePublishedToolEvidence>,
): void {
  let rebuilt
  try {
    rebuilt = materializePublishedTool(packet.sourceMaterial)
  } catch {
    throw new Error('development_alternate_published_tool_evidence_invalid')
  }
  ...
  if (
    ...
    || packet.tool.identity.businessId !== 'mock:business:alternate-quote-api'
    || packet.tool.identity.contractId !== 'btc-usd.spot'
    ...
    || packet.tool.pricingConfig.sourceRequirement.asset !== expectedPayment.asset
    || packet.tool.pricingConfig.sourceRequirement.atomicUnits !== '10000'
    || packet.tool.pricingConfig.pricingPolicyRef !== 'pricing-policy:sandbox-managed-x402:v1'
    || packet.tool.pricingConfig.publicDisplay !== 'on_request'
