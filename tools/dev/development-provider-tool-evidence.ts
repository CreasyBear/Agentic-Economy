...
import { runDevelopmentProviderToolEvidence } from './fixtures/provider-tool/development-provider-tool-evidence'
import { readAndVerifyProviderToolPacket, writeEvidencePacket } from './action-execution-evidence-packet'
import {
...
const commandIdentity = `development-provider-tool-evidence run ${path} ${expectedRevision}`
...
if (command === 'run') {
  ...
  const verified = await readAndVerifyProviderToolPacket(path, expectedRevision)
