...
import {
  resumeDevelopmentProviderToolObjective,
  runUnrestrictedTestOnlyDevelopmentProviderToolPhase,
...
const custody = createDevelopmentProviderToolSigningCustody(
  JSON.parse(await readFile(custodyPath, 'utf8')) as CustodyFile,
)

if (command === 'operation') {
  const phase = await runUnrestrictedTestOnlyDevelopmentProviderToolPhase(custody)
  ...
  const resumed = await resumeDevelopmentProviderToolObjective({
    ...
    spendingPolicy: phase.spendingPolicy,
    spendingPolicySnapshot: phase.midRun.spendingPolicySnapshot,
  ...
  const cancellationRecord = {
    executionRef: resumed.cancellationRun.view.executionRef,
  ...
  await writeFile(outputPath, JSON.stringify({
    ...
    spendingPolicySnapshot: resumed.store.exportSnapshot(),
    providerSnapshot: resumed.providerSnapshot,
    objectiveState: resumed.objectiveState,
    effectCounts: resumed.effectCounts,
    reconstructedInvocationRefs: resumed.reconstructed.map(({ executionRef }) => executionRef),
  ...
  const replayed = await resumeDevelopmentProviderToolObjective({
    processRef: `pid:${process.pid}`,
    spendingPolicy: phase.spendingPolicy,
    spendingPolicySnapshot: phase.spendingPolicySnapshot,
  ...
  await writeFile(outputPath, JSON.stringify({
    ...
    reconstructedInvocationRefs: replayed.reconstructed.map(({ executionRef }) => executionRef),
  }), { encoding: 'utf8', flag: 'wx' })
} else {
