...
export async function readAndVerifyEvidencePacket(path: string, expectedRevision: string) {
  ...
  if (envelope.schema !== 'ae.action-execution-development-evidence:v2') throw new Error('packet_schema_refused')
...
function validateProviderOperationAdvertisedChecks(
  ...
  const advertisedDispositions = {
    ...
    attributableReconciliation:
      ...
      && reconciliation.evidence.invocationRef === reconciliation.before.executionRef
  ...
  const transfer = evaluateAdr009Transfer({
    events: {
      ...
      controlled: [
        ...
        {
          kind: 'attempt',
          executionRef: terminal.executionRef,
    ...
    controlledReadback: {
      executionVersion: terminal.executionVersion,
      controlRecords: 1,
...
function validateProviderOperationLinkage(durable: PacketDurable, terminal: boolean) {
  ...
  const executionRef = String(control.executionRef)
  ...
  for (const attempt of durable.attempts) {
    ...
    if (
      attempt.executionRef !== executionRef
  ...
  for (const row of durable.history) {
    if (
      ...
      || typeof row.executionVersion !== 'number'
      || row.executionVersion <= priorVersion
      || row.executionVersion > Number(control.executionVersion)
      || (
    ...
    priorVersion = row.executionVersion
  }
  if (terminal) {
    if (
...
async function reconstructProviderOperationRows(durable: PacketDurable, terminal: boolean) {
  ...
  const resumed = await tracer.coldResume(executionRef)
  const view = resumed.inspect(executionRef)
...
async function reconstructPacketMeaning(packet: Record<string, unknown>) {
  ...
  if (durable.attempts.some((row) => row.executionRef !== executionRef)) {
    throw new Error('packet_attempt_linkage_refused')
  }
  if (durable.history.some((row) => row.executionRef !== executionRef)) {
  ...
  if (
    reference.executionRef !== identity.executionRef
  ...
  return {
    ...
    attributableAttempts: state.attempts.get(executionRef)?.size ?? 0,
    durableHistoryRecords: state.history.get(executionRef)?.length ?? 0,
...
function verifyTransfer(
  ...
  if (
    ...
    || directRead.lifecycleEmissions?.actionExecutionEmissions !== 0
  ...
  const controlled: TransferBoundaryEvent[] = [
    ...
    { kind: 'authority_decision', executionRef: view.executionRef },
    { kind: 'user_or_supervisor_decision', executionRef: view.executionRef },
    ...
    { kind: 'action_execution', executionRef: view.executionRef },
    ...durable.controls.map((row) => ({
      kind: 'control' as const,
      executionRef: String(row.executionRef),
    })),
