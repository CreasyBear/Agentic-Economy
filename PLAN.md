...
- 2026-08-25 T8 hosted PR source proof: the clean architecture branch was pushed without force and opened as GitHub PR `#215`; its PR event can run only the read-only source proof and advisory React review, while every staging/production job remains manual-dispatch-only on `main`. First exact hosted run `32784206259` passed the manifest, 423/423 invocation conformance, 85/85 chat conformance, anonymous seven-file Convex generation, architecture, lint, typecheck, 2,554/2,554 unit, 626/626 integration/Convex, type/import/standards/SEO/UI gates and benchmarks, then exposed a second Linux prerequisite defect: eighteen of twenty browser cases could not launch because the workflow had never installed the locked Playwright Chromium binary. `e808fe77a` adds only `npm exec -- playwright install --with-deps chromium` to the credential-free `source-proof` job after `npm ci`, plus a focused ordering/scope regression assertion; master reproduced 5/5 focused tests, typecheck, zero-warning lint, and diff check, then removed the clean child worktree and merged branch. Corrected hosted run `32785396205` passed in 11m45s: Chromium/system dependencies, manifest, 423/423 invocation conformance, 85/85 chat conformance, anonymous Convex generation, 54/54 architecture, 2,555/2,555 unit, 626/626 integration/Convex, 4/4 type, 41/41 import, standards/SEO/UI, twenty browser journeys, ten accessibility journeys, seven paid-operation surfaces, Node 20/22 CLI package proof, and production build. Hosted 20/256 projections used 22 queries at 6.431277/24.862489 ms p95, below frozen limits and with fewer queries than Wave 0. The tested PR merge revision `83ec692831db8cb95b2250989d8304031626f894` and head `947be1cebc61d5d318cc4b61973ba9ce7326bd87` had identical tree `9debfbb6829400ac1d7c023d1722a93303af76cb`; the sanitized run artifact was independently downloaded, parsed, hashed, and archived outside the worktree. React Doctor reported zero errors and advisory-only warnings; Vercel created the configured non-production PR preview automatically. All three opt-in staging/production jobs were skipped. No staging or production data/flag operation, provider effect, payment, package publication, credential/environment change, or production deployment ran; T4/T8 remain open for the authorized staging start/soak/completion or rollback, exact tarball publication/install smoke, and exact-revision paid gateway receipt.
