...
describe('selected directory resource resolution', () => {
  ...
  it('resolves the exact completed-index snapshot independently of upstream search rank or page', async () => {
    ...
    await backend.mutation(internal.x402DirectoryIndexStore.applyPage, { generation, offset: 0, reportedTotal: 1, observedAt: 2, runStartedAt: 1, workload, items: [prepareIndexedDirectorySource(item)] })
