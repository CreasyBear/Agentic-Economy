...
describe('supplier business bootstrap', () => {
  it('creates one unpublished programmable-provider workspace and replays safely', async () => {
    ...
    const first = await owner.mutation(api.catalog.ensureSupplierBusiness, {
      name: 'Example Data',
      slug: 'Example Data',
    ...
    await expect(owner.query(api.catalog.authorizeSupplierBusiness, {
      businessId: first.businessId,
    })).resolves.toBe(true)
    await expect(backend.query(api.catalog.authorizeSupplierBusiness, {
      businessId: first.businessId,
    })).resolves.toBe(false)

    const replay = await owner.mutation(api.catalog.ensureSupplierBusiness, {
  ...
  it('refuses anonymous, invalid, and conflicting slug claims without partial rows', async () => {
    const backend = convexTest(schema, modules)
    await expect(backend.mutation(api.catalog.ensureSupplierBusiness, {
      name: 'Anonymous',
      slug: 'anonymous',
    ...
    await expect(firstOwner.mutation(api.catalog.ensureSupplierBusiness, {
      name: 'Shared Name',
      slug: 'shared-name',
      website: 'https://first.example',
      providerIdentifier: 'first.example',
    })).resolves.toMatchObject({ kind: 'created', slug: 'shared-name' })
    await expect(secondOwner.mutation(api.catalog.ensureSupplierBusiness, {
      name: 'Shared Name',
      slug: 'shared-name',
    ...
    await expect(invalidOwner.mutation(api.catalog.ensureSupplierBusiness, {
      name: 'Invalid',
      slug: 'invalid',
  ...
  it('lets only the owning account correct the display name while preserving supplier identity', async () => {
    ...
    const created = await owner.mutation(api.catalog.ensureSupplierBusiness, {
    ...
    await expect(owner.mutation(api.catalog.renameSupplierBusiness, await withSourceWrite('catalog_publish', renameCommand))).resolves.toMatchObject({
    ...
    await expect(owner.mutation(api.catalog.renameSupplierBusiness, {
      ...renameCommand,
    ...
    await expect(sibling.mutation(api.catalog.renameSupplierBusiness, await withSourceWrite('catalog_publish', {
    ...
    await expect(backend.mutation(api.catalog.renameSupplierBusiness, await withSourceWrite('catalog_publish', {
  ...
  it('rolls back the canonical name when an existing public search projection cannot refresh', async () => {
    ...
    }))).rejects.toThrow('supplier_rename_projection_failed')
  ...
  it('propagates a published rename to public detail and registry search without republishing', async () => {
    ...
    }))).resolves.toMatchObject({ kind: 'updated', name: 'Current Supplier Name' })

    await expect(backend.query(api.registry.getPublicBusinessOfferingSupplyBySlug, { slug: created.slug })).resolves.toMatchObject({
      kind: 'found', business: { businessId: created.businessId, slug: created.slug, name: 'Current Supplier Name' },
    })
    await expect(backend.query(api.registry.searchPublicBusinessOfferingSupply, {
      query: 'Current Supplier Name', limit: 10,
    })).resolves.toMatchObject({
      kind: 'ok', items: [{ businessId: created.businessId, slug: created.slug, name: 'Current Supplier Name' }],
    })
  })
})
