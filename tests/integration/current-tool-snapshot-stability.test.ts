...
import { readCurrentPublishedTool } from '../../convex/capabilitySupplyCurrentTool'
import {
...
it('keeps strict currentDigest stable before expiry and refuses expired or changed authority', async () => {
  ...
  const [firstDigest, secondDigest] = await backend.run(async (ctx) => {
    const [first, second] = await Promise.all([
      readCurrentPublishedTool(ctx, published.toolRef, readiness.observedAt + 1),
      readCurrentPublishedTool(ctx, published.toolRef, readiness.observedAt + 1_000),
    ])
    return [first, second].map((tool) => tool === undefined
      ? null
  ...
  await expect(backend.run(async (ctx) => (
    (await readCurrentPublishedTool(ctx, published.toolRef, readiness.validUntil + 1)) === undefined
  ))).resolves.toBe(true)
  ...
  await expect(backend.run(async (ctx) => (
    (await readCurrentPublishedTool(ctx, published.toolRef, readiness.observedAt + 2_000)) === undefined
  ))).resolves.toBe(true)
})
