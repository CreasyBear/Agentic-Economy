...
test.describe('public owner routes', () => {
  ...
  test('registry search lists Sam and renders truthful no-results and pagination states', async ({ page }) => {
    ...
    await expect(page.getByText(/Published details only/)).toBeVisible()
    await expect(page.getByText('Service area', { exact: true })).toBeVisible()
    await expect(page.getByText('Response', { exact: true })).toBeVisible()
    await expect(page.getByText(/Best next step:/i)).toBeVisible()
    await expect(page.getByText('Response not supplied yet')).toBeVisible()
    await expect(page.getByRole('link', { name: /list\/claim your business/i })).toBeVisible()
    ...
    await expect(page.getByText('1 matching business for "emergency plumber parramatta".')).toBeVisible()
    ...
    await page.getByRole('button', { name: /^search businesses$/i }).click()
    await expect(page.getByText('No matching business yet')).toBeVisible()
    await expect(page.getByRole('link', { name: /list\/claim your business/i })).toBeVisible()
  ...
  test('claim submission readbacks use the submitted catalog instead of the default Sam record', async ({ page }, testInfo) => {
    ...
    await page.getByLabel('Fact note').fill('Owner supplied electrical service facts')
  ...
  test('public business page exposes citation facts without private authority fields', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'What comes from the reply' })).toBeVisible()
  ...
  test('privacy removal request validates and records a receipt', async ({ page }) => {
    ...
    await page.getByLabel('Page slug').fill('parramatta-emergency-plumbing')
