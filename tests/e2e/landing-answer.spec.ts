...
test.describe('landing query -> thread answer', () => {
  ...
  test('shows an options nudge when no matches are found', async ({ page }) => {
    ...
    const jsonAction = page.getByRole('button', { name: 'Agent-readable data' })
    await jsonAction.click()
    const preview = page.getByRole('dialog', { name: 'Agent-readable data' })
    await expect(preview).toBeVisible()
    await expect(preview.getByLabel('Agent-readable data')).toContainText(`"query": "${query}"`)
