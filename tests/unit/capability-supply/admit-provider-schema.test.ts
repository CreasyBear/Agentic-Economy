...
function minimalContract(capabilityId: string): CapabilityContractMetadata {
  return {
    ...
    description: 'A provider operation admitted through the deterministic normalizer.',
    customerAnnotations: [],
    dataUse: [],
    effects: [],
    evidence: [],
  ...
  return {
    offering: {
      ...
      presentation: {
        ...
        price: { kind: 'fixed' as const, amount: { currency: 'USD', units: '0', exponent: 2 } },
        materialTerms: [],
...
describe('admitProviderSchema deterministic normalizer', () => {
  it('inlines local references and derives dataUse, required-input annotations and completion evidence', async () => {
    const document = {
      components: {
        schemas: {
          Input: { type: 'object', properties: { q: { type: 'string', minLength: 1 } }, required: ['q'] },
    ...
    expect(result.contract.dataUse.map((entry) => entry.inputPointer)).toEqual(['/q'])
    expect(result.contract.customerAnnotations).toContainEqual(expect.objectContaining({
      document: 'input', pointer: '/q', role: 'request', inference: 'customer_required',
    }))
    expect(result.contract.evidence).toEqual([expect.objectContaining({
      outputPointer: '/result',
      purpose: 'completion',
    })])
    // completion evidence carries a matching output annotation
    expect(result.contract.customerAnnotations).toContainEqual(expect.objectContaining({
      document: 'output', pointer: '/result', role: 'completion_evidence',
    }))
  })
  ...
  it('strips a query api-key out of the input surface and preserves binding authority via the openapi import', async () => {
    const document = {
      ...
      security: [{ apiKey: [] }],
      components: {
        securitySchemes: { apiKey: { type: 'apiKey', in: 'query', name: 'appid' } },
      },
      paths: {
        '/search': {
          get: {
            ...
            responses: {
              '200': {
                content: {
                  'application/json': {
                    schema: { $schema: JSON_SCHEMA, type: 'object', properties: { result: { type: 'string' } }, required: ['result'] },
                  },
                },
              },
            },
    ...
    const result = await importOpenApiHttpCapability({
      ...
      commercial: commercial({ kind: 'provider_connection', connectionRef: 'connection:appid', providerRef: 'provider:keyed-api' }),
  ...
  it('preserves a dynamic-keyed output and grounds completion at its RFC 6901 root', async () => {
    const outputSchema = { type: 'object', additionalProperties: { type: 'number' } } as const
    const result = await admitProviderSchema({
  ...
  it('returns a named refusal for a circular local reference', async () => {
    const result = await admitProviderSchema({
      ...
      authority: { kind: 'keyless' },
      credential: { kind: 'keyless' },
      resolutionRoot: { components: { schemas: { Node: { type: 'object', properties: { next: { $ref: '#/components/schemas/Node' } } } } } },
      credentialParameterNames: [],
    }, dereferenceOpenApiSchema)

    expect(result).toEqual({ kind: 'refused', reason: 'admit_schema_circular_reference' })
  })
  ...
  it('returns a named refusal for a non-local (remote) reference', async () => {
    ...
    expect(result).toEqual({ kind: 'refused', reason: 'admit_schema_reference_unresolvable' })
  })
  ...
  it('returns a named refusal when a non-object output has no guaranteed field', async () => {
    ...
    expect(result).toEqual({ kind: 'refused', reason: 'admit_output_no_guaranteed_field' })
  })
})
