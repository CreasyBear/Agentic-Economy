...
describe('durable Action Execution lease', () => {
  it('grants one sync effect permit when two cold workers replay the same token', async () => {
    ...
    const create = () => createDurableActionExecutionTracer({
      ...
      nextExecutionRef: () => 'dev:sync:single-permit',
    ...
    const winner = await root.coldResume(prepared.executionRef)
    const loser = await root.coldResume(prepared.executionRef)
  ...
  it.each(origins)('expires a real lease, fails closed, reconciles, and cold-resumes takeover for $kind', async (origin) => {
    ...
    const create = () => createDurableActionExecutionTracer({
      ...
      now: () => now,
      nextExecutionRef: () => `dev:durable:expiry:${origin.kind}`,
    ...
    const cold = () => create().coldResume(prepared.executionRef)

    now = '2026-07-19T12:00:02.000Z'
    const expiry = await (await cold()).acquire({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: firstLease.view.executionVersion,
      authorityRef: prepared.authority!.reference,
      actor, origin, materialInput: input,
    ...
    expect(await (await cold()).acquire({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: expiry.view.executionVersion,
      authorityRef: prepared.authority!.reference,
      actor, origin, materialInput: input,
    ...
    const takeover = await (await cold()).acquire({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: reconciled.view.executionVersion,
      authorityRef: prepared.authority!.reference,
      actor, origin, materialInput: input,
    ...
    const port = createDevelopmentDurablePort(state)
    expect(await port.readHistory(prepared.executionRef, 0, 50)).toContainEqual(
    ...
    expect((await cold()).inspect(prepared.executionRef)).toMatchObject({
      origin,
      owner: actor,
      control: {
        state: 'leased',
    ...
    expect(JSON.stringify({
      ...
      history: await port.readHistory(prepared.executionRef, 0, 50),
