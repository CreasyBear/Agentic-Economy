...
const requestOrigin: ActionExecutionOrigin = {
  kind: 'request_owned',
  requestRef: 'mock:request:perth-plumber',
  revision: 3,
}

const standaloneOrigin: ActionExecutionOrigin = {
  kind: 'standalone',
  ...actor,
}
...
describe('in-memory Action Execution tracer', () => {
  beforeEach(() => {
  ...
  it('keeps a returned not-found distinct from runner execution success', async () => {
    ...
    const tracer = createInMemoryActionExecutionTracer({
      ...
      nextExecutionRef: () => 'dev:action-execution:read',
    })
  ...
  it.each([
    ...
    const tracer = createInMemoryActionExecutionTracer({
      action,
      now: () => '2026-07-19T06:00:00.000Z',
      nextExecutionRef: () => `dev:action-execution:${origin.kind}`,
  ...
  it('refuses cross-principal authority and invalidates changed material input without running', async () => {
    ...
    const tracer = createInMemoryActionExecutionTracer({
      action,
      now: () => '2026-07-19T06:00:00.000Z',
      nextExecutionRef: () => 'dev:action-execution:guard',
  ...
  it.each([
    ...
    const tracer = createInMemoryActionExecutionTracer({
      ...
      nextExecutionRef: () => `dev:action-execution:attempts:${origin.kind}`,
    ...
    const uncertain = await tracer.execute({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: preRelease.view.executionVersion,
      authorityRef: prepared.authority!.reference,
      actor,
      origin,
      materialInput: inquiryInput,
  ...
  it('fails closed when no release observer can prove a runner throw happened before release', async () => {
    ...
    const tracer = createInMemoryActionExecutionTracer({
      ...
      nextExecutionRef: () => 'dev:action-execution:missing-observer',
  ...
  it.each([
    ...
    for (const releasePosture of ['runner_not_yet_released', 'possibly_after_release'] as const) {
      ...
      const tracer = createInMemoryActionExecutionTracer({
        ...
        nextExecutionRef: () => `dev:timeout:${origin.kind}:${releasePosture}`,
      ...
      const pending = tracer.execute({
        executionRef: prepared.executionRef,
        expectedExecutionVersion: authority.view.executionVersion,
        authorityRef: prepared.authority!.reference,
        actor,
        origin,
        materialInput: inquiryInput,
      ...
      const currentAtTimeout = tracer.inspect(prepared.executionRef)
      ...
      expect(tracer.inspect(prepared.executionRef)).toEqual(currentAtTimeout)
      expect(developmentAdapter).toHaveBeenCalledTimes(1)
  ...
  it.each([
    ...
    const options = {
      action: requireDurableWriteFixtureAction(),
      now: () => now,
      nextExecutionRef: () => `dev:action-execution:fence:${origin.kind}`,
      nextAuthorityRef: () => `opaque:authority:fence:${origin.kind}`,
      nextAttemptRef: () => `dev:attempt:fence:${origin.kind}:${++attempt}`,
    }
    const tracer = createInMemoryActionExecutionTracer(options)
    const prepared = await tracer.prepare({
      origin,
      actor,
      input: inquiryInput,
    ...
    expect(await tracer.publishObservation({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: first.view.executionVersion - 1,
      ...firstToken,
      release: 'released',
    })).toMatchObject({ kind: 'refused', code: 'stale_execution_version' })

    await expect(tracer.executeAcquired({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: first.view.executionVersion - 1,
      ...firstToken,
    })).resolves.toMatchObject({ kind: 'refused', code: 'stale_execution_version' })
    expect(developmentAdapter).not.toHaveBeenCalled()

    const provenNotReleased = await tracer.publishObservation({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: first.view.executionVersion,
      ...firstToken,
    ...
    const takeover = await tracer.acquire({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: provenNotReleased.view.executionVersion,
      authorityRef: prepared.authority!.reference,
      actor,
      origin,
      materialInput: inquiryInput,
    ...
    expect(await tracer.publishObservation({
      executionRef: prepared.executionRef,
      expectedExecutionVersion: takeover.view.executionVersion,
      ...firstToken,
      release: 'released',
    })).toMatchObject({ kind: 'refused', code: 'effect_generation_stale' })
    expect(tracer.inspect(prepared.executionRef)).toEqual(takeover.view)
    ...
    const restored = createInMemoryActionExecutionTracer({
      ...options,
    ...
    expect(restored.inspect(prepared.executionRef)).toEqual(takeover.view)
    ...
    const cancelAfterRelease = await restored.cancel({
      executionRef: prepared.executionRef,
      idempotencyKey: `cancel:${prepared.executionRef}:possible-release`,
      expectedExecutionVersion: possibleRelease.view.executionVersion,
      actor,
      origin,
    })
    ...
    const unknownTracer = createInMemoryActionExecutionTracer({
      ...options,
      nextExecutionRef: () => `dev:action-execution:unknown:${origin.kind}`,
    })
    ...
    const unknownAuthority = await unknownTracer.decide({
      executionRef: unknownPrepared.executionRef,
      expectedExecutionVersion: unknownPrepared.executionVersion,
    ...
    const unknownLease = await unknownTracer.acquire({
      executionRef: unknownPrepared.executionRef,
      expectedExecutionVersion: unknownAuthority.view.executionVersion,
      authorityRef: unknownPrepared.authority!.reference,
      actor,
      origin,
      materialInput: inquiryInput,
    ...
    expect(await unknownTracer.acquire({
      executionRef: unknownPrepared.executionRef,
      expectedExecutionVersion: unknownLease.view.executionVersion,
      authorityRef: unknownPrepared.authority!.reference,
      actor,
      origin,
      materialInput: inquiryInput,
    ...
    const cancellationTracer = createInMemoryActionExecutionTracer({
      ...options,
      nextExecutionRef: () => `dev:action-execution:cancel:${origin.kind}`,
    })
    ...
    const cancelAuthority = await cancellationTracer.decide({
      executionRef: cancelPrepared.executionRef,
      expectedExecutionVersion: cancelPrepared.executionVersion,
    ...
    expect(await cancellationTracer.cancel({
      executionRef: cancelPrepared.executionRef,
      idempotencyKey: `cancel:${cancelPrepared.executionRef}:authority`,
      expectedExecutionVersion: cancelAuthority.view.executionVersion,
      actor,
      origin,
    })).toMatchObject({
      kind: 'accepted',
  ...
  it('refuses a runner completion that arrives after cancellation advanced control', async () => {
    ...
    const tracer = createInMemoryActionExecutionTracer({
      ...
      nextExecutionRef: () => 'dev:action-execution:late-completion',
    ...
    expect(developmentAdapter).toHaveBeenCalledTimes(1)
    const releaseStarted = tracer.inspect(prepared.executionRef)!
    ...
    const cancelled = await tracer.cancel({
      executionRef: prepared.executionRef,
      idempotencyKey: `cancel:${prepared.executionRef}:late-return`,
    ...
    expect(tracer.inspect(prepared.executionRef)).toEqual(
