...
it('shows complete indexed Provider counts and links to exact storefronts and separate pagination', () => {
  render(<DirectoryProvidersIndex window="30d" result={{ kind: 'ok', coverage, page: [{ key: 'example.com', label: 'Example', count: 142 }, { key: 'other.example.com', label: 'Other', count: 1 }], isDone: false, continueCursor: 'cursor:+/=' }} />)
  ...
  expect(Object.fromEntries(store.searchParams)).toEqual({ window: '30d', view: 'tools', provider: 'example.com' })
  const next = new URL(screen.getByRole('link', { name: 'Next Providers' }).getAttribute('href')!, 'https://aecon.ai')
  expect(Object.fromEntries(next.searchParams)).toEqual({ window: '30d', view: 'providers', providerCursor: 'cursor:+/=' })
...
it('keeps an empty intermediate page traversable and hides next on the final page', () => {
  const { rerender } = render(<DirectoryProvidersIndex window="30d" result={{ kind: 'ok', coverage, page: [], isDone: false, continueCursor: 'next' }} />)
  ...
  rerender(<DirectoryProvidersIndex window="30d" result={{ kind: 'ok', coverage, page: [], isDone: true, continueCursor: '' }} />)
...
it('reports index failure without presenting a sampled complete list', () => {
  render(<DirectoryProvidersIndex window="30d" result={{ kind: 'unavailable', reason: 'index_unavailable' }} />)
