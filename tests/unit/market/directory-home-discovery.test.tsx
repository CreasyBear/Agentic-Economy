...
function show(data = home) {
  ...
  render(<RouterContextProvider router={router}><DirectoryHomeDiscovery home={data} window="7d" onSelect={select} onSave={save} onCompare={compare} isSaved={noop} isComparing={noop} compareDisabled={noop} /></RouterContextProvider>)
...
it('orders editorial collections and preserves each source search in links and callbacks', () => {
  ...
  for (const callback of [select, save, compare]) expect(callback).toHaveBeenCalledWith({ entry, search: { query: 'image generation creative' } })
})
it('keeps healthy collections visible after a local failure and limits each shelf to eight Tools', () => {
  ...
  for (let i = 0; i < 8; i++) expect(screen.getByRole('button', { name: `Creative Tool ${i}` })).toBeTruthy()
  expect(screen.queryByRole('button', { name: 'Creative Tool 8' })).toBeNull()
