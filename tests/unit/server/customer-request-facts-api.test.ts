...
describe('customer Request facts HTTP API', () => {
  it('provides only facts and optimistic revision to the source application', async () => {
    ...
    const response = await handleCustomerRequestFactsPost(request({
      idempotencyKey: 'facts:1', expectedRevision: 1, facts: { destinationPostcode: '6000' },
    ...
    expect(provideFacts).toHaveBeenCalledWith({
      ...
      facts: { destinationPostcode: '6000' },
    })
  ...
  it('rejects empty facts before calling the application', async () => {
    const provideFacts = vi.fn()
    const response = await handleCustomerRequestFactsPost(request({
      idempotencyKey: 'facts:1', expectedRevision: 1, facts: {},
    }), 'request:1', { provideFacts })
