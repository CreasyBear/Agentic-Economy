...
describe('money payout policy', () => {
  ...
  it('holds below fixture threshold and gates transfers by KYC', () => {
    const held = transitionPayout({ current: payout, now: 10, action: { kind: 'review', autoApprove: true }, account: { detailsSubmitted: true, recipientCapabilityActive: true } })
    expect(held).toMatchObject({ kind: 'accepted', value: { state: 'held_threshold' } })
    const kyc = transitionPayout({ current: { ...payout, state: 'review', providerNetMinor: 3_000 }, now: 10, action: { kind: 'review', autoApprove: true }, account: { detailsSubmitted: false, recipientCapabilityActive: false } })
    expect(kyc).toMatchObject({ kind: 'accepted', value: { state: 'held_kyc' } })
    const ready: PayoutTransitionInput = { current: { ...payout, providerNetMinor: 3_000, state: 'held_threshold' }, now: 11, action: { kind: 'release_transfer', stripeTransferId: 'tr_1' }, account: { detailsSubmitted: true, recipientCapabilityActive: true } }
  ...
  it('freezes an unknown transfer and only settles with reconciliation', () => {
    const unknown = transitionPayout({ current: { ...payout, state: 'transfer_pending' }, now: 10, action: { kind: 'transfer_unknown', stripeTransferId: 'tr_1' }, account: { detailsSubmitted: true, recipientCapabilityActive: true } })
    ...
    expect(transitionPayout({ current: unknown.value, now: 11, action: { kind: 'reconcile', outcome: 'not_released', stripeTransferId: 'tr_1' }, account: { detailsSubmitted: true, recipientCapabilityActive: true } })).toMatchObject({ kind: 'accepted', value: { state: 'held_threshold' } })
    expect(transitionPayout({ current: unknown.value, now: 11, action: { kind: 'reconcile', outcome: 'paid', stripeTransferId: 'tr_1' }, account: { detailsSubmitted: true, recipientCapabilityActive: true } })).toMatchObject({ kind: 'accepted', value: { state: 'paid' } })
  })
