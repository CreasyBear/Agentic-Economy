...
describe('money payout policy', () => {
  it('requires status readback and does not infer KYC from redirect', () => {
    ...
    const statusInput = { businessId: 'business-1', currency: 'USD', exponent: 2, stripeAccountId: 'acct_1', event: { kind: 'status' as const, detailsSubmitted: true, recipientCapabilityActive: true, restricted: false, requirementsDigest: 'sha256:requirements', stripeEventId: 'evt_1', observedAt: 3 }, ...(returned.kind === 'accepted' ? { current: returned.value } : {}) }
  ...
  it('holds below fixture threshold and gates transfers by KYC', () => {
    ...
    const kyc = transitionPayout({ current: { ...payout, state: 'review', providerNet: amount('USD', '3000', 2) }, now: 10, action: { kind: 'review', autoApprove: true }, account: { state: 'submitted', detailsSubmitted: false, recipientCapabilityActive: false } })
    expect(kyc).toMatchObject({ kind: 'accepted', value: { state: 'held_kyc' } })
    const ready: PayoutTransitionInput = { current: { ...payout, providerNet: amount('USD', '3000', 2), state: 'held_threshold' }, now: 11, action: { kind: 'release_transfer', stripeTransferId: 'tr_1' }, account: { state: 'ready', detailsSubmitted: true, recipientCapabilityActive: true } }
  ...
  it('retains restricted onboarding state and refuses release despite stale success flags', () => {
    ...
    expect(transitionPayout({
      current: { ...payout, state: 'held_threshold', providerNet: amount('USD', '3000', 2) },
      now: 3,
