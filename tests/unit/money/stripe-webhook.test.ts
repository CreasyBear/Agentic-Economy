...
describe('money Stripe webhook adapter', () => {
  it('fails closed before Stripe setup and never applies a browser return', async () => {
    ...
    expect(await response.json()).toMatchObject({ kind: 'UNAVAILABLE', code: 'stripe_setup_required' })
  })

  it('verifies raw body before applying and supports duplicate event replay in the applier', async () => {
    const applied: string[] = []
    const event = { stripeEventId: 'evt_1', eventType: 'payment_intent.succeeded' as const, externalRef: 'pi_1', principalId: 'clerk_api_key:key-1', accountRef: 'clerk_api_key:key-1:USD', amount: amount('USD', '1000', 2), payloadDigest: 'payload-1', observedAt: 2 }
