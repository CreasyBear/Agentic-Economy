...
vi.mock('@/modules/capability-execution/call-worker/recover', async (importOriginal) => ({
  ...
  recoverCapabilityCall: recoverMock,
...
import { reconcileScheduled } from '../../../convex/capabilityCallWorker'
import {
...
function row(
  ...
  return {
    _id: `id:${callRef}`,
    callRef,
    principalId: `principal:${callRef}`,
    credentialId: `credential:${callRef}`,
...
function authorizedInvocation(callRef: unknown) {
  return {
    kind: 'authorized',
    authority: {
      principalId: `principal:${String(callRef)}`,
      accountRef: `account:${String(callRef)}`,
      credentialId: `credential:${String(callRef)}`,
      grantRef: `grant:${String(callRef)}`,
      grantGeneration: 1,
      policyDigest: `policy:${String(callRef)}`,
      expiresAt: 9_999_999,
    },
  }
}
...
describe('scheduled capability invocation reconciliation primitives', () => {
  it('uses the reconciliation index, caps at 25, and redacts candidate rows', async () => {
    ...
    expect(candidates[0]).toEqual({ callRef: 'candidate:0', attemptCount: 0, nextAttemptAt: 1_000 })
    expect(Object.keys(candidates[0] as object)).toEqual(['callRef', 'attemptCount', 'nextAttemptAt'])
  ...
  it('wins the first lease, blocks a live lease, rejects stale finishes, and permits takeover after expiry', async () => {
    ...
    await expect(claimHandler({ db }, { callRef: 'claimable', leaseOwner: 'worker:one', now: 1_000 }))
      .resolves.toEqual({ kind: 'claimed', principalId: 'principal:claimable', credentialId: 'credential:claimable' })
    await expect(claimHandler({ db }, { callRef: 'claimable', leaseOwner: 'worker:two', now: 1_001 }))
      .resolves.toEqual({ kind: 'not_claimed' })
    await expect(finishHandler({ db }, {
      callRef: 'claimable', leaseOwner: 'worker:two', now: 1_001, outcome: 'terminal',
    })).resolves.toEqual({ kind: 'refused', code: 'stale_lease' })

    await expect(claimHandler({ db }, { callRef: 'claimable', leaseOwner: 'worker:two', now: 61_000 }))
  ...
  it('applies the exact four backoffs and sends the fifth failure to manual review', async () => {
    ...
    for (const [index, backoff] of backoffs.entries()) {
      ...
      await expect(claimHandler({ db }, { callRef: 'backoff', leaseOwner: owner, now }))
        .resolves.toMatchObject({ kind: 'claimed' })
      const result = await finishHandler({ db }, {
        callRef: 'backoff', leaseOwner: owner, now, outcome: 'reconciliation_required',
      })
      if (index < 4) {
  ...
  it('clears reconciliation after a terminal recovery result', async () => {
    ...
    await expect(claimHandler({ db }, { callRef: 'complete', leaseOwner: 'worker:complete', now: 1_000 }))
      .resolves.toMatchObject({ kind: 'claimed' })
    await expect(finishHandler({ db }, {
      callRef: 'complete', leaseOwner: 'worker:complete', now: 1_001, outcome: 'terminal',
  ...
  it('initializes only absent schedules when an existing mutation projects reconciliation', async () => {
    ...
    await recordHandler({ db }, {
      callRef: 'absent', principalId: 'principal:absent', state: 'reconciliation_required',
      dispatchState: 'reconciliation_required', now: 5_000,
    })
    await recordHandler({ db }, {
      callRef: 'existing', principalId: 'principal:existing', state: 'reconciliation_required',
      dispatchState: 'reconciliation_required', now: 5_000,
    })
...
describe('scheduled capability invocation reconciliation worker', () => {
  it('does not widen cron authority when persisted invocation authority is refused', async () => {
    ...
    const due = [{ callRef: 'due:refused', attemptCount: 0, nextAttemptAt: 1_000 }]
    const ctx = {
      runQuery: vi.fn(async (reference: unknown, args: Record<string, unknown>) => {
        ...
        if (path.endsWith(':readOwnerRecovery')) return {
          principalId: `principal:${args.callRef}`, credentialId: `credential:${args.callRef}`,
        }
      ...
      runMutation: vi.fn(async (reference: unknown, args: Record<string, unknown>) => {
        ...
        if (path.endsWith(':reconcileCallWorkloadAuthority')) return { kind: 'refused' }
  ...
  it('processes candidates serially and continues after one item fails', async () => {
    const candidates = [
      { callRef: 'first', attemptCount: 0, nextAttemptAt: 1_000 },
      { callRef: 'second', attemptCount: 1, nextAttemptAt: 1_000 },
      { callRef: 'third', attemptCount: 2, nextAttemptAt: 1_000 },
    ]
    const order: string[] = []
    let active = 0
    let maximumActive = 0
    const finished: string[] = []
    recoverMock.mockImplementation(async (_ctx: unknown, args: { callRef: string }) => {
      active += 1
      maximumActive = Math.max(maximumActive, active)
      order.push(args.callRef)
      await Promise.resolve()
      active -= 1
      if (args.callRef === 'first') throw new Error('recovery_failed')
      return { kind: 'found', callRef: args.callRef, toolRef: 'tool', state: 'terminal' }
    })
    const ctx = {
      ...
      runMutation: vi.fn(async (reference: unknown, args: Record<string, unknown>) => {
        ...
        if (path.endsWith(':reconcileCallWorkloadAuthority')) {
          return authorizedInvocation(args.callRef)
        }
        if (path.endsWith(':claimAutomaticReconciliationCandidate')) {
          return { kind: 'claimed', principalId: `principal:${args.callRef}`, credentialId: `credential:${args.callRef}` }
        }
        if (path.endsWith(':finishAutomaticReconciliation')) {
          finished.push(String(args.callRef))
          return args.callRef === 'first' ? { kind: 'retried', attemptCount: 1, nextAttemptAt: 61_000 } : { kind: 'completed' }
        }
        throw new Error(`unexpected_mutation:${path}`)
      }),
    }
  ...
  it('stops starting new candidates after 45 seconds', async () => {
    ...
    const candidates = [
      { callRef: 'first', attemptCount: 0, nextAttemptAt: 1_000 },
      { callRef: 'second', attemptCount: 0, nextAttemptAt: 1_000 },
    ]
    const claimed: string[] = []
    recoverMock.mockImplementation(async (_ctx: unknown, args: { callRef: string }) => {
      claimed.push(args.callRef)
      now = 46_000
      return { kind: 'found', callRef: args.callRef, toolRef: 'tool', state: 'terminal' }
    })
    const ctx = {
      ...
      runMutation: vi.fn(async (reference: unknown, _args: Record<string, unknown>) => {
        ...
        if (path.endsWith(':reconcileCallWorkloadAuthority')) {
          return authorizedInvocation(_args.callRef)
        }
        if (path.endsWith(':claimAutomaticReconciliationCandidate')) {
  ...
  it('runs expiry first and spends only the remaining combined capacity on due candidates', async () => {
    ...
    const due = Array.from({ length: 30 }, (_, index) => ({
      callRef: `due:${index}`, attemptCount: 0, nextAttemptAt: 1_000,
    }))
    const recovered: string[] = []
    expireMock.mockImplementation(async (_ctx: unknown, args: { callRef: string }) => {
      recovered.push(`expire_authorization:${args.callRef}`)
      return { kind: 'reconciliation_required', callRef: args.callRef, toolRef: 'tool', evidence: {
      ...
      }, expiryDisposition: args.callRef === 'expired:manual' ? 'manual_review' : 'automatic' }
    })
    recoverMock.mockImplementation(async (_ctx: unknown, args: { callRef: string; mode?: string }) => {
      recovered.push(`${args.mode ?? 'status'}:${args.callRef}`)
      return { kind: 'found', callRef: args.callRef, toolRef: 'tool', state: 'terminal' }
    })
    const ctx = {
      runQuery: vi.fn(async (reference: unknown, args: Record<string, unknown>) => {
