...
vi.mock('@/modules/capability-execution/call-worker/providerConsequenceBridge', () => ({
...
import { callAttemptIdentityDigest, run } from '../../../convex/capabilityCallWorker'
...
type WorkerOptions = Readonly<{
  ...
  currentTool?: (operation: PublishedTool) => PublishedTool
  releaseCurrentOperation?: (operation: PublishedTool) => PublishedTool
  authorizationCurrentOperation?: (operation: PublishedTool) => PublishedTool
...
type Handler = (ctx: unknown, args: { callRef: string }) => Promise<unknown>
...
function operationFor(kind: WorkerKind, validUntil: number, priceUnits = '1', brokered = false): PublishedTool {
  const fixture = buildDevelopmentPublishedToolEvidence()
  const pricedOperation = operationWithPricing(fixture.tool, brokered
        ? {
            version: 'pricing:v3',
            kind: 'managed_x402',
...
function inRuntimeEnvironment(
  ...
  const identity: PublishedTool['identity'] & { runtimeEnvironment: 'sandbox' | 'production' } = {
    ...operation.identity,
    runtimeEnvironment,
  }
  return {
...
function sealCurrentOperation(operation: PublishedTool): PublishedTool {
  ...
  const connectionAuthority = operation.connectionAuthority === undefined
    ? undefined
    : (() => {
        const { operationRef: _legacyOperationRef, ...authority } = operation.connectionAuthority as typeof operation.connectionAuthority & {
          operationRef?: string
        }
        return { ...authority, toolRef: operationRef }
      })()
...
export function createWorker(kind: WorkerKind, options: WorkerOptions = {}): { ctx: Record<string, unknown>; state: WorkerState } {
  ...
  const authorityMaterial = {
    ...
    targetDigest: publishedToolIdentityDigest(operation.identity),
  ...
  const authority = {
    ...
    decisionDigest: canonicalDigest({
      ...
      invocationRef: authorityMaterial.callRef,
      operationRef: authorityMaterial.toolRef,
      inputDigest: authorityMaterial.inputDigest,
      grantRef: authorityMaterial.grantRef,
      grantGeneration: authorityMaterial.grantGeneration,
      grantDigest: authorityMaterial.grantDigest,
      reference: authorityMaterial.reference,
      targetDigest: authorityMaterial.targetDigest,
      consequence: authorityMaterial.consequence,
      limits: authorityMaterial.limits,
      expiresAt: authorityMaterial.expiresAt,
      acceptedBasis: canonicalAuthorityBasisMaterial(authorityMaterial.acceptedBasis),
    } as StableHashValue),
  }
  const operationJson = JSON.stringify(operation)
  const currentTool = sealCurrentOperation(options.currentTool?.(operation) ?? operation)
  const releaseCurrentOperation = sealCurrentOperation(
    options.releaseCurrentOperation?.(currentTool) ?? currentTool,
  )
  ...
  const dispatch: Record<string, unknown> = {
    ...(kind === 'x402' && options.sellerCanary !== true
      ? { quoteRef: `operation-commitment:v1:${'c'.repeat(64)}` }
      : {}),
    callRef: invocationRef,
  ...
  if (sellerCanary !== undefined) {
    ...
    dispatch.callRef = sellerCanary.callRef
    const canaryAuthorityMaterial = {
      ...authorityMaterial,
      callRef: sellerCanary.callRef,
    }
    dispatch.authority = {
      ...canaryAuthorityMaterial,
      decisionDigest: canonicalDigest({
        format: 'operation-invoke-authority:v1',
        invocationRef: canaryAuthorityMaterial.callRef,
        operationRef: canaryAuthorityMaterial.toolRef,
        inputDigest: canaryAuthorityMaterial.inputDigest,
        grantRef: canaryAuthorityMaterial.grantRef,
        grantGeneration: canaryAuthorityMaterial.grantGeneration,
        grantDigest: canaryAuthorityMaterial.grantDigest,
        reference: canaryAuthorityMaterial.reference,
        targetDigest: canaryAuthorityMaterial.targetDigest,
        consequence: canaryAuthorityMaterial.consequence,
        limits: canaryAuthorityMaterial.limits,
        expiresAt: canaryAuthorityMaterial.expiresAt,
        acceptedBasis: canonicalAuthorityBasisMaterial(canaryAuthorityMaterial.acceptedBasis),
      } as StableHashValue),
    }
  }
  const managedCustody = kind === 'x402'
  const paymentIdentifier = callAttemptIdentityDigest({
    callRef: String(dispatch.callRef),
    principalId: 'principal:test-worker',
    credentialId: 'credential:test-worker',
    applicationRef: 'application:test-worker',
    environment,
  ...
  mocks.claimCanonicalExecution.mockResolvedValue({ kind: 'claimed', snapshot: { control: canonicalControl, attempt: canonicalAttempt } })
  ...
  const managedCallReservation = {
    reservationRef: `call-reservation:${String(dispatch.callRef)}`,
    commitmentRef: String(dispatch.quoteRef ?? ''),
    decisionAudUnits: '20000',
    journalTransactionRef: `journal:call-reservation:${String(dispatch.callRef)}`,
    treasuryReservationRef: `treasury-reservation:${String(dispatch.callRef)}`,
  }
  const sellerCanarySnapshot = (candidate: PublishedTool) => ({
    toolJson: JSON.stringify(candidate),
    toolRef: operationRef,
  ...
  const ctx = {
    runQuery: vi.fn(async (reference: unknown, args?: Record<string, unknown>) => {
      ...
      switch (queryPath) {
        case 'capabilityCalls:openDispatch': return dispatch
        ...
        case 'capabilitySupplyTools:readCurrentPublishedToolSnapshot':
          currentToolReads += 1
          if (currentToolReads === 2) state.events.push('current-publication-price-revalidation')
          return {
              toolJson: JSON.stringify(
              currentToolReads === 1 ? currentTool : releaseCurrentOperation,
            ),
          }
        case 'capabilitySupplyCurrentTool:readExactSellerCanaryOperationSnapshot':
          currentToolReads += 1
          if (currentToolReads === 2) state.events.push('current-publication-price-revalidation')
        ...
        case 'capabilityCalls:readCurrentProviderConnectionAuthority':
          return sellerCanary === undefined
            ? { kind: 'credentialed' }
            : { kind: 'credentialless_x402' }
        case 'capabilityCalls:readProviderLeaseAuthority':
    ...
    runMutation: vi.fn(async (reference: unknown, args: Record<string, unknown>) => {
      ...
      switch (path) {
        case 'capabilityCalls:claimDispatch':
        ...
        case 'capabilityCalls:finalizeDispatch': {
        ...
        case 'capabilityCalls:reconcileCallWorkloadAuthority':
          return {
            kind: 'authorized',
            authority: {
  ...
  mocks.invokeProviderConsequenceViaVercel.mockImplementation(async (bridgeCtx: unknown, input: {
    ...
    const reserved = await runMutation('moneyLedger:reserveExternalInvocationSpend', {
      ...
      environment: dispatch.environment,
      invocationRef: dispatch.callRef,
      operationRef: dispatch.toolRef,
      attemptRef,
      effectGeneration: 1,
      providerRef,
