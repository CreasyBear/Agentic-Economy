...
describe('Tool chat OpenRouter contract', () => {
  it('sends exactly six provider-safe names that round-trip to canonical IDs', async () => {
    ...
    try {
      ...
      if (tools === undefined) throw new Error('Tool chat tools are unavailable')

      await generateText({
        model,
        prompt: 'Describe the available Market Tools.',
        tools,
      })
  ...
  it('feeds the literal completed Tool Call result into the model continuation step', async () => {
    const callToolName = CHAT_TOOL_NAME_MAP.canonicalToProvider['tool.call']
    const freshValue = 'fresh-provider-value-7f9c'
    const completed = {
      kind: 'completed' as const,
      callRef: 'invocation:provider-contract:1',
      toolRef: TOOL_REF,
    ...
    const server = await startOpenRouterContractServer([
      openRouterToolCallResponse(callToolName, {
