...
function storeFixture(): CustomerRequestAgentOAuthStore & { grants: Map<string, CustomerRequestAgentOAuthGrant> } {
...
const deviceClient: CustomerRequestAgentOAuthClient = {
...
const authClient: CustomerRequestAgentOAuthClient = {
  ...deviceClient,
...
async function deviceGrant(store: CustomerRequestAgentOAuthStore) {
...
describe('Customer Request OAuth state machine', () => {
  it('stores only one exact mode and hashes opaque values', async () => {
    ...
    expect(normalizeRequestedScopes('customer_requests:create customer_requests:approve_each')).toEqual({
      mode: 'approve_each', scopes: ['customer_requests:create', 'customer_requests:approve_each'],
    })
