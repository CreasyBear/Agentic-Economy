...
describe('registered action invocation contract', () => {
  it('requires and preserves the exact declared metadata', async () => { const classifiedAction = defineAction({
  ...
  it('binds a prepared WorkTree apply to its durable operation key', async () => { let invocationSequence = 0
  ...
  const replay = await tracer.acquire({
    invocationRef: prepared.invocationRef,
  ...
  const secondPrepared = await tracer.prepare({
    origin: workTreeApplyOrigin,
    actor: workTreeApplyActor,
    input,
    context: {},
  ...
  const secondDecision = await tracer.decide({
