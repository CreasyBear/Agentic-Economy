...
describe('durable write and read back', () => {
  it('persists a retained Call and reads it back by recovery reference', () => {
    const { record, created } = retainCallRecovery(request(), quote, 'idempotency-key-1')
...
describe('findCallRecovery', () => {
  it('finds an unresolved request by requestDigest when no idempotency key was supplied', () => {
    const { record } = retainCallRecovery(request(), quote, 'idempotency-key-2')
    const found = findCallRecovery(request())
    expect(found).toEqual(record)
  })

  it('finds a saved record by explicit idempotency identity even after it becomes terminal', () => {
    const withKey = request({ idempotencyKey: 'stable-key' })
    const { record } = retainCallRecovery(withKey, quote, 'idempotency-key-3')
    acknowledgeCallRecovery(record, { callRef: 'call:one', terminal: true })
    const found = findCallRecovery(withKey)
    expect(found?.recoveryRef).toBe(record.recoveryRef)
    expect(found?.terminal).toBe(true)
  })

  it('returns undefined once a digest-only request has resolved to a terminal state', () => {
    const { record } = retainCallRecovery(request(), quote, 'idempotency-key-4')
    acknowledgeCallRecovery(record, { callRef: 'call:one', terminal: true })
...
describe('acknowledgeCallRecovery', () => {
  it('moves the record to a callRef and terminal state and persists it', () => {
    const { record } = retainCallRecovery(request(), quote, 'idempotency-key-5')
    expect(record.callRef).toBeUndefined()
    expect(record.terminal).toBe(false)
    const updated = acknowledgeCallRecovery(record, { callRef: 'call:two', terminal: true })
...
describe('corrupt journal storage', () => {
  it('fails closed on a corrupt record while leaving other records intact', () => {
    const { record: corrupted } = retainCallRecovery(request(), quote, 'idempotency-key-6')
    const { record: healthy } = retainCallRecovery(request({ toolRef: 'operation:v1:other' }), quote, 'idempotency-key-7')
...
describe('write lock recovery', () => {
  it('reclaims a stale lock directory older than 30s and lets the write succeed', () => {
    mkdirSync(lockPath(), { recursive: true, mode: 0o700 })
    const staleMtime = new Date(Date.now() - 31_000)
    utimesSync(lockPath(), staleMtime, staleMtime)
    const { record, created } = retainCallRecovery(request(), quote, 'idempotency-key-8')
    expect(created).toBe(true)
    expect(readCallRecovery(record.recoveryRef, origin)).toEqual(record)
  })

  it('respects a fresh lock held by another lockSync call and fails closed without writing', () => {
    mkdirSync(callsDirectory(), { recursive: true, mode: 0o700 })
    const release = lockfile.lockSync(callsDirectory(), { realpath: false })
    try {
      expect(() => retainCallRecovery(request(), quote, 'idempotency-key-11')).toThrow(CliFailure)
      try {
        retainCallRecovery(request(), quote, 'idempotency-key-11')
        expect.unreachable()
      } catch (error) {
        expect(error).toBeInstanceOf(CliFailure)
