...
import { validateGatewayProductionSmokeReceipt } from "../../../tools/release/validate-tool-gateway-production-smoke-receipt";
...
describe("hosted Operation gateway smoke receipt", () => {
  it("validates a strict receipt through the source-only validator", () => {
    const receipt = strictReceipt();
    expect(validateGatewayProductionSmokeReceipt(receipt)).toEqual(receipt);
  });
