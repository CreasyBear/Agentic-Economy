...
} from "../../../tools/release/tool-gateway-production-smoke-receipt";
import type { GatewaySmokeConfig } from "../../../tools/release/tool-gateway-production-smoke";
import {
  isPublicToolRef,
...
import { CALL_ROUTE_CONTRACT } from "../../../src/modules/capability-execution/call-entry";
import {
...
import type { GatewayToolQuote } from "../../../tools/release/tool-gateway-production-smoke-call";
...
const controlToolRefCandidate = `operation:v1:${"c".repeat(64)}`;
if (!isPublicToolRef(controlToolRefCandidate))
  throw new Error("test_control_tool_ref_invalid");
export const controlToolRef = controlToolRefCandidate;
const toolRefCandidate = `operation:v1:${"a".repeat(64)}`;
if (!isPublicToolRef(toolRefCandidate))
  throw new Error("test_tool_ref_invalid");
export const toolRef = toolRefCandidate;
...
export const tool = {
  ...
  navigation: [],
} satisfies PublicToolDescriptor;

export const quoteRef = `operation-commitment:v1:${"d".repeat(64)}`;

export function committedQuote(
  ...
  return {
    kind: "committed",
    quoteRef,
    toolRef: selectedToolRef,
    toolVersion: 1,
    expiresAt: Math.max(observedAt, Date.now()) + 60_000,
    normalizedInput: { city: "Perth" },
    price,
    account: {
      accountRef: "account:buyer",
      available: { currency: "AUD", units: "5000000", exponent: 6 },
    },
    budget: {
      principalRef: "principal:buyer",
      maximumPerCall: { currency: "AUD", units: "1000000", exponent: 6 },
    },
    policyRefs: ["policy:buyer"],
...
function currentProjectionTool(): PublicToolDescriptor {
  const now = Date.now();
  return {
    ...tool,
    availability: {
...
export function publicToolSearchResponse(
  query = "paid",
  ...
  const source: ToolSearchResult = {
    ...
    matchedCount: 1,
    ranking: [{ toolRef, rank: 1, score: 1 }],
    pagination: { limit: 20, hasMore: false },
    navigation: [],
  };
  const projected = projectToolSearchChoices(source);
  if (projected.kind !== "ok")
    throw new Error("test_public_tool_search_projection_unexpected");
  return projected;
}

export function publicToolDescribeResponse(): Extract<
  ReturnType<typeof projectToolDescription>,
  { kind: "found" }
> {
  ...
  if (projected.kind !== "found")
    throw new Error("test_public_tool_description_projection_unexpected");
  return projected;
}

export function publicBusinessDetailResponse(
  businessId = tool.business.businessId,
) {
  const now = Date.now();
  return registryDetailAction.outputSchema.parse({
    kind: "found",
    ...
    business: {
      ...
      publicUrl: "https://provider.example",
...
export function completed(callRef = "invocation:provider:1") {
  return {
...
export function strictReceipt(): GatewayProductionSmokeReceipt {
  ...
  if (!isPublicToolRef(controlRefCandidate))
    throw new Error("test_control_tool_ref_invalid");
...
export function serviceFetch(responses: readonly unknown[]): {
  config: Pick<GatewaySmokeConfig, "baseUrl" | "apiKey" | "input" | "fetch">;
  urls: string[];
} {
  let index = 0;
  const urls: string[] = [];
