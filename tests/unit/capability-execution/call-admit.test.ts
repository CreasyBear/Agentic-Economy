...
describe('operation.invoke admit/preflight', () => {
  it('refuses malformed operation references before grant or source reads', async () => {
    ...
    const result = await service.callTool({
      ...
      input: { quoteRef: QUOTE_REF, decisionPrice: DECISION_PRICE, toolRef: 'not-an-operation-ref', input: {}, idempotencyKey: 'idem:invalid-ref' },
    })
  ...
  it('refuses stale publication material before constructing an adapter', async () => {
    ...
    const service = createCallApplication(runtime({
      currentTool: async () => ({ operation, toolRef: operationRef, descriptor: staleDescriptor }),
      dispatch: async () => {
        adapters += 1
    ...
    const result = await service.callTool({
      ...
      input: { quoteRef: QUOTE_REF, decisionPrice: DECISION_PRICE, toolRef: operationRef, input: { symbol: 'BTC', convert: 'USD' }, idempotencyKey: 'idem:stale-revision' },
    })

    expect(result).toMatchObject({ kind: 'refused', toolRef: operationRef, code: 'operation_not_current', retryable: false })
  ...
  it('cleans unique preflight refusals so a later valid call is admitted', async () => {
    ...
    const service = createCallApplication(runtime({
      currentTool: async () => (
        mode === 'stale'
          ? { operation, toolRef: operationRef, descriptor: staleDescriptor }
          : mode === 'invalid'
            ? { operation, toolRef: operationRef, descriptor: invalidDescriptor }
            : { operation, toolRef: operationRef, descriptor }
      ),
      idempotency: {
      ...
      policy: {
        ...
        evaluateAuthority: async () => ({
          ...
          basis: { kind: 'approval_required' as const, authorityRef: 'authority:valid' },
          expiresAt: new Date(Date.now() + 30_000).toISOString(),
        }),
      },
      dispatch: async () => ({ kind: 'enqueued' as const }),
    }))
    const invoke = (requestPrincipal: AgentAccessPrincipal, idempotencyKey: string) => service.callTool({
      principal: requestPrincipal,
      correlationId: `correlation:${idempotencyKey}`,
      input: { quoteRef: QUOTE_REF, decisionPrice: DECISION_PRICE, toolRef: operationRef, input: { symbol: 'BTC', convert: 'USD' }, idempotencyKey },
    })
    ...
    await expect(invoke(principal, 'idem:valid-after-preflight')).resolves.toMatchObject({ kind: 'pending', toolRef: operationRef })
  ...
  it('rejects contract-invalid input before authority or provider execution', async () => {
    ...
    const invalidDescriptor = { ...descriptor, validateInput: () => false }
    const service = createCallApplication(runtime({
      currentTool: async () => ({ operation, toolRef: operationRef, descriptor: invalidDescriptor }),
      policy: {
        ...runtime().policy,
        evaluateAuthority: async () => {
          authorityReads += 1
    ...
    const result = await service.callTool({
      ...
      input: { quoteRef: QUOTE_REF, decisionPrice: DECISION_PRICE, toolRef: operationRef, input: {}, idempotencyKey: 'idem:input-invalid' },
    })

    expect(result).toMatchObject({ kind: 'refused', toolRef: operationRef, code: 'input_invalid', retryable: false })
  ...
  it('refuses production principals before authority or dispatch for development evidence', async () => {
    ...
    const result = await service.callTool({
      ...
      input: { quoteRef: QUOTE_REF, decisionPrice: DECISION_PRICE, toolRef: operationRef, input: { symbol: 'BTC', convert: 'USD' }, idempotencyKey: 'idem:production-development-evidence' },
    })
