...
- 2026-08-25: Phase acceptance now requires a fresh post-implementation task. Each phase implementer stops with an exact ref and evidence handoff; a separate context-independent task performs gate validation, code and architecture review, outcome evaluation, and an Ox adversarial/red-team pass. The next phase may start only after that task passes or all findings are repaired and independently rechecked.
...
- 2026-08-25: Phase progression and final maturity evidence are separated. A phase may be `SOURCE_ACCEPTED` or `SOURCE_ACCEPTED_EVIDENCE_OPEN` and unblock its source-dependent successor when its frozen behavior, invariants, tests, codegen and release gates pass. Cloud access, hosted proof, vendor credentials, legal approval, soak periods, customer counts and commercial metrics remain named open evidence and block only their owning operational, branch or GA gate. `CHANGES_REQUIRED` is reserved for source defects, false gates, invariant failures or missing dependencies that make the next phase unsafe. L3 completion still requires the root ledger with every external/manual gate met and zero abandonments.
