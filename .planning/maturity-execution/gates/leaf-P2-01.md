...
- [x] G1: The leaf's observable outcome is implemented completely under the frozen contract.
  EVIDENCE: Context-local transactional service issues root authority only through a trusted Membership/Account port, delegates arbitrary multi-hop grants with strict monotonic narrowing, validates every persisted grant, parent-child edge and replayed admission snapshot before use, atomically admits consequences, advances revocation generations and returns reconstructed deep-frozen attributed snapshots; 37 owned assertions pass with exact 100% critical coverage.
...
- [x] G7: The expert reread, defect hunt and free-polish pass found no remaining improvement.
  EVIDENCE: Four final repair passes complete. Implementation enforces parent-child scope/resource narrowing inside pinned ancestry and shape-checks provenance before dereference; domain reread extended safe parsing to creation/revocation facts and malformed stored objects; hostile tests retain both verifier reproducers plus null, contradictory lifecycle and forged revoker cases; free polish keeps stable DelegationError failures and valid pinned replay after later generation advance/revocation. Final owned oxlint, tests and exact coverage are green.
