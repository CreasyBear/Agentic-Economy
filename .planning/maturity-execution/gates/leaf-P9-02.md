# Gates: P9-02 — SLO and commercial soak

Scope: The 30-day SLO and 90-day commercial gates are measured from durable production facts.

Ownership: tools/readiness/soak, tests/readiness/soak, tests/maturity/leaf-P9-02.test.ts
...
- [ ] G2: A leaf-specific executable contract test exists.
  CHECK: cd ../.. && test -f tests/maturity/leaf-P9-02.test.ts && echo LEAF_TEST_PRESENT
  EXPECT: LEAF_TEST_PRESENT
  EVIDENCE: pending

- [ ] G3: The leaf-specific contract test passes.
  CHECK: cd ../.. && npx vitest run tests/maturity/leaf-P9-02.test.ts
  EVIDENCE: pending

- [ ] G4: The critical negative invariant is proved: manual estimates cannot satisfy numeric gates.
  EVIDENCE: pending
...
- [x] G6: Owned production files contain no placeholder implementation markers.
  CHECK: cd ../.. && if rg -n '(TODO|FIXME|not implemented)' tests/maturity/leaf-P9-02.test.ts 2>/dev/null; then exit 1; else echo NO_PLACEHOLDERS; fi
