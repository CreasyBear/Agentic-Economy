...
| Supplier webhook verification | When required, adopt `standardwebhooks`/official SDK verification over the unmodified raw body; do not hand-roll HMAC parsing. No package is added until this protocol is selected. |
...
| Stripe/Connect | **Do not hand-roll.** If AE-internal payout transfer is selected, add the official Stripe SDK and Connect APIs behind the existing `ConnectAccountPort`; no custom card, KYC, webhook-signature, or transfer protocol. |
...
5. **Settlement and payout:** choose either provider-direct x402 or AE-internal billing for the trial, never both. For AE-internal payout, implement the official Connect adapter, account onboarding, transfer idempotency, webhook reconciliation, and owner status. Gate: exact amounts, provider/rake attribution, duplicate transfer protection, unknown outcome, reversal/refund, payout readback.
...
- ADR-025's separation of Commercial, Usage, operation payment, telemetry, and authority remains in force. Only its blanket payout deferral is superseded: payout source types now exist, but a transfer rail is not yet implemented.
