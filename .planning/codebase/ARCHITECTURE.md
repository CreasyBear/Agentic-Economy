...
Agentic Economy is a **market kernel** for admitted third-party Market Operations. The kernel owns operation identity and contract, authorization, exactly-once invocation, delivery evidence, and brokered money. Consuming agents own planning. HTTP, MCP, CLI, and first-party chat are thin adapters over one action plane.
...
│  ConvexHttpClient, source-write  │  │  mutations / queries / actions         │
...
| Inquiries | Governed send, customer record, owner reply (comms, not paid invoke) | `src/modules/inquiries/public.ts` |
| Discovery / SEO | Manifests, llms.txt, UCP, public JSON-LD | `src/modules/discovery/public.ts`, `src/modules/seo/public.ts` |
| Product frontier | Quarantine 410/403 for retired family doors | `src/modules/product-frontier/` |
...
| Business tools | Listing-bound prepare/commit descriptors | `src/modules/business-tools/public.ts` |
| Notification outbox | Novu/Resend dispatch + webhook ingest | `src/modules/notification-outbox/public.ts` |
| Demand / search gaps | Unmet query observations for operators | `src/modules/demand/public.ts` |
...
## Module inventory (where logic lives)

Treat `src/modules/` as the map. Kernel vs proving ground vs owner tooling:

**Market kernel (load-bearing):**
- `capability-contract` — JSON Schema contracts, effects, data-use, annotations (`public.ts`).
- `capability-contract-registry` — durable encoded documents.
- `capability-supply` — admit/publish/bind/eligibility/readiness/operation projection; `internal/{offering,binding,eligibility,publication,quarantine,operation-ledger,graph,shared}`; Node extras in `server.ts`.
- `registry` — public search documents, business/service DTOs, `operations.actions.ts`, `registry.actions.ts`, `directory-fallback.functions.ts`, `operation-detail-route.functions.ts`.
- `capability-execution` — `operation.execute` (`operation-execute.functions.ts` + `operation-execute.server.ts` + `operation-execute-mcp.actions.ts`) and `operation.invoke` (`operation-invoke.ts` + `operation-invoke.actions.ts` + `operation-recovery.actions.ts`). `public.ts` / `index.ts` currently re-export the keyless execute seam; invoke is imported from `operation-invoke.ts` / `operation-invoke-entry.ts`.
- `action-invocation` — claim, fenced execution, x402 reconciliation evidence, development host ports.
- `money` — exact amounts, ledger, budgets, live-money gate, Stripe in `server.ts`.
- `agent-access` — principals, OAuth (`oauth-state.ts`), policy grants (`policy.functions.ts`), `agent-access.ts`.
- `network-guard` — public HTTP target checks.
- `actions` + `common/action.ts` — registry and `defineAction`.
- `harness` — `run-loop.ts`, `emission-guard.ts`, `session-journal.ts`, `harness.functions.ts`.
- `model-gateway` — `openRouterModel`.
- `security` — CSRF/source-write, admin authority, disputes, duplicates, suppression.
- `observability` — funnel, audit, operation keys, operator controls.
- `governed-action` — JCS canonicalization for inquiry/business-tool commits.

**Distribution / catalog:**
- `catalog`, `business`, `discovery`, `seo`, `storefront`.

**First-party proving ground:**
- `answer`, `answer-thread`, `inquiries`, `work-tree`, `study`, `demand`.
- `business-tools` (listing tools at `src/routes/$slug.tools.$toolId.ts`).

**Owner / ops / parked:**
- `settings`, `notification-outbox`, `dev`, `sandbox-supply`, `imported-commitment`, `external-run`, `project-spine`, `routing-kernel`, `product-frontier`.
...
- **Fail-closed money and effects.** Paid work is `operation.invoke` via `POST /api/v1/operations/call`. Anonymous keyless reads are `operation.execute` (MCP + Answer only). `/api/v1/operations/execute` is a 410 tombstone, not keyless execute.
...
- Contains: Product shells (`AePublicShell`, `AeOperatorShell`), chat (`AeChat.tsx`), claim (`AeFindMyBusiness.tsx`), supply, inquiries, work-tree panels.
...
- Location: `convex/`
- Contains: `schema.ts` (spreads module table bundles), function files that call module helpers, `http.ts` (routing-v1 410s), `crons.ts`, `marketDispatchWorkpool.ts`.
...
6. Convex `capabilityOperationInvocations:invoke` (`convex/capabilityOperationInvocations.ts`) reserves idempotency, checks grant/budget/authority, and claims a durable Action Invocation (`src/modules/action-invocation/`).
...
8. Terminal or reconcile-only state is persisted; HTTP returns `operationInvokeResultSchema` (completed / pending / needs_authority / refused / reconciliation_required).
...
4. Surfaces: MCP (`operationExecuteAction` in `src/modules/capability-execution/operation-execute-mcp.actions.ts`) and Answer tool loop. **No public HTTP route.**
...
2. Route bounds body to 16 KiB, admits `answer-turn-submit`, resolves a pseudonymous session cookie (`src/modules/answer-thread/internal/session-cookie.ts` via `public.ts`).
...
4. Convex reservation (`convex/answerThreads.ts`) is generation-fenced (30s lease, 25 turns/thread).
5. `streamAnswerTurn` (`src/modules/answer-thread/internal/turn-orchestrator.ts`, exported from `server.ts`) runs harness phases. Safe turns enter `runAnswerToolUseAgent` (`src/modules/answer/internal/answer-tool-use-agent.ts` via `src/modules/answer/server.ts`).
6. Model-facing read tools are exactly `ANSWER_READ_TOOL_IDS` (`src/modules/answer-thread/answer-thread.schema.ts`): `registry.search`, `registry.detail`, `registry.operations.search|detail|compare|inspectPlan`, `operation.execute`. Capability tools are generated from admitted descriptors; `MAX_EFFECT_CALLS=1`.
7. Privacy + answer gate (`src/modules/answer/internal/operation-result-presentation.ts`, `answer-gate.ts`) run before prose. Stream frames are transient `data-answer-event` parts; durable projection is readback via `/t/$threadId` and `src/modules/answer-thread/answer-thread.functions.ts`.
8. Landing example pills come from `AE_CATALOG_EXAMPLE_ASKS` (`src/modules/answer/catalog-example-asks.ts`) rendered by `AeAnswerSuggestions` (`src/components/ae/chat/AeSuggestionChips.tsx`). The chat host does not call a follow-up-chip API. Thread continuation uses `classifyFollowUpIntent` (`src/modules/answer-thread/internal/follow-up-intent.ts`) and `answer-continuation-state.ts`.

### Secondary Flow — MCP

1. `POST|DELETE /mcp` (`src/routes/mcp.ts`) rate-limits and calls `handleMcpRequest` (`src/lib/server/mcp-api.ts`).
...
4. Quarantine family action ids (`customerRequest.*`, `inquiry.*` except `inquiry.readCustomerRecord`, `study.*`, `workTree.*`) surface as 410 (`src/modules/product-frontier/quarantine-write-admission.ts`).
...
1. `/claim` (`src/routes/claim.tsx`) uses `searchClaimableBusinessesServer`, storefront enrich/import (`src/modules/storefront/`), and `submitOwnerClaimServer` (`src/modules/catalog/owner-claim.functions.ts`).
...
3. Public listing is `/$slug` (`src/routes/$slug.tsx`) from catalog/registry projections. Inquiry is `/$slug/inquiry` via `src/modules/inquiries/`.

### Secondary Flow — Customer Request HTTP (tombstone)

1. `/api/v1/requests/*` and `/api/requests/*` exist as route files and return 410 via `src/lib/server/customer-request-gone.ts`.
2. Successor for paid market work is `/api/v1/operations/call` (`DEPRECATION_SUCCESSOR_PATH` in `src/modules/product-frontier/deprecation-notice.ts`).
3. Convex mutations for those families may still exist; HTTP/MCP doors are retired. Server-fn writes in quarantine families return 403 freeze, not 410.

**State Management:**
- **Durable:** Convex tables composed in `convex/schema.ts` from module `internal/convex-schema.ts` / `internal/schema.ts` bundles.
...
- Examples: `src/modules/answer/public.ts` + `server.ts`; `src/modules/answer-thread/public.ts` + `server.ts` + `testing.ts`; `src/modules/capability-supply/server.ts` (Node/x402/readiness); `src/modules/money/server.ts` (Stripe server fns).
...
- Purpose: Fail-closed retired family doors without deleting Convex history.
- Examples: `src/modules/product-frontier/quarantine-write-admission.ts`, `quarantine-family-actions.ts`
- Pattern: HTTP/MCP 410 except `inquiry.readCustomerRecord`; never attach deprecation headers to `/call`.
...
**Convex:**
- Location: `convex/schema.ts`, `convex/convex.config.ts` (workflow, workpool, rateLimiter, aggregate), `convex/http.ts`, `convex/crons.ts`
- Triggers: App-server `callPublicSourceAction` / mutations; schedulers; Workpool
- Responsibilities: Transactions, probes every 1 minute, nonce/OAuth/inquiry cleanup hourly, daily supplier settlement `0 0 * * *`.
...
- **Threading:** Single-threaded Node event loop on the Start host. Convex queries/mutations are serializable transactions. Market invoke uses `@convex-dev/workpool` (32 slots reserved in `convex/marketDispatchWorkpool.ts`). Answer generation uses a 30-second fenced lease (`ANSWER_TURN_EXECUTION_LEASE_MS` in `src/modules/answer-thread/answer-thread.schema.ts`). Worker file `convex/capabilityOperationInvocationWorker.ts` is `"use node"` for undici/x402.
- **Global state:** OpenRouter provider cache in `src/modules/model-gateway/public.ts`. Compiled JSON Schema validators in `src/modules/capability-contract/public.ts` (`compiledContracts` / `compiledPointedSchemas` maps, bounded). In-memory Answer stream sessions in `src/components/ae/chat/turn-stream-session.ts` (per Node isolate). Do not add new process-wide mutable stores for market identity.
...
- **Quarantine:** Do not restore Customer Request planners, compilers, or Workpool graphs. Do not serve routing-v1 paths (`convex/http.ts` → `src/modules/routing-kernel/retirement.ts`).
- **Answer tool bounds:** `ANSWER_AGENT_MAX_TOOL_CALLS=4` and `MAX_EFFECT_CALLS=1` in `src/modules/answer/internal/answer-tool-use-agent.ts`. Navigation must search then exact-detail before a capability call. Confirmation without a bound pending decision disables tools (`planPendingOperationClarification` in the orchestrator). Boundary/refusal uses `src/modules/answer/internal/boundary-prose.ts` without an LLM.
- **Catalog vs capability:** Public businesses/offerings live in `catalog`/`business` tables and `src/modules/registry/internal/service-projection.ts`. Market Operations are capability publications. Do not collapse them. Listings may lack `origin.catalog_offering`; do not invent that link in adapters.
- **Rate limits:** HTTP classes in `src/lib/server/rate-limit.ts` (`public-read`, `answer-turn-submit`, agent-access). Convex `@convex-dev/rate-limiter` via `convex/rateLimit.ts` / `convex/lib` helpers used by invoke.

## Convex area map

App-server never inlines these; it calls named functions through `src/lib/server/convex-source.ts`.

| Area | Convex files | Module owners |
|------|----------------|---------------|
| Schema | `convex/schema.ts` | Spreads every `internal/convex-schema.ts` / `internal/schema.ts` bundle |
| Invoke | `capabilityOperationInvocations.ts`, `capabilityOperationInvocationWorker.ts`, `actionInvocationControl.ts`, `marketDispatchWorkpool.ts` | `capability-execution`, `action-invocation` |
| Supply | `capabilitySupply.ts`, `capabilitySupplyOperations.ts`, `capabilitySupplyPublicationPorts.ts`, `capabilitySupplyEligiblePorts.ts`, `capabilitySupplyGraphPorts.ts`, `capabilitySupplyWriterPorts.ts`, `capabilitySupplyReadiness.ts`, `capabilitySupplyProjection.ts`, `capabilitySupplyOwnerFunnel.ts`, `capabilitySupplyOwnerSupply.ts`, `capabilityProviderConnections.ts`, `capabilityProviderApprovals.ts`, `capabilityProviderConnectionCleanup.ts`, `capabilityContractDocuments.ts` | `capability-supply`, `capability-contract-registry` |
| Registry / catalog | `registry.ts`, `catalog.ts`, `catalogRuntimeQueries.ts`, `business.ts`, `businessSupplyProjectionSnapshot.ts`, `discovery.ts`, `searchGap.ts` | `registry`, `catalog`, `business`, `discovery`, `demand` |
| Answer | `answerThreads.ts`, `harnessSessions.ts` | `answer-thread`, `harness` |
| Money | `moneyLedger.ts`, `moneyX402PaymentAttempts.ts`, `qualifiedUse.ts` | `money` |
| Agent access | `agentAccessPrincipals.ts`, `agentAccessOAuth.ts`, `agentAccessPolicy.ts` | `agent-access` |
| Inquiries | `inquiries.ts`, `inquirySourceStateLoad.ts`, `inquirySourceStatePersist.ts`, `inquiryNotificationBridge.ts` | `inquiries`, `notification-outbox` |
| WorkTree / study | `workTrees.ts`, `workTreeApprovals.ts`, `workTreeRepeatLedger.ts`, `studies.ts` | `work-tree`, `study` |
| Notifications | `notificationOutbox.ts`, `notificationOutboxPersistence.ts`, `notificationOutboxSourceState.ts`, `notificationOutboxReconstruction.ts` | `notification-outbox` |
...
| Other | `settings.ts`, `observability.ts`, `externalRuns.ts`, `projectSpine.ts`, `routingKernelV1History.ts`, `migrations.ts`, `serviceAssertion.ts`, `customerRequestUnlisted.ts` | matching modules |

Crons in `convex/crons.ts`: readiness probes (1 min), inquiry abuse buckets (1 h), source-write nonces (1 h), OAuth grants (1 h), daily supplier settlement midnight UTC.

## Action plane (current registered ids)

`src/modules/actions/index.ts` registers then **filters out** `isQuarantineFamilyActionId` from `listActions()` / `listMcpActions()`. Quarantine prefixes: `customerRequest.`, `inquiry.`, `study.`, `workTree.` — keep `inquiry.readCustomerRecord` on HTTP/MCP.

**Live market / catalog actions (non-quarantine):**
...
- `supply.publish`, `supply.withdraw`, `supply.earnings` (from `src/modules/capability-supply/supply-actions.ts`)
- `inquiry.submit` is quarantine-family; `inquiry.readCustomerRecord` remains readable.
...
**Do this instead:** `POST /api/v1/operations/call` with `Authorization` and `idempotencyKey` (`OPERATION_INVOKE_ROUTE_CONTRACT.invoke`).
...
### Restore quarantine family HTTP as live write doors

**What happens:** `/api/v1/requests/*` or MCP `customerRequest.*` / `workTree.*` writes are reopened.
**Why it's wrong:** `isQuarantineSurfaceRetired` returns 410; paid successor is `/call`.
**Do this instead:** Market invoke + WorkTree/Study remain Convex/server-fn where still admitted; do not un-tombstone HTTP.
...
- Discriminated kernel results (`kind: 'ok' | 'refused' | 'error' | 'needs_authority' | …`) — do not throw for expected refusals on invoke/execute.
...
**Tombstones:**
- `/api/v1/operations/execute` — 410, successor `/call`.
- `/api/v1/requests/*` and `/api/requests/*` — 410 Customer Request family (`src/lib/server/customer-request-gone.ts`).
- Convex `http.ts` `/v1/*` and Convex `/mcp` — routing-v1 410 (`src/modules/routing-kernel/retirement.ts`). Live MCP is Start `/mcp`.

**Inquiry (human comms, not paid invoke):**
- Pages: `src/routes/$slug.inquiry.tsx`, owner `src/routes/_operator/owner.inquiries.tsx`.
- Actions: `submitInquiryAction` is quarantine-family on MCP/HTTP; UI uses `inquiry.functions.ts` + governed-action digest (`src/modules/governed-action/public.ts`).
- Notifications: `src/lib/server/notification-dispatch.ts` + `convex/notificationOutbox.ts`.
...
**Validation:** Zod at HTTP/action boundaries. JSON Schema 2020-12 inside capability contracts (`@cfworker/json-schema`). Convex `v.*` validators on functions; `v.any()` only at documented JSON boundaries (`scanTypeScriptStandards`). Exact optional properties / `noUncheckedIndexedAccess` in `tsconfig.json`.
...
**Observability of money and evidence:** Hashes and usage summaries are first-class; missing provider cost is unavailable, never zero (`model-gateway`). Operation results strip secret-like keys before model/UI (`operation-result-presentation.ts`).
