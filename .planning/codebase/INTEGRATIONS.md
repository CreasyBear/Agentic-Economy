...
- OpenRouter — chat completions and model catalog for answer agent and Customer Request semantic interpret/compile.
  ...
  - Auth: `OPENROUTER_API_KEY`; model overrides `AE_LLM_MODEL` / `AE_LLM_MODELS` / Convex `AE_CUSTOMER_REQUEST_MODEL`; optional `AE_OPENROUTER_API_BASE_URL`.
...
- Resend — owner/customer inquiry email send + delivery webhooks.
  - Client: raw `fetch` in `src/lib/server/notification-provider.ts`.
  ...
  - Outbox coordination: `AE_NOTIFICATION_OUTBOX_SECRET` + Convex `notification-outbox` module.
- Novu — workflow triggers / readback for inquiry notifications.
  - Client: raw `fetch` in `src/lib/server/notification-provider.ts`.
  - Route: `src/routes/api.notification.novu-dispatch.ts`.
  - Auth: `NOVU_SECRET_KEY`, `NOVU_WORKFLOW_INQUIRY_OWNER`, optional `NOVU_WORKFLOW_INQUIRY_CUSTOMER`, `NOVU_API_BASE_URL`.
...
- Route-call signing (Convex execution): `AE_ROUTE_CALL_SIGNING_SECRET`, `AE_ROUTE_CALL_SIGNING_KEY_ID` (required in production Convex env per release gate).
...
**Product analytics:**
- PostHog — `posthog-js` / `posthog-node`; config in `src/lib/observability/config.ts`, funnel helpers under `src/lib/observability/`.
...
- Observability: `SENTRY_*`, `VITE_SENTRY_DSN`, `POSTHOG_*` / `VITE_POSTHOG_*`
...
- Action surfaces: declared in `src/modules/*/…actions.ts`, registered in `src/modules/actions/index.ts` with surfaces `ui` | `http` | `agentJson` | `answerThread` (no separate `/api/agent/tools` route in current `src/routes/`)
- Convex HTTP: sandbox providers + retired `/v1/*` and `/mcp` stubs (`convex/http.ts`)

---

*Integration audit: 2026-07-18 (commit 3463c1d4)*
