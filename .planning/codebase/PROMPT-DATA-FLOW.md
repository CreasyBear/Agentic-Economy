...
|---|---|---|---|
| SSR/readback entry | `src/routes/t.$threadId.tsx:29-40,72-114` | thread route → `loadThreadRouteReadback` calls `getPublicThreadProjection`, builds public SEO, supplies `initialProjection` to `AeChat` → redacted SSR projection or unavailable page | domain projection + base router; source-shape |
...
| AI SDK v7 model loop | `src/modules/answer/internal/answer-tool-use-agent.ts:229-381,402-437` | gateway model, prompts, four tool specs, abort → `generateText` + `Output.object`, maxRetries 0, serialized tool queue, `prepareStep` removes tools for the final prose step, custom stop condition → typed `AnswerProse`, tool records, model observations | AI SDK mechanics + AE budgets/schema/gate; source-shape |
...
| gates | `src/modules/answer/internal/answer-gate.ts:17-49`; `src/modules/answer-thread/internal/answer-turn-safety.ts:17-49`; `src/modules/answer-thread/internal/turn-orchestrator.ts:389-431` | snapshot/prose/allowed slugs → sanitization, grounding, location, epistemic-vocabulary, and injection-upgrade checks → accepted snapshot + gate summary or typed error/copy ID | domain; source-shape |
...
| public projection/readback | `convex/answerThreads.ts:598-621`; `src/modules/answer-thread/internal/public-projection.ts:45-101`; `src/routes/api.answer.threads.$threadId.ts:11-29`; `src/routes/t.$threadId.tsx:90-114` | durable rows → capped ascending turns, redacted prose/artifacts/work log/check counts; raw prompts/tool payloads/hashes/private harness data are omitted → API/SSR public projection | domain projection + Convex base; source-shape |
...
The current answer read set is exactly four IDs: `registry.search`, `registry.detail`,
`sandbox.checkup_quote`, and `web.discover` (`src/modules/answer-thread/answer-thread.schema.ts:27-33`;
...
flowchart TD
  ...
  GRAPH --> PROPOSAL["model/deterministic proposal"] --> VALIDATE["strict semantic validation"]
...
| preview composition/projection | `src/modules/customer-request/application/interpret-compile/preview.ts:9-138`; `src/modules/customer-request/application/consumer-plan-projection.ts:152-211`; `src/components/ae/services/AeServiceList.tsx:13-50`; `src/components/ae/plan/AeConsumerPlan.tsx:22-65` | customer job/network → bounded graph, `proposeThenCompile(finalAttempt: true)`, max 32 steps/64 refs/5-minute expiry, public-supply join capped at three options per step → consumer-neutral frontier/queued/attention plan or needs-information/unavailable result | domain projection; source-shape |
...
| proposal ladder | `src/modules/customer-request/application/interpret-compile/interpret.ts:76-118,120-203`; `src/modules/customer-request/application/interpret-compile/interpreter.ts:22-62` | intent/amendment/graph → submit attempts once non-final then final; no provider key uses deterministic token matching; keyed path uses OpenRouter and final-attempt deterministic fallback → model/deterministic proposal or refusal | domain interpreter; source-shape |
...
| protocol | source evidence | input → processing → output | evidence ceiling |
|---|---|---|---|
...
| answer browser/SSR replay | React fetch/SSE, SSR loader, Convex query (`src/components/ae/chat/turn-stream-session.ts:24-98`; `src/routes/t.$threadId.tsx:90-114`; `convex/answerThreads.ts:598-621`) | server-wins projection merge, client abort boundary, no durable stopped claim | cold reload/readback packet; local session cache is not durability |
...
|---|---|---|
| answer admission | 100% reject invalid/over-bound bodies; body ≤16 KiB; duplicate client turn key does not re-admit within 30 s | route/body/guard source and fixture packet (`src/lib/server/bounded-request-body.ts:7-75`; `src/modules/answer-thread/internal/turn-guard.ts:11-98`) |
...
- Runtime/package truth: `package.json:60-159`; `package-lock.json:11-98,152-169,21435-21451,21688-21705`; `node_modules/ai/package.json:1-10,44-48`; `node_modules/@ai-sdk/provider-utils/package.json:1-49`; `node_modules/@openrouter/ai-sdk-provider/package.json:1-49`; `node_modules/promptfoo/package.json:42-47,319-344`.
...
- Answer SSR/browser seams: `src/routes/t.$threadId.tsx:29-40,90-114`; `src/components/ae/chat/answer-stream.ts:62-93`; `src/components/ae/chat/turn-stream-session.ts:24-98`; `src/components/ae/chat/AeThreadTurnStreamSection.tsx:94-183`; `src/components/ae/chat/AeChat.tsx:127-166,208-235,630-668`.
