...
| Retrieval first | `src/modules/answer-thread/internal/turns/retrieval-first.ts:41-175,256-308`; `src/modules/answer-thread/internal/tool-runner.ts:145-229`; `src/modules/storefront/storefront.actions.ts:320-329`; `src/modules/storefront/internal/business-enrichment.ts:261-319` | route, plan, query/context | registry search through harness; deterministic hit; qualifying empty may call registered `web.discover`; both registry and web paths use canonical tool execution, persisted tool records, and private model observations | snapshot, imported claims, canonical tool records, or no snapshot | domain + base |
...
| AI SDK loop | `src/modules/answer/internal/answer-tool-use-agent.ts:112-121,249-361,409-465`; `src/modules/answer-thread/internal/turns/agent.ts:78-93` | prompts, OpenRouter model, tools, abort | missing-key fail closed; one guarded `generateText` sequence with tools + `Output.object`; bounded `stopWhen`; final tool-less structured step; every SDK step maps to one private model-request observation with usage/cost or an explicit unavailable reason | typed prose, canonical action tool records, exact per-provider-request accounting; thrown failures remain in harness events | **lib** loop + domain budgets/schema |
...
| Case catalogs | `eval/answer/lib/cases.ts:153-184,202-307`; `eval/answer/lib/coverage.ts:61-148`; `eval/answer/lib/suite.ts:120-151` | turn/thread/harness case declarations | execute turn/thread cases; direct retrieval requires exactly 0 model requests; Paramata recovery requires exactly 2 provider requests and 2 persisted tool runs; qualifying empty retrieval preserves separate `registry.search` and `web.discover` records; audit harness metadata coverage separately | executable cases + coverage/count verdict | domain eval |
| In-process answer evaluator | `tests/helpers/answer-turn-stream.ts:3-20`; `eval/answer/lib/evaluators.ts:127-151,491-628,658-837`; `src/routes/api.answer.turn.ts:43-145` | case query/context + deterministic fixture | isolate the local registry from ambient Convex URLs, install in-memory answer-thread state, start `performance.now()` immediately before route invocation, mark first parsed non-`thread` frame, drain terminal response, then read the persisted harness summary; reject wrong counts, non-finite metrics, and model work lacking either cost or an explicit unavailability reason | answer turn result + evidence + finite request clocks, exact counts, usage, and cost evidence | base test seam + shared domain |
| Shared harness/finalization | `src/modules/answer-thread/internal/turn-orchestrator.ts:449-508`; `src/modules/answer-thread/internal/answer-turn-finalization.ts:150-329`; `tests/helpers/answer-thread-test-port.ts:14-109`; `src/modules/harness/run-collector.ts:242-266,404-448` | evaluator route result | run Flow A harness and finalization; test port stores evidence in memory; each AI SDK provider step is recorded once before persistence, and the private harness summary aggregates exact model/tool counts, usage, and cost once | in-memory frozen evidence/harnessRun plus private aggregate summary | shared base/domain |
...
| Score/report | `eval/answer/lib/suite.ts:38-76,135-174,176-218,325-398`; `tests/eval/answer-pipeline.test.ts:84-216`; `eval/answer/scripts/run-suite.ts:9-25` | evaluated cases + coverage | calculate thresholds/outcomes; aggregate exact model/tool counts, usage, estimated cost, sorted cost-unavailable reasons, and `performanceByPath` p95/max; preserve report redaction and existing `ok` semantics | sanitized `answer-eval-suite-report:v3` with per-turn and aggregate evidence | domain eval + base runner |
...
| Hosted answer-runtime smoke | `tests/deploy-smoke/answer-runtime-production-smoke.spec.ts:34-95,205-251,353-426`; `src/lib/dev/local-e2e-business-fixtures.ts:32-142` | deployment base URL + optional selection seed | fetch live public catalog; exclude every known development fixture slug; choose a unique category/locality subject deterministically; exercise exact retrieval then bounded typo/model recovery through public UI; require terminal citations, fresh readback, no consequential effect, and a receipt | deployment-only proof when executed, or failed/unavailable smoke | base + domain |
...
| Model telemetry | AI SDK `onStepEnd` owns provider-step completion and usage; AE maps each step one-for-one into the private harness collector; eval exports only aggregate counts/usage/cost and route clocks; no `@ai-sdk/otel` dependency or deployment trace | `src/modules/answer/internal/answer-tool-use-agent.ts:251-274,409-465`; `src/modules/answer-thread/internal/turns/agent.ts:83-90`; `src/modules/harness/run-collector.ts:404-448`; `eval/answer/lib/evaluators.ts:127-151` | **lib** usage / **base** journal / **domain** eval; preserve one record per provider request |
| Timeout plumbing | semantic transport uses SDK timeout + abort; the answer model sequence runs inside the harness guard and passes its abort signal into the SDK | `src/modules/customer-request/openrouter-transport.ts:74-89`; `src/modules/answer/internal/answer-tool-use-agent.ts:294-303,409-420` | **base**; keep one cancellation source per model sequence |
...
| C2 | an outer harness model envelope previously collapsed a multi-step AI SDK call into one request with zero usage/cost | resolved: the guard is now a non-counting harness phase and each SDK step emits exactly one model-request observation before persistence; v3 reads the aggregate once (`src/modules/answer/internal/answer-tool-use-agent.ts:409-465`; `src/modules/answer-thread/internal/turns/agent.ts:83-90`; `eval/answer/lib/evaluators.ts:127-151`) | harness base/eval; preserve one-to-one provider-request accounting without adding OTel |
...
| C6 | local captures can be mistaken for hosted/provider proof (`tests/eval/answer-pipeline.test.ts:84-194`; `tests/deploy-smoke/answer-runtime-production-smoke.spec.ts:34-95`) | explicit proof ceiling: local route/harness and Promptfoo remain source/mock; hosted selection excludes every shared development fixture; runtime-selected public smoke counts only when its receipt/readback exists | release/eval owner; never upgrade absent smoke to deployed |
...
- 2026-08-02 completion audit: registered `web.discover` now preserves canonical tool-call and private model-request evidence; the answer harness records one observation per AI SDK provider step; eval requires exact counts and explicit model cost evidence; hosted smoke excludes all development fixtures; the Vercel function/runtime contract is pinned to Node 22 (`src/modules/answer-thread/internal/tool-runner.ts:145-229`; `src/modules/answer-thread/internal/turns/agent.ts:83-90`; `eval/answer/lib/evaluators.ts:727-776`; `tests/deploy-smoke/answer-runtime-production-smoke.spec.ts:216-241`; `package.json:156-159`; `vite.config.ts:57-66`).
