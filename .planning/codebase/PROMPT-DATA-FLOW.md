...
| request/session | `src/routes/api.answer.turn.ts`; `src/components/ae/chat/answer-stream.ts`; `src/components/ae/chat/AeChat.tsx` | body `{query, threadId?, searchContext?}`, owner cookie, and required bounded `X-AE-Turn-Key` → resolve pseudonymous owner and canonical request digest → reserved/replayed admission or typed RFC 9457 refusal; initial URL asks derive the key from a page-random scope plus query, while submitted turns mint a fresh random key | base + domain; source-shape |
