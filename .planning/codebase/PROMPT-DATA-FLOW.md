...
| Prompt assembly | `src/modules/answer/internal/answer-llm-prompts.ts:1-126`; `src/modules/answer/internal/answer-tool-use-agent.ts:246-357,696-711` | agent query/context, intent, tool results | catalog-only instructions policy, canonical read-tool names, inert data, JSON-replacer string sanitization, strict AnswerProse shape | instructions + user prompt payloads | domain |
| AI SDK loop | `src/modules/answer/internal/answer-tool-use-agent.ts:114-123,246-357,405-454` | prompts, OpenRouter model, tools, abort | missing-key fail closed; one `generateText` with tools + `Output.object`; bounded `stopWhen`; final tool-less structured step; `onStepEnd` usage | typed prose, tool records, per-step accounting; thrown failures remain in harness events | **lib** loop + domain budgets/schema |
...
| In-process answer evaluator | `tests/helpers/answer-turn-stream.ts:3-20`; `eval/answer/lib/evaluators.ts:463-572,630-749`; `src/routes/api.answer.turn.ts:43-145` | case query/context + deterministic fixture | install in-memory answer-thread port, start `performance.now()` immediately before route invocation, mark first parsed non-`thread` frame through the optional callback, drain terminal response, then read harness summary and enforce expectations | answer turn result + evidence + finite request-to-first-progress/request-to-completion metrics, counts, usage, and cost | base test seam + shared domain |
...
| Promptfoo direct probe | `eval/answer/promptfooconfig.yaml:152-182`; `eval/answer/providers/gate.mjs:7-26`; `eval/answer/scripts/run-case.ts:1-6`; `eval/answer/lib/evaluators.ts:759-878` | Promptfoo vars + fixture server | direct `runAnswerToolUseAgent`; bypass route/harness/persistence; source/mock model behavior only | in-memory tool input/gate/slug/accounting assertion, never route or deployment proof | eval probe + **lib** agent |
