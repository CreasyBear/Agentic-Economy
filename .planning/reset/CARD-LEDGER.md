...
| P1-fix-held-charge | Leased refuseBeforeClaim restores or freezes an existing authorizeInvocationCharge hold | P1-d2 | committed |
| P1-d3 | Idempotent daily settlement cron | P1-d2 | committed |
...
| Card | Concern | Depends on | Status |
| --- | --- | --- | --- |
| P3-transport-proof | Port only the market-owned claim, release, registered-transport, output, and redaction invariants from `transport-canonical.test.ts` to atomic worker/transport proof | P2 | committed |
| P3-cancel-proof | Map only the atomic status, cancel, and reconcile invariants from `cancellation-canonical.test.ts`; explicitly exclude Customer Request provider-cancellation orchestration | P2 | committed |
| P3-invoke-proof | Map only the single-operation identity, authority, replay, settlement, and recovery invariants from `customer-request-v2-multi-capability-route.test.ts`; exclude planning/DAG/confirmation/repeat/problem/support/progress semantics | P2 | committed |
| P3-cutover | After all three proof cards, replace the three Customer Request entries in `npm run test:conformance` and the product-frontier `requiredConformancePaths` (24→27 paths) with these seven atomic successors: `tests/unit/convex/capability-operation-worker.test.ts`, `tests/integration/capability-operation-workpool.test.ts`, `tests/unit/capability-execution/operation-recovery-actions.test.ts`, `tests/unit/capability-execution/operation-invoke.test.ts`, `tests/unit/server/operation-invoke-api.test.ts`, `tests/unit/action-invocation/durable-action-invocation.test.ts`, `tests/unit/convex/capability-operation-recovery.test.ts` | P3-transport-proof, P3-cancel-proof, P3-invoke-proof | committed |
| P3-val | Re-run full conformance floor (≥10); assert no path lost without equivalent | P3-cutover | committed |
| P3-rev | Ports do not weaken assertions or substitute fixtures for live kernel proof | P3-val | committed |
...
| P4-c | Drain in-flight router-named checkpoints; migrate thread tool IDs / optional intent; retain thread storage | P4-b | committed |
| P4-d | Replace named router files with one bounded AI SDK tool loop | P4-c | committed |
| P4-e | MCP/CLI/chat parity + "chat has no tool MCP lacks" structural assertion | P4-d | committed |
| P4-f | Review: no chat-only market capability; no quarantined surface reintroduced | P4-e | committed |

## Phase 5 — Quarantine and deprecate

| Card | Concern | Depends on | Status |
| --- | --- | --- | --- |
| P5-a | Three-artifact frontier v2 receipt: manifest, `verify-product-frontier.mjs`, `product-frontier-manifest.test.ts` | P3-rev, P4 | committed |
...
| P5-d | RFC 9457 HTTP 410 on mutating family doors and `/execute`; Sunset pulled to 18 Aug 2026; never `/call` | P6-export-tool | pending |
...
| P6-export-tool | Checksummed per-table export toolchain; hash-only for governedSendReceiptKeys wrap fields | P6-freeze-gaps | pending |
