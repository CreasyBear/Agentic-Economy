...
| Card | Concern | Depends on | Status |
| --- | --- | --- | --- |
| P3-transport-proof | Port only the market-owned claim, release, registered-transport, output, and redaction invariants from `transport-canonical.test.ts` to atomic worker/transport proof | P2 | committed (`ca212e9`) |
| P3-cancel-proof | Map only the atomic status, cancel, and reconcile invariants from `cancellation-canonical.test.ts`; explicitly exclude Customer Request provider-cancellation orchestration | P2 | committed (`ca212e9`) |
| P3-invoke-proof | Map only the single-operation identity, authority, replay, settlement, and recovery invariants from `customer-request-v2-multi-capability-route.test.ts`; exclude planning/DAG/confirmation/repeat/problem/support/progress semantics | P2 | committed (`ca212e9`) |
