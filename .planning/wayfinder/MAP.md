...
- Skills every session must consult: `ae-agent-surfaces`, `ae-agentic-payments-stack`, `ae-public-copy-guardrails`, `ae-verification-gates`, `wayfinder`.
...
- One-feature product — `/` is THE view: the human twin of `/api/v1/services` (search → priced service rows → instant sandbox quote), loader uses the same `projectPublicServicesPage` projection as the API. `/registry`, `/for-agents`, `/about`, `/help` 301 to `/`. Decided 2026-07-30 under founder directive.
