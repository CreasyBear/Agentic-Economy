...
Options to grill:
- **Inbound x402** on open endpoints (parity-maximal; crypto custody, settlement, and outcome-unknown/reconciliation obligations follow — see `ae-agentic-payments-stack` boundaries).
...
Constraints from the payments skill: credentials server-side, exact amount/currency ceiling per admitted step, outcome-unknown after release requires reconciliation, no claim of customer-reachable payment until the intended surface is proven.
...
- No claim of customer-reachable payment until the intended surface is proven (`ae-agentic-payments-stack` ceiling).
