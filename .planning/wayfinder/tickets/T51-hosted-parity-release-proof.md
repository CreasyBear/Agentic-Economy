...
Status: framework cutover and T51 harness are merged on `main` through `8a9a937a8d19cd94f44be529dfcb15b3d515785c`; the workflow fails closed instead of skipping hosted proof. GitHub Actions run `30780035974` passed the complete credential-free source job and exact-revision Vercel deploy; the Convex deploy passed bundle typechecking after the previous repairs, then correctly rejected a production `registry-search-document:v2` row removed from the current schema before any hosted mutation. The schema is now widened to accept the exact historical aggregate document alongside current per-Offering writes, with executable schema coverage; local schema tests (7), typecheck, and lint pass. No hosted parity claim is made until a new exact-SHA run deploys Convex, passes the production lifecycle and T51 parity jobs, and retains the sanitized packet.
