...
Status: framework cutover and T51 harness are merged on `main` through `0bcb4608f980ad57b14f8533bec74dce28e58950`; the workflow fails closed instead of skipping hosted proof. GitHub Actions run `30782467940` passed the complete source job, Vercel and Convex deploys, production execution configuration, and the full governed acceptance seed including immutable identity rotation and retirement. Its readiness poll then inspected only the first 10 of more than 18 eligible publications, so the two exact route publications were outside the bounded result even though the seed returned both exact refs. The poll bound is now 32, matching the existing supply ceiling; focused workflow contract tests (13) pass. No hosted parity claim is made until a new exact-SHA run passes the production lifecycle and T51 parity jobs and retains the sanitized packet.
