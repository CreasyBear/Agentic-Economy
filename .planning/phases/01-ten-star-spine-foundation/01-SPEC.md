...
Current repo state is planning-only. `node .codex/gsd-core/bin/gsd-tools.cjs init phase-op 1` found Phase 1 at `.planning/phases/01-ten-star-spine-foundation`; `query find-phase 1` found one executable slice: `01-01-source-mining-ledger-PLAN.md`. The fresh repo has no `package.json`, `apps/`, `src/`, `convex/`, or `tests/` runtime tree yet. `.planning/REQUIREMENTS.md` is referenced by GSD init but is absent, so this SPEC becomes the locked Phase 1 what/why source for discuss/plan.

Authority docs already exist for the phase boundary: `PROJECT.md`, `ENGINEERING-STANDARDS.md`, `SOURCE-MINING.md`, `source-mining/phase-1-ledger.md`, `SECURITY-SPEC.md`, `AI-SPEC.md`, `SEO-AEO-SPEC.md`, `GTM-READINESS.md`, `PRODUCT-LENS.md`, `ROADMAP.md`, `STATE.md`, `PHASE.md`, and `PREMORTEM.md`. The handoff mentioned `FABLE-5-FOUNDATION-REVIEW.md`, but that file is not present in the phase directory; the current review lens source is `PRODUCT-LENS.md`, including the Matt Pocock TypeScript lens and the explicit `/mattpocock-review` PR08 gate in `PHASE.md`.
